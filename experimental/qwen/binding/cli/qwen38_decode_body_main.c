...
#include "experimental/qwen/programs/qwen38_text_decode_body_source.h"
...
static iree_status_t qwen38_decode_body_run(void) {
  ...
  if (FLAG_transition_count < 1 ||
      FLAG_transition_count > QWEN38_DECODE_CAPACITY) {
  ...
  if (iree_status_is_ok(status)) {
    ...
    if (program_info->rebindable_binding_count != 5 ||
      ...
      status = iree_make_status(
          IREE_STATUS_FAILED_PRECONDITION,
          "the decode body command program published an incompatible ABI");
    }
  }
  ...
  if (iree_status_is_ok(status)) {
    ...
    if (finite_count != QWEN38_HIDDEN_ELEMENT_COUNT) {
      status = iree_make_status(IREE_STATUS_DATA_LOSS,
                                "the Qwen3.8 body produced %" PRIhsz
                                "/%d finite values",
...
int main(int argc, char** argv) {
  ...
  if (iree_status_is_ok(status)) status = qwen38_decode_body_run();
  if (!iree_status_is_ok(status)) {

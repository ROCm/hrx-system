...
#include "experimental/qwen/programs/qwen38_text_prefill_body_source.h"
...
#define QWEN38_PREFILL_CAPACITY 512
...
IREE_FLAG(int32_t, position_base, 0,
          "Absolute position of the first input token.");

static const char* const qwen38_prefill_body_usage =
    "Advances the complete Qwen3.8 base model over one causal token span "
...
static iree_status_t qwen38_prepare_prefill_body(
    qwen_tooling_runtime_context_t* runtime_context,
    iree_allocator_t host_allocator,
    qwen_tooling_command_program_t** out_program) {
  if (qwen38_text_prefill_body_source_size() != 1) {
    return iree_make_status(
        IREE_STATUS_FAILED_PRECONDITION,
        "Qwen3.8 prefill body source must contain exactly one file");
  }
  ...
  const qwen_tooling_command_program_options_t options = {
      ...
      .root_name = IREE_SV("qwen38_text_prefill_body"),
  };
...
static iree_status_t qwen38_prefill_body_run(void) {
  ...
  if (FLAG_token_count < 1 || FLAG_token_count > QWEN38_PREFILL_CAPACITY) {
    status = iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                              "--token_count must be between 1 and %d",
                              QWEN38_PREFILL_CAPACITY);
  } else if (FLAG_position_base < 0 ||
             FLAG_position_base > QWEN38_PREFILL_CAPACITY - FLAG_token_count) {
    status = iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "--position_base plus --token_count must fit capacity %d",
        QWEN38_PREFILL_CAPACITY);
  }
  if (iree_status_is_ok(status)) {
  ...
  if (iree_status_is_ok(status)) {
    fprintf(stderr,
            ...
            "prefill program...\n");
    status =
        qwen38_prepare_prefill_body(&runtime_context, host_allocator, &program);
  }
  if (iree_status_is_ok(status)) {
  ...
  if (iree_status_is_ok(status)) {
    status = qwen38_allocate_buffer(
        device, IREE_HAL_MEMORY_TYPE_DEVICE_LOCAL,
        IREE_HAL_BUFFER_USAGE_STORAGE | IREE_HAL_BUFFER_USAGE_TRANSFER,
        /*minimum_alignment=*/256, QWEN38_GDN_STATE_BYTE_LENGTH,
        &gdn_state_buffer);
  }
  if (iree_status_is_ok(status)) {
    status = qwen38_allocate_buffer(
  ...
  if (iree_status_is_ok(status)) {
    status = qwen38_fill_and_wait(device, gdn_state_buffer);
  }
  if (iree_status_is_ok(status)) {
  ...
  if (iree_status_is_ok(status)) {
    status = qwen38_upload_control(program, program_info, FLAG_token_count,
                                   FLAG_position_base, control_data_offset,
  ...
  if (iree_status_is_ok(status)) {
    fprintf(stderr,
            ...
            " commands; executing all 64 layers for %d token(s)...\n",
    ...
    const iree_hal_buffer_binding_t bindings[6] = {
        ...
        [2] =
            {
                .buffer = gdn_state_buffer,
                .offset = 0,
                .length = IREE_HAL_WHOLE_BUFFER,
            },
        [3] =
            {
                .buffer = attention_cache_buffer,
                .offset = 0,
                .length = IREE_HAL_WHOLE_BUFFER,
            },
        [4] =
  ...
  if (iree_status_is_ok(status)) {
    ...
    if (finite_count != hidden_element_count) {
      status = iree_make_status(IREE_STATUS_DATA_LOSS,
                                "the Qwen3.8 body produced %" PRIhsz "/%" PRIhsz
                                " finite values",
  ...
  if (iree_status_is_ok(status)) {
    fprintf(
        stdout,
        "Qwen3.8 prefill processed %d token(s): %" PRIhsz " commands, %" PRIhsz
...
int main(int argc, char** argv) {
  iree_flags_set_usage("qwen38-prefill-body-cli", qwen38_prefill_body_usage);
  ...
  if (iree_status_is_ok(status)) status = qwen38_prefill_body_run();
  if (!iree_status_is_ok(status)) {

...
// Process-local services created from standard IREE tooling flags.
//
// A context owns exactly one HAL device group and one anonymous parameter
// index/provider pair. The model and every request/program using the context
// must be released before the context is deinitialized.

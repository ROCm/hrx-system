...
| `routed_down_q6_next_q8_bringup_workaround.loom` | `qwen3_moe/routed_down_q6k.loom` and `qwen3_moe/routed_down_next_q8.loom` | The same source-to-low specialization gap blocks the storage-selected Q6_K direct route. The one-function fork makes only the model dimensions structural and delegates both contraction and next-row completion to canonical device templates. |

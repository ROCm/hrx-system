...
static iree_string_view_t ShortFullRequestJson() {
  return IREE_SV(
      ...
      "\"latent_height\":8,\"sampler\":\"V4_DEFAULT_20\",\"seed\":1}}");
}

static iree_string_view_t ManyStepFullRequestJson() {
  return IREE_SV(
      "{\"prompt\":\"a city\",\"generation\":{\"latent_width\":8,"
      "\"latent_height\":8,\"sampler\":\"V4_QUALITY_48\",\"seed\":1}}");
}
...
static iree_string_view_t WideLongFullRequestJson() {
  return IREE_SV(
      ...
      "\"latent_height\":8,\"sampler\":\"V4_TURBO_12\",\"seed\":2}}");
}
...
static id4_ideogram4_generation_residency_selection_t SelectGenerationResidency(
    const id4_ideogram4_generation_plan_t* plan,
  ...
  id4_ideogram4_generation_residency_select_options_t options;
  std::memset(&options, 0, sizeof(options));
  options.structure_size = sizeof(options);
...
class SessionTest : public ::testing::Test {
 protected:
  void SetUp() override {
    device_group_ = id4::test::CreateGpuPlanningDeviceGroup();
...
TEST_F(SessionTest, PlansGenerationFromDynamicPromptLength) {
  ...
  EXPECT_EQ(long_summary.denoise_step_count, 20u);
  ...
  EXPECT_EQ(long_summary.vae_attention_implementation,
            ID4_VAE_ATTENTION_IMPLEMENTATION_MATERIALIZED);
...
TEST_F(SessionTest, PlansGenerationBoundaryShapesFromDynamicLatentShape) {
  ...
  EXPECT_EQ(summary.denoise_step_count, 12u);
...
TEST_F(SessionTest, SelectsGenerationResidencyWithinBudget) {
  TokenizerPtr tokenizer = LoadTokenizer();
  ScopedRequest request;
  IREE_ASSERT_OK(id4_ideogram4_request_parse_json(
      ShortFullRequestJson(), iree_allocator_system(), &request.value));
...
TEST_F(SessionTest, ResidencySelectionRejectsImpossibleBudget) {
  ...
  id4_ideogram4_generation_resource_statistics_t issue_phase_statistics =
      EstimateGenerationResources(
          plan.get(), ID4_IDEOGRAM4_GENERATION_RESIDENCY_MODE_ISSUE_PHASES,
          ID4_IDEOGRAM4_GENERATION_RESIDENT_STAGE_NONE);
  ASSERT_GT(issue_phase_statistics.stage_serial_total_peak_byte_length, 0u);
  ...
  IREE_EXPECT_STATUS_IS(IREE_STATUS_RESOURCE_EXHAUSTED,
                        id4_ideogram4_generation_plan_select_residency(
                            plan.get(), &options, &selection));
}

...
"""ID4 project presubmit entry point."""
...
PROJECT_NAME = "id4"
PROJECT_ROOT = "experimental/id4/"
CMAKE_TEST_REGEX = "^experimental/id4/"
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run ID4 project presubmit.")
    ...
    parser.add_argument("--tests", action="store_true", help="Run ID4 tests.")
    parser.add_argument(
        "--files-from",
...
def bazel_build_command() -> list[str]:
    return ["bazel", "build", "//experimental/id4/..."]


def bazel_test_command() -> list[str]:
    return ["bazel", "test", "--config=presubmit", "//experimental/id4/..."]
...
def main() -> int:
    ...
    if not should_run_tests(args.files_from):
        print("id4 presubmit: no ID4-affecting files")
        return 0
    if args.lane == "bazel":
        ok = run_bazel_tests()

...
TEST(PlanTest, ReportsParameterWindowStatistics) {
  DeviceGroupPtr device_group(id4::test::CreateLocalSyncDeviceGroup(),
                              iree_hal_device_group_release);
  ...
  const id4_pipeline_region_plan_t regions[] = {
      {
          ...
          // Placement containing this region.
          /*.placement_id=*/0,
          // Binding capacity covering parameter and local slots.
          /*.binding_capacity=*/2,
          // No local slab is bound by this synthetic region.
          /*.local_binding_slot=*/1,
          // No local tensors are allocated by this synthetic region.
          /*.local_tensor_alignment=*/0,
          // Dry-run region statistics.
          /*.statistics=*/{},
          // No local tensor lifetime records.
          /*.local_lifetime_count=*/0,
          // No local tensor lifetime records.
          /*.local_lifetimes=*/nullptr,
          // Region 0 reads parameter load group 0.
          /*.parameter_load_group_count=*/IREE_ARRAYSIZE(region0_load_groups),
          // Region 0 parameter load group ordinals.
          /*.parameter_load_groups=*/region0_load_groups,
      },
      {
          // Human-readable region name.
          ...
          // No local tensor lifetime records.
          /*.local_lifetimes=*/nullptr,
          // Region 1 reads parameter load group 1.
          /*.parameter_load_group_count=*/IREE_ARRAYSIZE(region1_load_groups),
          // Region 1 parameter load group ordinals.
          /*.parameter_load_groups=*/region1_load_groups,
      },
      {
          // Human-readable region name.
          /*.name=*/IREE_SV("region2"),
          // No source program backs this synthetic plan.
          /*.source_operation_offset=*/0,
          // No source program backs this synthetic plan.
          /*.source_operation_count=*/0,
          ...
          // No local tensor lifetime records.
          /*.local_lifetimes=*/nullptr,
          // Region 2 reads group 1 again and introduces group 2.
          /*.parameter_load_group_count=*/IREE_ARRAYSIZE(region2_load_groups),
          // Region 2 parameter load group ordinals.
          /*.parameter_load_groups=*/region2_load_groups,
      },
  };
  ...
  IREE_ASSERT_OK(id4_pipeline_plan_parameter_window_statistics(
      plan.get(), /*region_window_size=*/1, &statistics));
  EXPECT_EQ(statistics.region_window_size, 1u);
  EXPECT_EQ(statistics.window_count, 3u);
  EXPECT_EQ(statistics.full_slab_target_byte_length, 600u);
  EXPECT_EQ(statistics.peak_window_target_byte_length, 500u);
  EXPECT_EQ(statistics.peak_window_source_byte_length, 500u);
  EXPECT_EQ(statistics.total_window_target_byte_length, 800u);
  EXPECT_EQ(statistics.total_window_source_byte_length, 800u);
  EXPECT_EQ(statistics.peak_window_load_group_count, 2u);
  EXPECT_EQ(statistics.total_window_load_group_count, 4u);
  EXPECT_EQ(statistics.peak_window_encode_load_step_count, 0u);
  ...
  EXPECT_EQ(statistics.window_count, 2u);
  EXPECT_EQ(statistics.peak_window_target_byte_length, 500u);
  EXPECT_EQ(statistics.total_window_target_byte_length, 800u);
  EXPECT_EQ(statistics.total_window_load_group_count, 4u);

  IREE_ASSERT_OK(id4_pipeline_plan_parameter_window_statistics(
      plan.get(), /*region_window_size=*/3, &statistics));
  EXPECT_EQ(statistics.window_count, 1u);
  EXPECT_EQ(statistics.peak_window_target_byte_length, 600u);
  EXPECT_EQ(statistics.total_window_target_byte_length, 600u);
  EXPECT_EQ(statistics.total_window_load_group_count, 3u);

  IREE_EXPECT_STATUS_IS(IREE_STATUS_INVALID_ARGUMENT,
                        id4_pipeline_plan_parameter_window_statistics(
                            plan.get(), /*region_window_size=*/0, &statistics));

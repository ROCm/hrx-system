...
struct id4_pipeline_plan_t {
  ...
  id4_pipeline_parameter_request_table_t* parameter_request_tables;
  // Number of planned parameter tensors.
  iree_host_size_t parameter_tensor_count;
  // Planned parameter tensors owned by this plan.
...
static void id4_pipeline_plan_destroy(id4_pipeline_plan_t* plan) {
  ...
  for (iree_host_size_t i = 0; i < plan->parameter_slab_count; ++i) {
    ...
    id4_pipeline_parameter_request_t* requests =
        (id4_pipeline_parameter_request_t*)request_table->values;
    for (iree_host_size_t j = 0; j < request_table->count; ++j) {
      id4_pipeline_string_release(requests[j].key, host_allocator);
    }
    iree_allocator_free(host_allocator, requests);
  }
  iree_allocator_free(host_allocator, plan->parameter_request_tables);
  iree_allocator_free(host_allocator, plan->parameter_slabs);
  for (iree_host_size_t i = 0; i < plan->parameter_tensor_count; ++i) {
    id4_pipeline_string_release(plan->parameter_tensors[i].layout.name,
                                host_allocator);
  }
  iree_allocator_free(host_allocator, plan->parameter_tensors);
  for (iree_host_size_t i = 0; i < plan->placement_count; ++i) {
...
static iree_status_t id4_pipeline_plan_copy_parameter_slabs(
  ...
  if (!options->parameter_request_tables) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "parameter request table array is required");
  }
  IREE_RETURN_IF_ERROR(iree_allocator_malloc_array(
  ...
  IREE_RETURN_IF_ERROR(iree_allocator_malloc_array(
      plan->host_allocator, plan->parameter_slab_count,
      sizeof(plan->parameter_request_tables[0]),
      (void**)&plan->parameter_request_tables));
  memset(plan->parameter_slabs, 0,
         plan->parameter_slab_count * sizeof(plan->parameter_slabs[0]));
  memset(
      plan->parameter_request_tables, 0,
      plan->parameter_slab_count * sizeof(plan->parameter_request_tables[0]));
  for (iree_host_size_t i = 0; i < plan->parameter_slab_count; ++i) {
    const id4_pipeline_parameter_slab_plan_t* source =
        &options->parameter_slabs[i];
    const id4_pipeline_parameter_request_table_t* source_request_table =
        &options->parameter_request_tables[i];
    IREE_RETURN_IF_ERROR(
        id4_pipeline_parameter_slab_validate(source, plan->placement_count));
    if (source_request_table->count != 0 && !source_request_table->values) {
      return iree_make_status(
          IREE_STATUS_INVALID_ARGUMENT,
          "parameter request table %" PRIhsz " values are required", i);
    }
    ...
    *target = *source;
    ...
    if (target_request_table->count == 0) continue;
    id4_pipeline_parameter_request_t* requests = NULL;
    IREE_RETURN_IF_ERROR(iree_allocator_malloc_array(
        plan->host_allocator, target_request_table->count, sizeof(requests[0]),
        (void**)&requests));
    memset(requests, 0, target_request_table->count * sizeof(requests[0]));
    target_request_table->values = requests;
    for (iree_host_size_t j = 0; j < target_request_table->count; ++j) {
      requests[j].span = source_request_table->values[j].span;
      IREE_RETURN_IF_ERROR(
          id4_pipeline_string_clone(source_request_table->values[j].key,
                                    plan->host_allocator, &requests[j].key));
    }
  }
  return iree_ok_status();
}

static iree_status_t id4_pipeline_plan_copy_parameter_tensors(
    id4_pipeline_plan_t* plan,
    const id4_pipeline_plan_create_options_t* options) {
  plan->parameter_tensor_count = options->parameter_tensor_count;
  if (plan->parameter_tensor_count == 0) return iree_ok_status();
  if (plan->parameter_slab_count == 0) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "parameter tensors require at least one parameter slab");
  }
  if (!options->parameter_tensors) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "parameter tensor array is required");
  }
  IREE_RETURN_IF_ERROR(iree_allocator_malloc_array(
      plan->host_allocator, plan->parameter_tensor_count,
      sizeof(plan->parameter_tensors[0]), (void**)&plan->parameter_tensors));
  memset(plan->parameter_tensors, 0,
         plan->parameter_tensor_count * sizeof(plan->parameter_tensors[0]));
  iree_host_size_t global_request_count = 0;
  for (iree_host_size_t i = 0; i < plan->parameter_slab_count; ++i) {
    if (!iree_host_size_checked_add(global_request_count,
                                    plan->parameter_request_tables[i].count,
                                    &global_request_count)) {
      return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                              "global parameter request count overflows");
    }
  }
  for (iree_host_size_t i = 0; i < plan->parameter_tensor_count; ++i) {
    const id4_pipeline_parameter_tensor_plan_t* source =
        &options->parameter_tensors[i];
    if (id4_pipeline_tensor_dtype_byte_length(source->layout.dtype) == 0) {
      return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                              "parameter tensor %" PRIhsz " dtype is invalid",
                              i);
    }
    if (iree_string_view_is_empty(source->layout.name)) {
      return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                              "parameter tensor %" PRIhsz " name is required",
                              i);
    }
    if (source->layout.byte_length == 0) {
      return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                              "parameter tensor %.*s byte length is zero",
                              (int)source->layout.name.size,
                              source->layout.name.data);
    }
    if (source->parameter_slab_index >= plan->parameter_slab_count) {
      return iree_make_status(
          IREE_STATUS_OUT_OF_RANGE,
          "parameter tensor %.*s slab index %" PRIhsz
          " exceeds slab count %" PRIhsz,
          (int)source->layout.name.size, source->layout.name.data,
          source->parameter_slab_index, plan->parameter_slab_count);
    }
    const id4_pipeline_parameter_slab_plan_t* slab =
        &plan->parameter_slabs[source->parameter_slab_index];
    const id4_pipeline_parameter_request_table_t* request_table =
        &plan->parameter_request_tables[source->parameter_slab_index];
    if (source->request_count == 0) {
      return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                              "parameter tensor %.*s request count is zero",
                              (int)source->layout.name.size,
                              source->layout.name.data);
    }
    if (source->request_offset > request_table->count ||
        source->request_count > request_table->count - source->request_offset) {
      return iree_make_status(
          IREE_STATUS_OUT_OF_RANGE,
          "parameter tensor %.*s request range [%" PRIhsz ", %" PRIhsz
          ") exceeds slab request count %" PRIhsz,
          (int)source->layout.name.size, source->layout.name.data,
          source->request_offset,
          source->request_offset + source->request_count, request_table->count);
    }
    if (source->offset > slab->byte_length ||
        source->layout.byte_length > slab->byte_length - source->offset) {
      return iree_make_status(
          IREE_STATUS_OUT_OF_RANGE,
          "parameter tensor %.*s byte range exceeds slab byte length",
          (int)source->layout.name.size, source->layout.name.data);
    }
    if (source->global_request_offset > global_request_count ||
        source->request_count >
            global_request_count - source->global_request_offset) {
      return iree_make_status(
          IREE_STATUS_OUT_OF_RANGE,
          "parameter tensor %.*s global request range [%" PRIhsz ", %" PRIhsz
          ") exceeds global request count %" PRIhsz,
          (int)source->layout.name.size, source->layout.name.data,
          source->global_request_offset,
          source->global_request_offset + source->request_count,
          global_request_count);
    }
    for (iree_host_size_t j = 0; j < source->request_count; ++j) {
      const id4_pipeline_parameter_request_t* request =
          &request_table->values[source->request_offset + j];
      if (request->span.buffer_offset < source->offset) {
        return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                                "parameter tensor %.*s request %" PRIhsz
                                " starts before tensor byte range",
                                (int)source->layout.name.size,
                                source->layout.name.data, j);
      }
      const iree_device_size_t request_tensor_offset =
          request->span.buffer_offset - source->offset;
      if (request_tensor_offset > source->layout.byte_length ||
          request->span.length >
              source->layout.byte_length - request_tensor_offset) {
        return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                                "parameter tensor %.*s request %" PRIhsz
                                " exceeds tensor byte range",
                                (int)source->layout.name.size,
                                source->layout.name.data, j);
      }
    }
    id4_pipeline_parameter_tensor_plan_t* target = &plan->parameter_tensors[i];
    ...
    target->parameter_slab_index = source->parameter_slab_index;
    target->request_offset = source->request_offset;
    target->request_count = source->request_count;
    target->global_request_offset = source->global_request_offset;
...
static iree_status_t id4_pipeline_plan_emit_parameter_slab_diagnostics(
  ...
  for (iree_host_size_t i = 0; i < plan->parameter_slab_count; ++i) {
    ...
    id4_pipeline_parameter_slab_diagnostic_t parameter_slab = {
        ...
        // Allocation planning is independent of provider scope.
        ...
        .request_count = request_table->count,
    };
...
iree_status_t id4_pipeline_plan_create(
  ...
  *out_plan = NULL;
  ...
  if (iree_status_is_ok(status)) {
    status = id4_pipeline_plan_copy_parameter_tensors(plan, options);
  }
  if (iree_status_is_ok(status)) {
...
id4_pipeline_plan_parameter_tensor_at(const id4_pipeline_plan_t* plan,
                                      iree_host_size_t index) {
  if (!plan || index >= plan->parameter_tensor_count) return NULL;
  return &plan->parameter_tensors[index];
}
...
static iree_device_size_t id4_pipeline_plan_load_step_target_length(
  ...
  const id4_pipeline_parameter_request_table_t* request_table =
      &plan->parameter_request_tables[step->target_slab_index];
  ...
  for (iree_host_size_t i = 0; i < step->request_count; ++i) {
    ...
    byte_length += request_table->values[request_index].span.length;
  }
  return byte_length;
}
...
static bool id4_pipeline_plan_region_window_has_prior_load_group(
    const id4_pipeline_plan_t* plan, iree_host_size_t window_start_region,
    iree_host_size_t current_region_index,
    iree_host_size_t current_group_ordinal) {
  const iree_host_size_t group_index =
      plan->regions[current_region_index]
          .parameter_load_groups[current_group_ordinal];
  for (iree_host_size_t region_index = window_start_region;
       region_index <= current_region_index; ++region_index) {
    const id4_pipeline_region_plan_t* region = &plan->regions[region_index];
    const iree_host_size_t group_ordinal_limit =
        region_index == current_region_index
            ? current_group_ordinal
            : region->parameter_load_group_count;
    for (iree_host_size_t group_ordinal = 0;
         group_ordinal < group_ordinal_limit; ++group_ordinal) {
      if (region->parameter_load_groups[group_ordinal] == group_index) {
        return true;
      }
    }
  }
  ...
  for (iree_host_size_t i = 0; i < step->request_count; ++i) {
    ...
    if (request_index >= request_table->count) {
      return iree_make_status(
          ...
          request_index, step->target_slab_index, request_table->count);
    }
...
static iree_status_t id4_pipeline_plan_parameter_load_group_byte_statistics(
  ...
  iree_host_size_t encode_load_step_count = 0;
  const iree_host_size_t step_limit = group.step_offset + group.step_count;
  for (iree_host_size_t step_index = group.step_offset; step_index < step_limit;
       ++step_index) {
    const id4_pipeline_parameter_load_step_t* step =
        &plan->parameter_load_steps[step_index];
...
iree_status_t id4_pipeline_plan_parameter_window_statistics(
  ...
  if (region_window_size == 0) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "region window size must be non-zero");
  }
  ...
  for (iree_host_size_t window_start_region = 0;
       window_start_region < plan->region_count;
       window_start_region += region_window_size) {
    ++statistics.window_count;
    const iree_host_size_t remaining_region_count =
        plan->region_count - window_start_region;
    const iree_host_size_t window_region_count =
        iree_min(region_window_size, remaining_region_count);
    const iree_host_size_t window_end_region =
        window_start_region + window_region_count;

    iree_device_size_t window_source_byte_length = 0;
    iree_device_size_t window_target_byte_length = 0;
    iree_host_size_t window_load_group_count = 0;
    iree_host_size_t window_encode_load_step_count = 0;
    for (iree_host_size_t region_index = window_start_region;
         region_index < window_end_region; ++region_index) {
      const id4_pipeline_region_plan_t* region = &plan->regions[region_index];
      for (iree_host_size_t group_ordinal = 0;
           group_ordinal < region->parameter_load_group_count;
           ++group_ordinal) {
        if (id4_pipeline_plan_region_window_has_prior_load_group(
                plan, window_start_region, region_index, group_ordinal)) {
          continue;
        }
        const iree_host_size_t group_index =
    ...
    statistics.peak_window_target_byte_length = iree_max(
    ...
    statistics.peak_window_encode_load_step_count =
        iree_max(statistics.peak_window_encode_load_step_count,
                 window_encode_load_step_count);
    statistics.total_window_encode_load_step_count +=
        window_encode_load_step_count;
  }
...
static iree_status_t id4_pipeline_plan_validate_parameter_request_table(
    const id4_pipeline_parameter_request_table_t* plan_table,
    const id4_pipeline_parameter_request_table_t* retained_table,
    iree_host_size_t slab_index) {
  if (!retained_table) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "parameter slab %" PRIhsz " has no retained request table", slab_index);
  }
  if (plan_table->count != retained_table->count) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "parameter slab %" PRIhsz " request count mismatch: plan has %" PRIhsz
        " requests but slab set has %" PRIhsz,
        slab_index, plan_table->count, retained_table->count);
  }
  for (iree_host_size_t i = 0; i < plan_table->count; ++i) {
    if (!id4_pipeline_parameter_requests_equal(&plan_table->values[i],
                                               &retained_table->values[i])) {
      return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
...
iree_status_t id4_pipeline_plan_validate_parameter_slabs(
  ...
  const bool has_deferred_load_context =
      id4_pipeline_parameter_slab_set_has_deferred_load_context(slab_set);
  for (iree_host_size_t i = 0; i < plan->parameter_slab_count; ++i) {
    IREE_RETURN_IF_ERROR(id4_pipeline_plan_validate_parameter_request_table(
        &plan->parameter_request_tables[i],
        id4_pipeline_parameter_slab_set_request_table_at(slab_set, i), i));
  }
  if (!has_deferred_load_context) return iree_ok_status();
...
id4_pipeline_plan_append_parameter_load_kind_statistics_json(
  ...
  const struct {
    ...
    id4_pipeline_parameter_load_step_kind_t kind;
      ...
      {IREE_SV("encode_fp8_e4m3_linear_rhs_tile"),
...
static iree_status_t id4_pipeline_plan_append_parameter_window_statistics_json(
    const id4_pipeline_plan_t* plan, iree_string_builder_t* builder) {
  const iree_host_size_t candidate_window_sizes[] = {
      1, 2, 4, 8, 16, plan->region_count,
  };
  iree_host_size_t emitted_window_sizes[IREE_ARRAYSIZE(candidate_window_sizes)];
  iree_host_size_t emitted_window_size_count = 0;

  IREE_RETURN_IF_ERROR(iree_string_builder_append_cstring(builder, "["));
  for (iree_host_size_t i = 0; i < IREE_ARRAYSIZE(candidate_window_sizes);
       ++i) {
    const iree_host_size_t window_size = candidate_window_sizes[i];
    if (window_size == 0 || window_size > plan->region_count) continue;
    bool already_emitted = false;
    for (iree_host_size_t j = 0; j < emitted_window_size_count; ++j) {
      already_emitted =
          already_emitted || emitted_window_sizes[j] == window_size;
    }
    if (already_emitted) continue;
    emitted_window_sizes[emitted_window_size_count++] = window_size;

    id4_pipeline_parameter_window_statistics_t statistics;
    IREE_RETURN_IF_ERROR(id4_pipeline_plan_parameter_window_statistics(
        plan, window_size, &statistics));
    if (emitted_window_size_count != 1) {
      IREE_RETURN_IF_ERROR(iree_string_builder_append_cstring(builder, ","));
    }
    IREE_RETURN_IF_ERROR(iree_string_builder_append_format(
        builder,
        "{\"region_window_size\":%" PRIhsz ",\"window_count\":%" PRIhsz
        ",\"full_slab_target_byte_length\":%" PRIu64
        ",\"peak_window_target_byte_length\":%" PRIu64
        ",\"peak_window_source_byte_length\":%" PRIu64
        ",\"total_window_target_byte_length\":%" PRIu64
        ",\"total_window_source_byte_length\":%" PRIu64
        ",\"peak_window_load_group_count\":%" PRIhsz
        ",\"total_window_load_group_count\":%" PRIhsz
        ",\"peak_window_encode_load_step_count\":%" PRIhsz
        ",\"total_window_encode_load_step_count\":%" PRIhsz
        ",\"largest_load_group_target_byte_length\":%" PRIu64
        ",\"largest_load_group_index\":",
        statistics.region_window_size, statistics.window_count,
        (uint64_t)statistics.full_slab_target_byte_length,
        (uint64_t)statistics.peak_window_target_byte_length,
        (uint64_t)statistics.peak_window_source_byte_length,
        (uint64_t)statistics.total_window_target_byte_length,
        (uint64_t)statistics.total_window_source_byte_length,
        statistics.peak_window_load_group_count,
        statistics.total_window_load_group_count,
        statistics.peak_window_encode_load_step_count,
        statistics.total_window_encode_load_step_count,
        (uint64_t)statistics.largest_load_group_target_byte_length));
    if (statistics.largest_load_group_index == IREE_HOST_SIZE_MAX) {
...
iree_status_t id4_pipeline_plan_format_json(const id4_pipeline_plan_t* plan,
  ...
  IREE_RETURN_IF_ERROR(iree_string_builder_append_cstring(builder, "}"));
  IREE_RETURN_IF_ERROR(iree_string_builder_append_cstring(
      builder, ",\"parameter_window_statistics\":"));
  IREE_RETURN_IF_ERROR(
      id4_pipeline_plan_append_parameter_window_statistics_json(plan, builder));
  IREE_RETURN_IF_ERROR(
  ...
  IREE_RETURN_IF_ERROR(
      iree_string_builder_append_cstring(builder, "],\"parameter_tensors\":["));
  for (iree_host_size_t i = 0; i < plan->parameter_tensor_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(iree_string_builder_append_format(
        ...
        ",\"request_offset\":%" PRIhsz ",\"request_count\":%" PRIhsz
        ",\"global_request_offset\":%" PRIhsz ",\"offset\":%" PRIu64
        ",\"byte_length\":%" PRIu64 "}",
        tensor->program_tensor_ordinal, tensor->parameter_slab_index,
        tensor->request_offset, tensor->request_count,
        tensor->global_request_offset, (uint64_t)tensor->offset,

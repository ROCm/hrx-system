...
TEST(ParameterSlabIntegration, DeriveResidentSetReplacesOnlySelectedDomain) {
  RuntimeContext& context = SharedRuntimeContext();
  iree_hal_device_t* device =
      id4_tooling_runtime_context_primary_device(&context.value);
  id4_pipeline_diagnostics_sink_t diagnostics_sink;
  id4_pipeline_diagnostics_sink_initialize_ignore(&diagnostics_sink);
  constexpr iree_device_size_t kSlabByteLength = 16;
  constexpr iree_hal_buffer_usage_t kSlabUsage =
      IREE_HAL_BUFFER_USAGE_DISPATCH_STORAGE |
      IREE_HAL_BUFFER_USAGE_TRANSFER_SOURCE |
      IREE_HAL_BUFFER_USAGE_TRANSFER_TARGET;

  id4_pipeline_parameter_slab_plan_t base_plans[] = {
      id4_pipeline_make_device_local_parameter_slab_plan(
          /*placement_id=*/0, /*binding_slot=*/3, IREE_HAL_QUEUE_AFFINITY_ANY,
          kSlabUsage, kSlabByteLength,
          /*alignment=*/16),
      id4_pipeline_make_device_local_parameter_slab_plan(
          /*placement_id=*/0, /*binding_slot=*/4, IREE_HAL_QUEUE_AFFINITY_ANY,
          kSlabUsage, kSlabByteLength,
          /*alignment=*/16),
  };
  base_plans[1].domain = IREE_SV("lora_patchable");
  const id4_pipeline_parameter_request_table_t empty_request_table =
      id4_pipeline_make_parameter_request_table(/*count=*/0,
                                                /*values=*/nullptr);
  id4_pipeline_parameter_slab_load_t base_loads[] = {
      {
          // Plan-local base slab index.
          .slab_index = 0,
          // Shared base slab allocation plan.
          .slab = &base_plans[0],
          // Empty request metadata retained by the resident set.
          .request_table = &empty_request_table,
          // Primary device index in the runtime device group.
          .device_index = 0,
          // Primary runtime device receiving the slab allocation.
          .device = device,
          // Slab is available to all queues on the primary device.
          .queue_affinity = IREE_HAL_QUEUE_AFFINITY_ANY,
      },
      {
          // Plan-local patchable slab index.
          .slab_index = 1,
          // LoRA-patchable base slab allocation plan.
          .slab = &base_plans[1],
          // Empty request metadata retained by the resident set.
          .request_table = &empty_request_table,
          // Primary device index in the runtime device group.
  ...
  OwningRef<id4_pipeline_parameter_slab_set_t,
      ...
      base_slab_set;
  IREE_ASSERT_OK(id4_pipeline_parameter_slab_set_create_uninitialized(
      IREE_ARRAYSIZE(base_loads), base_loads, IREE_SV("lora.base"),
      &diagnostics_sink, iree_allocator_system(), base_slab_set.out()));

  id4_pipeline_parameter_slab_plan_t replacement_plan =
      id4_pipeline_make_device_local_parameter_slab_plan(
          /*placement_id=*/0, /*binding_slot=*/99, IREE_HAL_QUEUE_AFFINITY_ANY,
          kSlabUsage, kSlabByteLength,
          /*alignment=*/16);
  replacement_plan.domain = IREE_SV("lora_patchable");
  id4_pipeline_parameter_slab_load_t replacement_load = {
      // Source set-local slab index.
      .slab_index = 0,
      // Independently prepared replacement slab allocation plan.
      .slab = &replacement_plan,
      // Empty request metadata retained by the resident set.
      .request_table = &empty_request_table,
      // Primary device index in the runtime device group.
      .device_index = 0,
  ...
  OwningRef<id4_pipeline_parameter_slab_set_t,
            id4_pipeline_parameter_slab_set_release>
      replacement_slab_set;
  IREE_ASSERT_OK(id4_pipeline_parameter_slab_set_create_uninitialized(
      /*load_count=*/1, &replacement_load, IREE_SV("lora.replacement"),
      &diagnostics_sink, iree_allocator_system(), replacement_slab_set.out()));

  const uint8_t shared_value[kSlabByteLength] = {
      0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11,
      0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11,
  };
  const uint8_t replacement_value[kSlabByteLength] = {
      0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33,
      0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33,
  };
  IREE_ASSERT_OK(iree_hal_device_transfer_h2d(
      device, shared_value,
      id4_pipeline_parameter_slab_set_buffer_at(base_slab_set.get(), 0),
      /*target_offset=*/0, sizeof(shared_value),
      IREE_HAL_TRANSFER_BUFFER_FLAG_DEFAULT, iree_infinite_timeout()));
  IREE_ASSERT_OK(iree_hal_device_transfer_h2d(
      device, replacement_value,
      id4_pipeline_parameter_slab_set_buffer_at(replacement_slab_set.get(), 0),
      /*target_offset=*/0, sizeof(replacement_value),
      IREE_HAL_TRANSFER_BUFFER_FLAG_DEFAULT, iree_infinite_timeout()));

  const id4_pipeline_parameter_slab_replacement_t replacement = {
      // Replaces the patchable slab in the base set.
      .target_slab_index = 1,
      // Guards the semantic slab selected by the recipe.
      .expected_domain = IREE_SV("lora_patchable"),
      // Owns the independently prepared replacement slab.
      .source_slab_set = replacement_slab_set.get(),
      // Supplies the only slab in the replacement source set.
      .source_slab_index = 0,
  };
  OwningRef<id4_pipeline_parameter_slab_set_t,
            id4_pipeline_parameter_slab_set_release>
      derived_slab_set;
  IREE_ASSERT_OK(id4_pipeline_parameter_slab_set_derive(
      base_slab_set.get(), /*replacement_count=*/1, &replacement,
      iree_allocator_system(), derived_slab_set.out()));

  EXPECT_EQ(
      id4_pipeline_parameter_slab_set_buffer_at(derived_slab_set.get(), 0),
      id4_pipeline_parameter_slab_set_buffer_at(base_slab_set.get(), 0));
  EXPECT_EQ(
      id4_pipeline_parameter_slab_set_buffer_at(derived_slab_set.get(), 1),
      id4_pipeline_parameter_slab_set_buffer_at(replacement_slab_set.get(), 0));
  EXPECT_EQ(id4_pipeline_parameter_slab_set_plan_at(derived_slab_set.get(), 1)
                ->binding_slot,
            base_plans[1].binding_slot);
  EXPECT_TRUE(iree_string_view_equal(
      id4_pipeline_parameter_slab_set_plan_at(derived_slab_set.get(), 1)
          ->domain,
      IREE_SV("lora_patchable")));
  EXPECT_FALSE(id4_pipeline_parameter_slab_set_has_deferred_load_context(
      derived_slab_set.get()));
  EXPECT_EQ(
      id4_pipeline_parameter_slab_set_load_group_count(derived_slab_set.get()),
      0u);

  id4_pipeline_parameter_slab_replacement_t stale_replacement = replacement;
  stale_replacement.expected_domain = IREE_SV("stale_domain");
  OwningRef<id4_pipeline_parameter_slab_set_t,
            id4_pipeline_parameter_slab_set_release>
      rejected_slab_set;
  IREE_EXPECT_STATUS_IS(
      IREE_STATUS_INVALID_ARGUMENT,
      id4_pipeline_parameter_slab_set_derive(
          base_slab_set.get(), /*replacement_count=*/1, &stale_replacement,
          iree_allocator_system(), rejected_slab_set.out()));
  const id4_pipeline_parameter_slab_replacement_t duplicate_replacements[] = {
      replacement,
      replacement,
  };
  IREE_EXPECT_STATUS_IS(
      IREE_STATUS_INVALID_ARGUMENT,
      id4_pipeline_parameter_slab_set_derive(
          base_slab_set.get(), IREE_ARRAYSIZE(duplicate_replacements),
          duplicate_replacements, iree_allocator_system(),
          rejected_slab_set.out()));

  base_slab_set.reset();
  replacement_slab_set.reset();
  uint8_t actual_shared_value[kSlabByteLength] = {};
  uint8_t actual_replacement_value[kSlabByteLength] = {};
  IREE_ASSERT_OK(iree_hal_device_transfer_d2h(
      device,
      id4_pipeline_parameter_slab_set_buffer_at(derived_slab_set.get(), 0),
      /*source_offset=*/0, actual_shared_value, sizeof(actual_shared_value),
      IREE_HAL_TRANSFER_BUFFER_FLAG_DEFAULT, iree_infinite_timeout()));
  IREE_ASSERT_OK(iree_hal_device_transfer_d2h(
      device,
      id4_pipeline_parameter_slab_set_buffer_at(derived_slab_set.get(), 1),
      /*source_offset=*/0, actual_replacement_value,
      sizeof(actual_replacement_value), IREE_HAL_TRANSFER_BUFFER_FLAG_DEFAULT,
      iree_infinite_timeout()));
  EXPECT_EQ(std::memcmp(actual_shared_value, shared_value,
                        sizeof(actual_shared_value)),
            0);
  EXPECT_EQ(std::memcmp(actual_replacement_value, replacement_value,
                        sizeof(actual_replacement_value)),
            0);
}

...
static iree_status_t id4_pipeline_parameter_slab_set_validate_resident_source(
  ...
  if (!slab_set) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "%.*s parameter slab set is required",
                            (int)source_name.size, source_name.data);
  }
  if (id4_pipeline_parameter_slab_set_has_deferred_load_context(slab_set)) {
    return iree_make_status(
        IREE_STATUS_FAILED_PRECONDITION,
        "%.*s parameter slab set retains a deferred loading schedule",
        (int)source_name.size, source_name.data);
  }
  if (!id4_pipeline_parameter_slab_set_has_resident_buffers(slab_set)) {
    return iree_make_status(
        IREE_STATUS_FAILED_PRECONDITION,
        "%.*s parameter slab set has no complete resident buffer set",
        (int)source_name.size, source_name.data);
  }
  if (slab_set->load_count != slab_set->count || !slab_set->loads ||
      !slab_set->slab_plans || !slab_set->request_tables) {
    return iree_make_status(
        IREE_STATUS_FAILED_PRECONDITION,
        "%.*s parameter slab set has incomplete retained plan metadata",
        (int)source_name.size, source_name.data);
  }
  return iree_ok_status();
}

static iree_status_t id4_pipeline_parameter_slab_replacement_validate(
    const id4_pipeline_parameter_slab_set_t* base_slab_set,
    const id4_pipeline_parameter_slab_replacement_t* replacement,
    iree_host_size_t replacement_index) {
  if (replacement->target_slab_index >= base_slab_set->count) {
    return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                            "parameter slab replacement %" PRIhsz
                            " target index %" PRIhsz
                            " is outside base slab count %" PRIhsz,
                            replacement_index, replacement->target_slab_index,
                            base_slab_set->count);
  }
  IREE_RETURN_IF_ERROR(id4_pipeline_parameter_slab_set_validate_resident_source(
      replacement->source_slab_set, IREE_SV("replacement source")));
  if (replacement->source_slab_index >= replacement->source_slab_set->count) {
    return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                            "parameter slab replacement %" PRIhsz
                            " source index %" PRIhsz
                            " is outside source slab count %" PRIhsz,
                            replacement_index, replacement->source_slab_index,
                            replacement->source_slab_set->count);
  }

  const id4_pipeline_parameter_slab_plan_t* target_plan =
      &base_slab_set->slab_plans[replacement->target_slab_index];
  const id4_pipeline_parameter_slab_plan_t* source_plan =
      &replacement->source_slab_set->slab_plans[replacement->source_slab_index];
  if (!iree_string_view_equal(target_plan->domain,
                              replacement->expected_domain)) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "parameter slab replacement %" PRIhsz
        " expected domain '%.*s' but target slab has domain '%.*s'",
        replacement_index, (int)replacement->expected_domain.size,
        replacement->expected_domain.data, (int)target_plan->domain.size,
        target_plan->domain.data);
  }
  if (!iree_string_view_equal(source_plan->domain,
                              replacement->expected_domain)) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "parameter slab replacement %" PRIhsz
        " expected domain '%.*s' but source slab has domain '%.*s'",
        replacement_index, (int)replacement->expected_domain.size,
        replacement->expected_domain.data, (int)source_plan->domain.size,
        source_plan->domain.data);
  }

  iree_hal_buffer_t* source_buffer =
      replacement->source_slab_set->buffers[replacement->source_slab_index];
  const iree_device_size_t source_byte_length =
      iree_hal_buffer_byte_length(source_buffer);
  if (source_plan->byte_length != source_byte_length ||
      target_plan->byte_length != source_byte_length) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "parameter slab replacement %" PRIhsz
                            " byte length mismatch: target=%" PRIu64
                            ", source plan=%" PRIu64 ", source buffer=%" PRIu64,
                            replacement_index, target_plan->byte_length,
                            source_plan->byte_length, source_byte_length);
  }
  if (target_plan->alignment != 0 &&
      (!iree_device_size_is_power_of_two(target_plan->alignment) ||
       source_plan->alignment < target_plan->alignment ||
       iree_hal_buffer_byte_offset(source_buffer) % target_plan->alignment !=
           0)) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "parameter slab replacement %" PRIhsz
                            " does not satisfy target alignment %" PRIu64,
                            replacement_index, target_plan->alignment);
  }

  const id4_pipeline_parameter_slab_load_t* target_load =
      &base_slab_set->loads[replacement->target_slab_index];
  const iree_hal_buffer_placement_t source_placement =
      iree_hal_buffer_allocation_placement(source_buffer);
  if (source_placement.device != target_load->device) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "parameter slab replacement %" PRIhsz
        " source buffer is not allocated on the target device",
        replacement_index);
  }
  const iree_hal_queue_affinity_t target_queue_affinity =
      target_plan->target_params.queue_affinity;
  const bool source_covers_target_queues =
      iree_hal_queue_affinity_is_any(target_queue_affinity)
          ? iree_hal_queue_affinity_is_any(
                source_plan->target_params.queue_affinity)
          : iree_all_bits_set(source_placement.queue_affinity,
                              target_queue_affinity);
  if (!source_covers_target_queues) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "parameter slab replacement %" PRIhsz
                            " source buffer queue affinity 0x%" PRIx64
                            " does not cover target affinity 0x%" PRIx64,
                            replacement_index, source_placement.queue_affinity,
                            target_queue_affinity);
  }
  IREE_RETURN_IF_ERROR(iree_status_annotate_f(
      iree_hal_buffer_validate_memory_type(
          iree_hal_buffer_memory_type(source_buffer),
          target_plan->target_params.type),
      "parameter slab replacement %" PRIhsz, replacement_index));
  IREE_RETURN_IF_ERROR(iree_status_annotate_f(
      iree_hal_buffer_validate_access(
          iree_hal_buffer_allowed_access(source_buffer),
          target_plan->target_params.access),
      "parameter slab replacement %" PRIhsz, replacement_index));
  return iree_status_annotate_f(
      iree_hal_buffer_validate_usage(
          iree_hal_buffer_allowed_usage(source_buffer),
          target_plan->target_params.usage),
      "parameter slab replacement %" PRIhsz, replacement_index);
}
...
iree_status_t id4_pipeline_parameter_slab_set_derive(
  ...
  *out_slab_set = NULL;
  IREE_RETURN_IF_ERROR(id4_pipeline_parameter_slab_set_validate_resident_source(
      base_slab_set, IREE_SV("base")));
  if (replacement_count == 0) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "at least one parameter slab replacement is "
                            "required");
  }
  if (!replacements) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "parameter slab replacement array is required");
  }
  for (iree_host_size_t i = 0; i < replacement_count; ++i) {
    IREE_RETURN_IF_ERROR(id4_pipeline_parameter_slab_replacement_validate(
        base_slab_set, &replacements[i], i));
    for (iree_host_size_t j = 0; j < i; ++j) {
      if (replacements[i].target_slab_index ==
          replacements[j].target_slab_index) {
        return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                                "parameter slab replacements %" PRIhsz
                                " and %" PRIhsz
                                " both replace target slab %" PRIhsz,
                                j, i, replacements[i].target_slab_index);
      }
    }
  }
  id4_pipeline_parameter_slab_set_t* slab_set = NULL;
  ...
  if (iree_status_is_ok(status)) {
    status = id4_pipeline_parameter_slab_set_copy_loads(
  ...
  if (iree_status_is_ok(status)) {
    ...
    for (iree_host_size_t i = 0; i < replacement_count; ++i) {
      ...
      iree_hal_buffer_release(
          slab_set->buffers[replacement->target_slab_index]);
      slab_set->buffers[replacement->target_slab_index] =
          replacement->source_slab_set->buffers[replacement->source_slab_index];
      iree_hal_buffer_retain(slab_set->buffers[replacement->target_slab_index]);
    }
    *out_slab_set = slab_set;
  } else {
    id4_pipeline_parameter_slab_set_release(slab_set);

...
enum {
  ...
  ID4_QWEN3_VL_LINEAR_WMMA_TOKEN_BLOCK_M96 = 96,
...
typedef enum id4_qwen3_vl_kernel_kind_e {
  ...
  ID4_QWEN3_VL_KERNEL_LINEAR_BF16_BF16_WMMA_M48N32 = 17,
  // Dense BF16 activation/weight WMMA linear kernel for 96x32 tiles.
  ID4_QWEN3_VL_KERNEL_LINEAR_BF16_BF16_WMMA_M96N32 = 18,
  // Dense BF16 activation/weight scalar linear tail kernel.
  ID4_QWEN3_VL_KERNEL_LINEAR_BF16_BF16_TAIL = 19,
  // BF16 activation transpose pack kernel.
  ID4_QWEN3_VL_KERNEL_LINEAR_INPUT_PACK_TRANSPOSE_BF16_BF16 = 20,
  // Materialized QK score WMMA kernel.
  ID4_QWEN3_VL_KERNEL_ATTENTION_QK_SCORES_WMMA = 21,
  // Masked BF16 softmax kernel for WMMA attention.
  ID4_QWEN3_VL_KERNEL_ATTENTION_SOFTMAX_MASK_BF16 = 22,
  // Materialized PV WMMA kernel.
  ID4_QWEN3_VL_KERNEL_ATTENTION_PV_WMMA = 23,
  // Fused MLP gate/up/SwiGLU WMMA kernel for 16-token tiles.
  ID4_QWEN3_VL_KERNEL_MLP_GATE_UP_SILU_PRODUCT_BF16_WMMA_M16N32 = 24,
  // Fused MLP gate/up/SwiGLU WMMA kernel for 32-token tiles.
  ID4_QWEN3_VL_KERNEL_MLP_GATE_UP_SILU_PRODUCT_BF16_WMMA_M32N32 = 25,
  // BF16 tensor tail zeroing kernel.
  ID4_QWEN3_VL_KERNEL_ZERO_TAIL_BF16 = 26,
...
enum {
  ...
  ID4_QWEN3_VL_KERNEL_KIND_COUNT = ID4_QWEN3_VL_KERNEL_ZERO_TAIL_BF16 + 1,
...
static const id4_qwen3_vl_program_linear_wmma_tile_t
    id4_qwen3_vl_program_linear_wmma_m96n32_tile = {
        // Number of token rows covered by one WMMA dispatch tile.
        .token_block = ID4_QWEN3_VL_LINEAR_WMMA_TOKEN_BLOCK_M96,
        // Number of output rows covered by one WMMA dispatch tile.
        .output_row_block = ID4_QWEN3_VL_LINEAR_WMMA_OUTPUT_ROW_BLOCK_M32,
        // Kernel kind implementing this tile shape.
        .kernel_kind = ID4_QWEN3_VL_KERNEL_LINEAR_BF16_BF16_WMMA_M96N32,
};
...
id4_qwen3_vl_program_select_linear_wmma_tile(uint32_t token_count,
  ...
  if ((token_count %
       id4_qwen3_vl_program_linear_wmma_m96n32_tile.token_block) == 0 &&
      token_count < ID4_QWEN3_VL_LINEAR_WMMA_M64N64_MIN_TOKEN_COUNT &&
      (output_size %
       id4_qwen3_vl_program_linear_wmma_m96n32_tile.output_row_block) == 0 &&
      output_size >= ID4_QWEN3_VL_LINEAR_WMMA_M64N64_MIN_OUTPUT_SIZE) {
    return &id4_qwen3_vl_program_linear_wmma_m96n32_tile;
  }
  if ((token_count %
...
static const id4_pipeline_kernel_ref_t
    id4_qwen3_vl_program_kernel_refs[ID4_QWEN3_VL_KERNEL_KIND_COUNT] = {
        ...
        [ID4_QWEN3_VL_KERNEL_LINEAR_BF16_BF16_WMMA_M96N32] =
            {IREE_SVL("qwen3_vl/linear_bf16_f32_wmma"),
             IREE_SVL("id4_qwen3_vl_linear_bf16_bf16_wmma_m96n32")},
...
static iree_status_t id4_qwen3_vl_program_author_token_embedding(
  ...
  IREE_RETURN_IF_ERROR(id4_qwen3_vl_program_bf16_coverage(
      "token embedding output", token_count, hidden_size, token_capacity,
      hidden_size, &output_coverage));
  ...
  id4_pipeline_program_dispatch_binding_t bindings[] = {
      ...
      id4_qwen3_vl_program_bf16_logical_write(*out_hidden_states,
                                              &output_coverage),
  };
  ...
  IREE_RETURN_IF_ERROR(id4_qwen3_vl_program_zero_bf16_tail(
      builder, ID4_QWEN3_VL_TENSOR_EMBEDDED_HIDDEN_STATES, UINT32_MAX,
      *out_hidden_states, &output_coverage));
...
static iree_status_t id4_qwen3_vl_program_author_linear(
  ...
  if (wmma_token_count != 0) {
    ...
    IREE_RETURN_IF_ERROR(id4_qwen3_vl_program_make_config_bindings(
        wmma_tile->kernel_kind, IREE_ARRAYSIZE(wmma_config_values),
    ...
    IREE_RETURN_IF_ERROR(id4_qwen3_vl_program_dispatch_named(
        builder, wmma_name, wmma_tile->kernel_kind,
...
static iree_status_t id4_qwen3_vl_program_author_rotary(
  ...
  id4_qwen3_vl_program_bf16_coverage_t output_coverage;
  IREE_RETURN_IF_ERROR(id4_qwen3_vl_program_bf16_coverage(
      "rotary output", token_count, channel_count, token_capacity,
      channel_count, &output_coverage));
  const id4_qwen3_vl_program_config_value_t config_values[] = {
      {ID4_QWEN3_VL_CONFIG_TOKEN_COUNT, token_count},
...
static iree_status_t id4_qwen3_vl_program_author_silu_gate(
  ...
  IREE_RETURN_IF_ERROR(id4_qwen3_vl_program_acquire_tensor(
      ...
      id4_pipeline_program_make_shape_rank2(token_capacity, intermediate_size),
      out_activation));
  id4_qwen3_vl_program_bf16_coverage_t activation_coverage;
  IREE_RETURN_IF_ERROR(id4_qwen3_vl_program_bf16_coverage(
      "SiLU gate activation", token_count, intermediate_size, token_capacity,
      intermediate_size, &activation_coverage));
  const id4_qwen3_vl_program_config_value_t config_values[] = {
      {ID4_QWEN3_VL_CONFIG_TOKEN_COUNT, token_count},
      {ID4_QWEN3_VL_CONFIG_INTERMEDIATE_SIZE, intermediate_size},
  };
  ...
  id4_pipeline_program_dispatch_binding_t bindings[] = {
      ...
      id4_qwen3_vl_program_bf16_logical_write(*out_activation,
                                              &activation_coverage),
  };
  IREE_RETURN_IF_ERROR(id4_qwen3_vl_program_dispatch(
      builder, ID4_QWEN3_VL_OPERATION_SILU_GATE, layer_ordinal,
  ...
  return id4_qwen3_vl_program_zero_bf16_tail(
      builder, ID4_QWEN3_VL_TENSOR_LAYER_MLP_ACTIVATION, layer_ordinal,
      *out_activation, &activation_coverage);
}
...
static iree_status_t id4_qwen3_vl_program_author_residual_add(
  ...
  id4_qwen3_vl_program_bf16_coverage_t output_coverage;
  IREE_RETURN_IF_ERROR(id4_qwen3_vl_program_bf16_coverage(
      "residual add output", token_count, hidden_size, token_capacity,
      hidden_size, &output_coverage));
  const id4_qwen3_vl_program_config_value_t config_values[] = {
      {ID4_QWEN3_VL_CONFIG_TOKEN_COUNT, token_count},
      {ID4_QWEN3_VL_CONFIG_HIDDEN_SIZE, hidden_size},

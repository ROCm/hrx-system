...
|--------|---------|-------------|
| `HRX_IREE_SOURCE_DIR` | `third_party/iree-runtime` | Path to IREE source tree |
...
HRX vendors a pruned IREE runtime snapshot under `third_party/iree-runtime`.
The import is generated by script and is tracked as a squashed snapshot, not as
a git submodule. HRX-generated import metadata is written beside the import as
`third_party/iree-runtime.HRX_VENDOR.json`, leaving the imported IREE tree free
of generated HRX files.

Create the pristine vendored runtime import from an explicit IREE commit:
...
Apply the local patch queue as one commit per patch:

```bash
python scripts/vendor_iree_runtime.py apply-patches
```

After editing `third_party/iree-runtime`, refresh the patch queue from the
commits after the pristine import:
...
Check that the committed snapshot still matches the importer:
...
Local IREE patches go in `scripts/iree-runtime-patches/*.patch` and are applied
after the pristine import. `third_party/hsa-runtime-headers` is intentionally
not vendored: HRX is built in the ROCm universe and will provide HSA
headers/libraries through `find_package(hsa-runtime64)`.

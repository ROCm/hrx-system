...
RDMA core headers follow the same facade contract through
`//runtime/config/net:transport_rdma`. The default network transport set does
not enable RDMA, so wildcard Bazel and CMake builds do not fetch rdma-core
headers. Explicit RDMA transport builds resolve `//third_party:rdma_core_headers`
to the pinned `@rdma_core_headers` repository in Bazel or to the
`iree::third_party::rdma_core_headers` adapter target in CMake.
...
RDMA headers:     //third_party:rdma_core_headers

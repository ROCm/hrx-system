...
def device_binary_targets():
    ...
    for exact_target, code_object_target in EXACT_TARGET_CODE_OBJECTS:
        ...
        if code_object_target not in names:
            values.append(
                DeviceBinaryTarget(
                    name=code_object_target,
                    architecture=code_object_target,
                )
            )
            names.add(code_object_target)
    return values
...
def exact_targets_for_code_object(code_object_target):
    return [
        exact_target
        for exact_target, mapped_target in EXACT_TARGET_CODE_OBJECTS
        if mapped_target == code_object_target
    ]
...
def expand_device_binary_target_selections(selections):
    ...
    exact_to_code_object = dict(EXACT_TARGET_CODE_OBJECTS)

...
typedef enum iree_hal_hsa_native_flatbuffer_schema_e {
  IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_NONE = 0,
  IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_HIP = 1,
  IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_AMDGPU = 2,
} iree_hal_hsa_native_flatbuffer_schema_t;
...
static iree_status_t iree_hal_hsa_read_native_flatbuffer_header(
    ...
    iree_hal_hsa_native_flatbuffer_schema_t* out_schema) {
  IREE_ASSERT_ARGUMENT(out_flatbuffer_data);
  IREE_ASSERT_ARGUMENT(out_schema);
  *out_flatbuffer_data = iree_const_byte_span_empty();
  *out_schema = IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_NONE;

  iree_status_t hip_status = iree_hal_read_executable_flatbuffer_header(
      executable_data, unsafe_infer_size,
      iree_hal_hip_ExecutableDef_file_identifier, out_flatbuffer_data);
  if (iree_status_is_ok(hip_status)) {
    *out_schema = IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_HIP;
    return iree_ok_status();
  }
  iree_status_ignore(hip_status);

  IREE_RETURN_IF_ERROR(iree_hal_read_executable_flatbuffer_header(
      executable_data, unsafe_infer_size,
      iree_hal_amdgpu_ExecutableDef_file_identifier, out_flatbuffer_data));
  *out_schema = IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_AMDGPU;
  return iree_ok_status();
}
...
iree_status_t iree_hal_hsa_native_executable_infer_format(
  ...
  iree_hal_hsa_native_flatbuffer_schema_t flatbuffer_schema =
      IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_NONE;
  IREE_RETURN_IF_ERROR(iree_hal_hsa_read_native_flatbuffer_header(
      executable_data, unsafe_infer_size, &contained_data,
      &flatbuffer_schema));

  // Verify the flatbuffer structure.
  int verify_ret = flatcc_verify_error_not_supported;
  if (flatbuffer_schema == IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_HIP) {
    verify_ret = iree_hal_hip_ExecutableDef_verify_as_root(
        contained_data.data, contained_data.data_length);
  } else if (flatbuffer_schema ==
             IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_AMDGPU) {
    verify_ret = iree_hal_amdgpu_ExecutableDef_verify_as_root(
        contained_data.data, contained_data.data_length);
  }
...
static iree_status_t iree_hal_hsa_native_executable_create_flatbuffer(
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_hsa_read_native_flatbuffer_header(
              ...
              &executable_flatbuffer, &flatbuffer_schema));
  ...
  iree_hal_amdgpu_ExecutableDef_table_t amdgpu_executable_def = NULL;
  iree_hal_amdgpu_ModuleDef_vec_t amdgpu_modules_vec = NULL;
  iree_hal_amdgpu_ExportDef_vec_t amdgpu_exports_vec = NULL;
  ...
  if (flatbuffer_schema == IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_HIP) {
    ...
    module_count = iree_hal_hip_ModuleDef_vec_len(hip_modules_vec);
    hip_exports_vec = iree_hal_hip_ExecutableDef_exports_get(hip_executable_def);
    export_count = iree_hal_hip_ExportDef_vec_len(hip_exports_vec);
  } else {
    amdgpu_executable_def =
        iree_hal_amdgpu_ExecutableDef_as_root(executable_flatbuffer.data);
    amdgpu_modules_vec =
        iree_hal_amdgpu_ExecutableDef_modules_get(amdgpu_executable_def);
    module_count = iree_hal_amdgpu_ModuleDef_vec_len(amdgpu_modules_vec);
    amdgpu_exports_vec =
        iree_hal_amdgpu_ExecutableDef_exports_get(amdgpu_executable_def);
    export_count = iree_hal_amdgpu_ExportDef_vec_len(amdgpu_exports_vec);
  }
  ...
  IREE_TRACE({
    for (iree_host_size_t i = 0; i < export_count; ++i) {
      iree_hal_debug_ExportDef_table_t export_debug_info = NULL;
      if (flatbuffer_schema == IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_HIP) {
        iree_hal_hip_ExportDef_table_t export_def =
            iree_hal_hip_ExportDef_vec_at(hip_exports_vec, i);
        export_debug_info = iree_hal_hip_ExportDef_debug_info_get(export_def);
      } else {
        iree_hal_amdgpu_ExportDef_table_t export_def =
            iree_hal_amdgpu_ExportDef_vec_at(amdgpu_exports_vec, i);
        export_debug_info =
  ...
  if (flatbuffer_schema == IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_HIP) {
    ...
    iree_hal_debug_publish_source_files(
        iree_hal_amdgpu_ExecutableDef_source_files_get(amdgpu_executable_def));
  }
  ...
  if (module_count > 0) {
    ...
    if (iree_status_is_ok(status)) {
      ...
      for (iree_host_size_t i = 0; i < module_count && iree_status_is_ok(status);
           ++i) {
        flatbuffers_string_t hsaco_image = NULL;
        if (flatbuffer_schema == IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_HIP) {
          iree_hal_hip_ModuleDef_table_t module_def =
              iree_hal_hip_ModuleDef_vec_at(hip_modules_vec, i);
          hsaco_image = iree_hal_hip_ModuleDef_hsaco_image_get(module_def);
        } else {
          iree_hal_amdgpu_ModuleDef_table_t module_def =
              iree_hal_amdgpu_ModuleDef_vec_at(amdgpu_modules_vec, i);
          hsaco_image = iree_hal_amdgpu_ModuleDef_image_get(module_def);
        }
  ...
  for (iree_host_size_t j = 0; j < topology.count && iree_status_is_ok(status);
    ...
    for (iree_host_size_t i = 0; i < export_count && iree_status_is_ok(status); ++i) {
      iree_hal_hip_ExportDef_table_t hip_export_def = NULL;
      iree_hal_amdgpu_ExportDef_table_t amdgpu_export_def = NULL;
      uint32_t module_ordinal = 0;
      flatbuffers_string_t kernel_name = NULL;
      if (flatbuffer_schema == IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_HIP) {
        hip_export_def = iree_hal_hip_ExportDef_vec_at(hip_exports_vec, i);
        module_ordinal = iree_hal_hip_ExportDef_module_ordinal_get(hip_export_def);
        kernel_name = iree_hal_hip_ExportDef_kernel_name_get(hip_export_def);
      } else {
        amdgpu_export_def =
            iree_hal_amdgpu_ExportDef_vec_at(amdgpu_exports_vec, i);
        kernel_name =
            iree_hal_amdgpu_ExportDef_symbol_name_get(amdgpu_export_def);
      }
      ...
      if (flatbuffer_schema == IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_HIP) {
        ...
        for (iree_host_size_t k = 0; k < module_count; ++k) {
          status = IREE_HSA_CALL_TO_STATUS(
              hsa_executable_get_symbol_by_name(
                  per_device_data->executables[k], kernel_name, &agent,
                  &kernel_symbol),
              "hsa_executable_get_symbol_by_name");
          if (iree_status_is_ok(status)) break;
          if (k + 1 < module_count) {
      ...
      if (flatbuffer_schema == IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_HIP) {
        ...
        const iree_hal_amdgpu_Dims_t* workgroup_size =
            iree_hal_amdgpu_ExportDef_workgroup_size_get(amdgpu_export_def);
        if (workgroup_size) {
          kernel_info->block_dims[0] = workgroup_size->x;
          kernel_info->block_dims[1] = workgroup_size->y;
          kernel_info->block_dims[2] = workgroup_size->z;
        } else {
          kernel_info->block_dims[0] = 1;
          kernel_info->block_dims[1] = 1;
          kernel_info->block_dims[2] = 1;
      ...
      kernel_info->constant_count =
          iree_hal_amdgpu_ExportDef_constant_count_get(amdgpu_export_def);
        iree_hal_amdgpu_BindingBits_vec_t binding_flags_vec =
            iree_hal_amdgpu_ExportDef_binding_flags_get(amdgpu_export_def);
        kernel_info->binding_count =
            iree_hal_amdgpu_BindingBits_vec_len(binding_flags_vec);
      }
      ...
      if (module_infos) {
        iree_host_size_t metadata_module_ordinal =
            flatbuffer_schema == IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_HIP
                ? module_ordinal
                : 0;
        if (metadata_module_ordinal < module_count) {
          iree_hal_hsa_apply_parsed_kernel_metadata(
              &module_infos[metadata_module_ordinal], kernel_name,
              host_allocator, kernel_info);
        }
      }
      ...
      IREE_TRACE({
        ...
        iree_hal_debug_ExportDef_table_t export_debug_info =
            flatbuffer_schema == IREE_HAL_HSA_NATIVE_FLATBUFFER_SCHEMA_HIP
                ...
                : iree_hal_amdgpu_ExportDef_debug_info_get(amdgpu_export_def);
        export_info_ptr +=

...
static iree_hal_hsa_allocator_t *
iree_hal_hsa_allocator_cast(iree_hal_allocator_t *base_value) {
...
iree_status_t iree_hal_hsa_allocator_create(
    iree_hal_device_t *parent_device, iree_hal_hsa_device_topology_t topology,
...
iree_hal_hsa_allocator_query_buffer_compatibility(
  ...
  // HSA doesn't have managed memory like HIP/CUDA, so device-local +
  // host-visible allocations need to fall back to host-local + device-visible
...
static void iree_hal_hsa_buffer_free(iree_hal_hsa_buffer_type_t buffer_type,
  ...
  if (iree_all_bits_set(compat_params.type,
    ...
    if (device_info->device_local_memory_pool_valid) {
      ...
      if (iree_status_is_ok(status) && device_info->cpu_agent.handle) {
        ...
        IREE_HSA_IGNORE_ERROR(
            hsa_amd_agents_allow_access(2, agents, NULL, device_ptr));
      }
    } else {
    ...
    if (device_info->host_visible_memory_pool_valid) {
      ...
      if (iree_status_is_ok(status)) {
        ...
        IREE_HSA_IGNORE_ERROR(
            hsa_amd_agents_allow_access(1, agents, NULL, host_ptr));
      }
    } else {
      status = iree_make_status(IREE_STATUS_UNAVAILABLE,
  ...
  if (iree_status_is_ok(status)) {
    ...
    *out_buffer = buffer;
    ...
    if (!buffer && (device_ptr || host_ptr)) {
      iree_hal_hsa_buffer_free(buffer_type, device_ptr, host_ptr);
    } else {

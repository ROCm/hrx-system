...
iree_cuda_stream_flags_to_internal(unsigned int cuda_flags) {
...
iree_cuda_event_flags_to_internal(unsigned int cuda_flags) {
...
iree_cuda_mempool_attr_to_internal(CUmemPool_attribute attr) {
  switch (attr) {
...
iree_cuda_limit_to_internal(CUlimit limit) {
  switch (limit) {
...
CUDAAPI CUresult cuEventSynchronize(CUevent hEvent) {
  IREE_TRACE_ZONE_BEGIN(z0);
  iree_status_t status = iree_hal_streaming_event_synchronize(
...
CUDAAPI CUresult cuLaunchCooperativeKernel(
  ...
  status = iree_hal_streaming_launch_kernel(
      (iree_hal_streaming_symbol_t *)f, &params,
      (iree_hal_streaming_stream_t *)hStream);

...
pyre_status_t pyre_mem_pool_create(pyre_device_t device,
  ...
  pyre_mem_pool_s* pool = (pyre_mem_pool_s*)calloc(1, sizeof(pyre_mem_pool_s));
  if (!pool) {
...
static void pyre_mem_pool_destroy(pyre_mem_pool_s* pool) {
  iree_slim_mutex_deinitialize(&pool->mutex);
  free(pool);
}

void pyre_mem_pool_retain(pyre_mem_pool_t pool) {
  if (pool) {
    iree_atomic_ref_count_inc(&pool->ref_count);
  }
}

void pyre_mem_pool_release(pyre_mem_pool_t pool) {
  if (pool && iree_atomic_ref_count_dec(&pool->ref_count) == 1) {
    pyre_mem_pool_destroy(pool);
  }
}
...
pyre_status_t pyre_mem_pool_get_attribute(pyre_mem_pool_t pool,
  ...
  *out_value = 0;
  ...
  switch (attr) {
    case PYRE_MEM_POOL_ATTR_REUSE_FOLLOW_EVENT_DEPENDENCIES:
      *out_value = pool->reuse_follow_event_dependencies ? 1 : 0;
      break;
    case PYRE_MEM_POOL_ATTR_REUSE_ALLOW_INTERNAL_DEPENDENCIES:
      *out_value = pool->reuse_allow_internal_dependencies ? 1 : 0;
      break;
    case PYRE_MEM_POOL_ATTR_REUSE_ALLOW_OPPORTUNISTIC:
      *out_value = pool->reuse_allow_opportunistic ? 1 : 0;
      break;
    case PYRE_MEM_POOL_ATTR_RELEASE_THRESHOLD:
      *out_value = pool->release_threshold;
      break;
    case PYRE_MEM_POOL_ATTR_RESERVED_MEM_CURRENT:
      *out_value = pool->reserved_mem_current;
      break;
    case PYRE_MEM_POOL_ATTR_RESERVED_MEM_HIGH:
      *out_value = pool->reserved_mem_high;
      break;
    case PYRE_MEM_POOL_ATTR_USED_MEM_CURRENT:
      *out_value = pool->used_mem_current;
      break;
    case PYRE_MEM_POOL_ATTR_USED_MEM_HIGH:
      *out_value = pool->used_mem_high;
      break;
    default:
      status = pyre_make_status(PYRE_STATUS_INVALID_ARGUMENT,
...
pyre_status_t pyre_mem_pool_set_attribute(pyre_mem_pool_t pool,
                                          ...
                                          uint64_t value) {
  if (!pool) {
    return pyre_make_status(PYRE_STATUS_INVALID_ARGUMENT, "pool is NULL");
  }

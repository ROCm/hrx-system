...
typedef enum pyre_graph_block_type_e {
  PYRE_GRAPH_BLOCK_TYPE_QUEUE_BARRIER = 0,
  PYRE_GRAPH_BLOCK_TYPE_QUEUE_FILL,
  PYRE_GRAPH_BLOCK_TYPE_QUEUE_COPY,
  PYRE_GRAPH_BLOCK_TYPE_QUEUE_HOST_CALL,
  PYRE_GRAPH_BLOCK_TYPE_QUEUE_DISPATCH,
  PYRE_GRAPH_BLOCK_TYPE_QUEUE_EXECUTE,
} pyre_graph_block_type_t;

typedef void (*pyre_graph_host_callback_fn_t)(void* user_data);

typedef struct pyre_graph_barrier_block_attrs_t {
  iree_hal_execute_flags_t flags;
} pyre_graph_barrier_block_attrs_t;

typedef struct pyre_graph_fill_block_attrs_t {
...
} pyre_graph_fill_block_attrs_t;

typedef struct pyre_graph_copy_block_attrs_t {
...
} pyre_graph_copy_block_attrs_t;

typedef struct pyre_graph_host_call_block_attrs_t {
  pyre_graph_host_callback_fn_t fn;
  void* user_data;
  iree_hal_host_call_flags_t flags;
} pyre_graph_host_call_block_attrs_t;

typedef struct pyre_graph_dispatch_block_attrs_t {
...
} pyre_graph_dispatch_block_attrs_t;

typedef struct pyre_graph_execute_block_attrs_t {
  iree_hal_command_buffer_t* command_buffer;
  iree_hal_execute_flags_t flags;
} pyre_graph_execute_block_attrs_t;

typedef union pyre_graph_block_attrs_t {
  pyre_graph_barrier_block_attrs_t barrier;
  pyre_graph_fill_block_attrs_t fill;
  pyre_graph_copy_block_attrs_t copy;
  pyre_graph_host_call_block_attrs_t host_call;
  pyre_graph_dispatch_block_attrs_t dispatch;
  pyre_graph_execute_block_attrs_t execute;
} pyre_graph_block_attrs_t;

typedef struct pyre_graph_exec_block_t {
  pyre_graph_block_type_t type;
...
} pyre_graph_exec_block_t;

typedef struct pyre_graph_block_ptrs_t {
  ...
  pyre_graph_block_attrs_t* attrs;
} pyre_graph_block_ptrs_t;

//===----------------------------------------------------------------------===//
// Block helpers
//===----------------------------------------------------------------------===//

static inline void pyre_graph_block_get_ptrs(
    pyre_graph_exec_block_t* block, pyre_graph_block_ptrs_t* out_ptrs) {
  ...
  out_ptrs->attrs = (pyre_graph_block_attrs_t*)ptr;
}

static pyre_status_t pyre_graph_block_allocate(
    iree_arena_allocator_t* arena, pyre_graph_block_type_t type,
    uint32_t node_start_index, uint32_t node_count,
    uint16_t wait_semaphore_count, uint16_t signal_semaphore_count,
    pyre_graph_exec_block_t** out_block, pyre_graph_block_ptrs_t* out_ptrs) {
  iree_host_size_t size = sizeof(pyre_graph_exec_block_t);
  ...
  size += sizeof(pyre_graph_block_attrs_t);

  pyre_graph_exec_block_t* block = NULL;
  ...
  pyre_graph_block_get_ptrs(block, out_ptrs);
  *out_block = block;
...
typedef struct pyre_graph_node_index_set_t {
  uint32_t values[8];
  uint32_t count : 31;
  uint32_t invalid : 1;
} pyre_graph_node_index_set_t;

static inline void pyre_graph_node_index_set_reset(
    pyre_graph_node_index_set_t* set) {
  set->count = 0;
  set->invalid = 0;
}

static bool pyre_graph_node_index_set_test_hazard(
    const pyre_graph_node_index_set_t* set, uint32_t value) {
...
static void pyre_graph_node_index_set_insert(
    pyre_graph_node_index_set_t* set, uint32_t value) {
...
static pyre_status_t pyre_graph_record_partition(
    pyre_graph_exec_s* exec, pyre_graph_sort_node_t* sorted_nodes,
    uint32_t node_start_index, uint32_t node_count,
  ...
  pyre_graph_node_index_set_t barrier_index_set;
  pyre_graph_node_index_set_reset(&barrier_index_set);
  for (uint32_t i = 0; iree_status_is_ok(status) && i < node_count; ++i) {
    pyre_graph_sort_node_t* sort_node = &sorted_nodes[node_start_index + i];
    if (sort_node->stream_id != stream_id) continue;
    pyre_graph_node_s* node = sort_node->node;
    if (in_stream_count > 1) {
      for (uint32_t j = 0; j < node->dependency_count; ++j) {
        ...
        const bool has_hazard = pyre_graph_node_index_set_test_hazard(
        ...
        pyre_graph_node_index_set_insert(&barrier_index_set,
                                         sort_node->sorted_index);
      }
    }
    ++in_stream_count;
    switch (node->type) {
      case PYRE_GRAPH_NODE_TYPE_INTERNAL_KERNEL: {
        const pyre_graph_kernel_node_attrs_internal_t* attrs =
            &node->attrs.kernel;
      ...
      case PYRE_GRAPH_NODE_TYPE_INTERNAL_MEMCPY: {
        const pyre_graph_memcpy_node_attrs_internal_t* attrs =
            &node->attrs.memcpy;
        status = iree_hal_command_buffer_copy_buffer(
            command_buffer, attrs->src_ref, attrs->dst_ref, attrs->flags);
        break;
      }
      case PYRE_GRAPH_NODE_TYPE_INTERNAL_MEMSET: {
        const pyre_graph_memset_node_attrs_internal_t* attrs =
            &node->attrs.memset;
...
pyre_status_t pyre_graph_exec_instantiate_locked(
  ...
  pyre_graph_schedule_t schedule;
  pyre_graph_edge_t* additional_edges =
      exec->graph ? exec->graph->additional_edges : NULL;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, pyre_graph_schedule_nodes(node_blocks, node_count, additional_edges,
  ...
  for (iree_host_size_t p = 0; p < schedule.partition_count; p++) {
    const pyre_graph_partition_t* partition = &schedule.partitions[p];

    uint16_t wait_semaphore_count = 0;
    if (p > 0) {
      pyre_graph_partition_t* prev_partition = &schedule.partitions[p - 1];
    ...
    if (partition->type == PYRE_GRAPH_PARTITION_TYPE_RECORDABLE) {
      ...
      for (uint8_t s = 0; s < stream_count; s++) {
        ...
        pyre_graph_block_ptrs_t ptrs;
        IREE_RETURN_AND_END_ZONE_IF_ERROR(
            z0, pyre_graph_block_allocate(
                    &exec->arena_allocator,
                    PYRE_GRAPH_BLOCK_TYPE_QUEUE_EXECUTE,
        ...
        IREE_RETURN_AND_END_ZONE_IF_ERROR(
            z0, pyre_graph_record_partition(
      ...
      pyre_graph_exec_block_t* block = NULL;
      pyre_graph_block_ptrs_t ptrs;
      if (partition->type == PYRE_GRAPH_PARTITION_TYPE_HOST_CALL) {
        ...
        IREE_RETURN_AND_END_ZONE_IF_ERROR(
            z0, pyre_graph_block_allocate(
                    &exec->arena_allocator,
                    PYRE_GRAPH_BLOCK_TYPE_QUEUE_BARRIER,
                    partition->start_index, partition->count,
                    wait_semaphore_count, signal_semaphore_count, &block,
                    &ptrs));
...
static pyre_status_t pyre_graph_host_callback(
    void* user_data, const uint64_t args[4],
    iree_hal_host_call_context_t* context) {
  IREE_TRACE_ZONE_BEGIN(z0);
  pyre_graph_host_callback_fn_t call_fn =
      (pyre_graph_host_callback_fn_t)args[0];
...
pyre_status_t pyre_graph_exec_launch(pyre_graph_exec_t exec,
  ...
  for (uint32_t block_index = 0;
       ...
       block_index++) {
    pyre_graph_exec_block_t* block = exec->blocks[block_index];
    pyre_graph_block_ptrs_t ptrs;
    pyre_graph_block_get_ptrs(block, &ptrs);
    ...
    switch (block->type) {
      ...
      case PYRE_GRAPH_BLOCK_TYPE_QUEUE_FILL:
        ...
        break;
      case PYRE_GRAPH_BLOCK_TYPE_QUEUE_COPY:
        ...
        break;
      case PYRE_GRAPH_BLOCK_TYPE_QUEUE_DISPATCH: {
      ...
      case PYRE_GRAPH_BLOCK_TYPE_QUEUE_EXECUTE:
        status = iree_hal_device_queue_execute(
            ...
            IREE_HAL_EXECUTE_FLAG_NONE);
        break;
      case PYRE_GRAPH_BLOCK_TYPE_QUEUE_HOST_CALL: {
        ...
        status = iree_hal_device_queue_host_call(
            hal_device, IREE_HAL_QUEUE_AFFINITY_ANY, wait_semaphores,
            signal_semaphores,
            iree_hal_make_host_call(pyre_graph_host_callback, NULL),

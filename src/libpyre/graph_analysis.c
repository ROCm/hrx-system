...
#define PYRE_GRAPH_MIN_PARTITION_SIZE_FOR_STREAMS 16
#define PYRE_GRAPH_MAX_PARTITION_SIZE 2048
#define PYRE_GRAPH_MAX_WORKSTREAMS 8
#define PYRE_GRAPH_MIN_NODES_PER_STREAM 32
#define PYRE_GRAPH_MAX_REACHABILITY_BITS 64
#define PYRE_GRAPH_MAX_FAN_OUT 8

//===----------------------------------------------------------------------===//
// Phase 1: Preparation
//===----------------------------------------------------------------------===//

static bool pyre_graph_prepare_nodes(pyre_graph_node_block_t* node_blocks,
                                     iree_host_size_t node_count,
                                     pyre_graph_sort_node_t* sort_nodes,
                                     uint32_t* node_index_map) {
  uint32_t index = 0;
  for (pyre_graph_node_block_t* block = node_blocks; block;
       block = block->next) {
    if (block->next) {
      IREE_PREFETCH_RO(block->next, 1);
    }
    for (iree_host_size_t i = 0; i < block->count; ++i) {
      pyre_graph_node_s* node = block->nodes[i];
      sort_nodes[index] = (pyre_graph_sort_node_t){
  ...
  for (uint32_t i = 0; is_sorted && i < node_count; ++i) {
    pyre_graph_node_s* node = sort_nodes[i].node;
...
static pyre_status_t pyre_graph_topological_sort(
    pyre_graph_sort_node_t* nodes, uint32_t node_count,
    uint32_t* node_index_map, pyre_graph_edge_t* additional_edges,
  ...
  pyre_graph_edge_t* edge = additional_edges;
  while (edge) {
  ...
  while (queue_head < queue_tail) {
    ...
    for (uint32_t i = 0; i < node_count; ++i) {
      ...
      pyre_graph_node_s* node = nodes[i].node;
      for (uint32_t j = 0; j < node->dependency_count; ++j) {
...
typedef struct pyre_graph_stream_state_t {
  uint64_t reachability_mask;
  uint32_t last_node_index;
} pyre_graph_stream_state_t;

typedef struct pyre_uint32x2_t {
  uint32_t values[2];
} pyre_uint32x2_t;

static pyre_uint32x2_t pyre_graph_partition_with_streams(
    pyre_graph_sort_node_t* nodes, uint32_t node_count,
    uint32_t* node_index_map, pyre_graph_partition_t* partitions) {
  uint32_t partition_count = 0;
  uint32_t block_count = 0;

  pyre_graph_stream_state_t streams[PYRE_GRAPH_MAX_WORKSTREAMS];
  uint8_t active_streams = 0;

  for (uint32_t i = 0; i < node_count;) {
    bool is_recordable = pyre_graph_node_is_recordable(nodes[i].type);
    if (!is_recordable) {
      pyre_graph_partition_type_t partition_type;
      switch (nodes[i].type) {
        case PYRE_GRAPH_NODE_TYPE_INTERNAL_EMPTY:
          partition_type = PYRE_GRAPH_PARTITION_TYPE_EMPTY;
          break;
        case PYRE_GRAPH_NODE_TYPE_INTERNAL_HOST_CALL:
          partition_type = PYRE_GRAPH_PARTITION_TYPE_HOST_CALL;
          break;
        default:
          partition_type = PYRE_GRAPH_PARTITION_TYPE_EMPTY;
          break;
      }
      partitions[partition_count] = (pyre_graph_partition_t){
          .start_index = i,
          .count = 1,
          .type = partition_type,
          .stream_count = 1,
      ...
      while (i < node_count &&
             partition_size < PYRE_GRAPH_MAX_PARTITION_SIZE &&
             pyre_graph_node_is_recordable(nodes[i].type)) {
        ...
        const bool use_workstreams =
            (i - recordable_start) >= PYRE_GRAPH_MIN_PARTITION_SIZE_FOR_STREAMS;
        ...
        if (use_workstreams && connected_streams == 0 &&
            active_streams < PYRE_GRAPH_MAX_WORKSTREAMS) {
          uint32_t remaining_in_partition =
              PYRE_GRAPH_MAX_PARTITION_SIZE - partition_size;
          if (remaining_in_partition >= PYRE_GRAPH_MIN_NODES_PER_STREAM) {
            assigned_stream = active_streams++;
            streams[assigned_stream].last_node_index = i;
            if ((i - recordable_start) < PYRE_GRAPH_MAX_REACHABILITY_BITS) {
          ...
          const bool is_sync_point =
              (nodes[i].node->dependency_count > PYRE_GRAPH_MAX_FAN_OUT);
      ...
      partitions[partition_count] = (pyre_graph_partition_t){
          ...
          .type = PYRE_GRAPH_PARTITION_TYPE_RECORDABLE,
  ...
  return (pyre_uint32x2_t){{partition_count, block_count}};
}
...
pyre_status_t pyre_graph_schedule_nodes(
  ...
  pyre_graph_sort_node_t* sorted_nodes = NULL;
  ...
  const bool is_sorted = pyre_graph_prepare_nodes(node_blocks, node_count,
                                                   sorted_nodes,
                                                   node_index_map);
  IREE_TRACE_ZONE_APPEND_VALUE_I64(z0, is_sorted);

  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, pyre_graph_topological_sort(sorted_nodes, node_count, node_index_map,
                                      additional_edges, arena, is_sorted));

  pyre_graph_partition_t* partitions = NULL;
  ...
  const pyre_uint32x2_t partition_block_counts =
      pyre_graph_partition_with_streams(sorted_nodes, node_count,

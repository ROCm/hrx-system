...
static iree_status_t iree_hal_remote_server_bulk_router_on_start(
    void* user_data, uint64_t transfer_id, uint64_t total_size,
    iree_net_bulk_frame_flags_t flags) {
  iree_hal_remote_server_bulk_router_t* router =
      (iree_hal_remote_server_bulk_router_t*)user_data;
  return router->operations.start(router->user_data, transfer_id, total_size,
                                  flags);
}

static iree_status_t iree_hal_remote_server_bulk_router_on_data(
...
static iree_status_t iree_hal_remote_server_bulk_router_on_complete(
...
static iree_status_t iree_hal_remote_server_bulk_router_on_abort(
...
static void iree_hal_remote_server_bulk_router_on_transport_error(
...
static void iree_hal_remote_server_bulk_router_on_send_complete(
...
static void iree_hal_remote_server_bulk_router_on_credit(
...
iree_net_bulk_channel_callbacks_t iree_hal_remote_server_bulk_router_callbacks(
  ...
  iree_net_bulk_channel_callbacks_t callbacks = {
      .on_start = iree_hal_remote_server_bulk_router_on_start,
      .on_data = iree_hal_remote_server_bulk_router_on_data,
      .on_complete = iree_hal_remote_server_bulk_router_on_complete,
      .on_abort = iree_hal_remote_server_bulk_router_on_abort,
      .on_transport_error =
          iree_hal_remote_server_bulk_router_on_transport_error,
      .on_send_complete = iree_hal_remote_server_bulk_router_on_send_complete,
      .on_credit = iree_hal_remote_server_bulk_router_on_credit,
      .user_data = router,
  };
  return callbacks;
}

...
// HAL remote bulk channel router.
//
// Bridges the transport-level iree_net_bulk_channel callback ABI to
// HAL remote bulk operations. The router owns callback status ownership and
// transport event normalization; operation callbacks own HAL/file/profile
// behavior and must perform their own locking.

#ifndef IREE_HAL_REMOTE_UTIL_BULK_ROUTER_H_
#define IREE_HAL_REMOTE_UTIL_BULK_ROUTER_H_
...
typedef struct iree_hal_remote_bulk_router_t iree_hal_remote_bulk_router_t;

// Called when a START frame is received.
typedef iree_status_t (*iree_hal_remote_bulk_router_start_fn_t)(
    void* user_data, uint64_t transfer_id, uint64_t total_size,
    iree_net_bulk_frame_flags_t flags);

// Called when a DATA frame is received.
typedef iree_status_t (*iree_hal_remote_bulk_router_data_fn_t)(
...
typedef iree_status_t (*iree_hal_remote_bulk_router_complete_fn_t)(
    void* user_data, uint64_t transfer_id);

// Called when an ABORT frame is received.
typedef iree_status_t (*iree_hal_remote_bulk_router_abort_fn_t)(
    void* user_data, uint64_t transfer_id);

// Called when the bulk transport reports an error. Consumes |status|.
typedef void (*iree_hal_remote_bulk_router_transport_error_fn_t)(
    void* user_data, iree_status_t status);

// Called when a send operation completes. Consumes |status|.
typedef void (*iree_hal_remote_bulk_router_send_complete_fn_t)(
    void* user_data, uint64_t operation_user_data, iree_status_t status);

// Called when peer receive credit is replenished.
typedef void (*iree_hal_remote_bulk_router_credit_fn_t)(void* user_data);

// Retains callback user data for asynchronous send-completion handling.
typedef void (*iree_hal_remote_bulk_router_retain_fn_t)(void* user_data);

// Releases callback user data retained for asynchronous send-completion
// handling.
typedef void (*iree_hal_remote_bulk_router_release_fn_t)(void* user_data);

// Operation callbacks supplied by the owning bulk session.
typedef struct iree_hal_remote_bulk_router_operations_t {
  // Optional callback retaining |user_data| across send completion dispatch.
  iree_hal_remote_bulk_router_retain_fn_t retain;

  // Optional callback releasing |user_data| after send completion dispatch.
  iree_hal_remote_bulk_router_release_fn_t release;

  // Required START handler.
  iree_hal_remote_bulk_router_start_fn_t start;

  // Required DATA handler.
  iree_hal_remote_bulk_router_data_fn_t data;

  // Required COMPLETE handler.
  iree_hal_remote_bulk_router_complete_fn_t complete;

  // Required ABORT handler.
  iree_hal_remote_bulk_router_abort_fn_t abort;

  // Required transport error handler. Consumes the received status.
  iree_hal_remote_bulk_router_transport_error_fn_t transport_error;

  // Required send completion handler. Consumes the received status.
  iree_hal_remote_bulk_router_send_complete_fn_t send_complete;

  // Required credit replenishment handler.
  iree_hal_remote_bulk_router_credit_fn_t credit;
} iree_hal_remote_bulk_router_operations_t;

// Routes transport callbacks to HAL remote bulk operations.
typedef struct iree_hal_remote_bulk_router_t {
...
#endif  // IREE_HAL_REMOTE_UTIL_BULK_ROUTER_H_

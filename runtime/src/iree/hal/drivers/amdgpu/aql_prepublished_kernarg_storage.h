...
// Mode used to materialize reusable command-buffer kernarg templates.
typedef enum iree_hal_amdgpu_aql_prepublished_kernarg_storage_mode_e {
  IREE_HAL_AMDGPU_AQL_PREPUBLISHED_KERNARG_STORAGE_MODE_DISABLED = 0,
  ...
  IREE_HAL_AMDGPU_AQL_PREPUBLISHED_KERNARG_STORAGE_MODE_DEVICE_FINE_HOST_COHERENT =
      1,
} iree_hal_amdgpu_aql_prepublished_kernarg_storage_mode_t;

// Storage mode for finalized reusable command-buffer kernarg templates.
typedef struct iree_hal_amdgpu_aql_prepublished_kernarg_storage_t {
  // Selected backing mode.
  iree_hal_amdgpu_aql_prepublished_kernarg_storage_mode_t mode;

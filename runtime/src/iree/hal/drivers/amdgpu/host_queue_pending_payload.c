...
static iree_status_t iree_hal_amdgpu_pending_op_issue_capture_timestamp(
    iree_hal_amdgpu_pending_op_t* op,
    const iree_hal_amdgpu_wait_resolution_t* resolution,
    iree_hal_amdgpu_pending_op_payload_issue_t* issue) {
  iree_status_t status = iree_hal_amdgpu_host_queue_submit_capture_timestamp(
      op->queue, resolution, op->signal_semaphore_list,
      op->capture_timestamp.target_buffer, op->capture_timestamp.target_offset,
      op->capture_timestamp.flags,
      IREE_HAL_AMDGPU_HOST_QUEUE_SUBMISSION_FLAG_NONE, &issue->ready);
  if (iree_status_is_ok(status) && issue->ready) {
    op->retained_resource_count = 0;
  }
  ...
  switch (op->type) {
    ...
    case IREE_HAL_AMDGPU_PENDING_OP_HOST_ACTION:
      return iree_hal_amdgpu_pending_op_issue_host_action(op, resolution,
                                                          issue);
    case IREE_HAL_AMDGPU_PENDING_OP_CAPTURE_TIMESTAMP:
      return iree_hal_amdgpu_pending_op_issue_capture_timestamp(op, resolution,
                                                                issue);
    default:
...
iree_status_t iree_hal_amdgpu_host_queue_defer_capture_timestamp(
    iree_hal_amdgpu_host_queue_t* queue,
    const iree_hal_semaphore_list_t* wait_semaphore_list,
    const iree_hal_semaphore_list_t* signal_semaphore_list,
    iree_hal_buffer_t* target_buffer, iree_device_size_t target_offset,
  ...
  IREE_RETURN_IF_ERROR(iree_hal_amdgpu_pending_op_allocate(
      queue, wait_semaphore_list, signal_semaphore_list,
      IREE_HAL_AMDGPU_PENDING_OP_CAPTURE_TIMESTAMP, max_resources, &op));
  iree_hal_amdgpu_pending_op_retain(op, (iree_hal_resource_t*)target_buffer);
  op->capture_timestamp.target_buffer = target_buffer;
  op->capture_timestamp.target_offset = target_offset;
  op->capture_timestamp.flags = flags;
  *out_op = op;
  return iree_ok_status();
}

...
iree-run-loom --backend=amdgpu --device=amdgpu --probe-hal
# Device ordinal 0 (danger, this may change across reboots):
iree-run-loom --backend=amdgpu --device=amdgpu:0 --probe-hal
# Device with a stable UUID for a device:
iree-run-loom --backend=amdgpu \
...
ROCR_VISIBLE_DEVICES=2,3 iree-run-loom \
  --backend=amdgpu --device=amdgpu --probe-hal
...
Add `--constant=`, `--binding=`, and `--expected-binding=` flags matching the
kernel ABI when running kernels that require arguments or output validation.

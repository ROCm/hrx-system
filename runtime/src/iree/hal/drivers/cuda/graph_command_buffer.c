...
static iree_status_t iree_hal_cuda_graph_command_buffer_execution_barrier(
  ...
  if (flags != IREE_HAL_EXECUTION_BARRIER_FLAG_NONE) {
    return iree_make_status(
        IREE_STATUS_UNIMPLEMENTED,
        "system-scoped CUDA graph execution barriers are unsupported");
  }
  iree_hal_cuda_graph_command_buffer_t* command_buffer =
      iree_hal_cuda_graph_command_buffer_cast(base_command_buffer);
  IREE_TRACE_ZONE_BEGIN(z0);
...
static iree_status_t iree_hal_cuda_graph_command_buffer_atomic_wait(
    iree_hal_command_buffer_t* base_command_buffer,
    iree_hal_execution_stage_t source_stage_mask,
    iree_hal_execution_stage_t target_stage_mask,
  ...
  return iree_make_status(
      IREE_STATUS_UNIMPLEMENTED,
      "CUDA graph command buffers do not support atomic waits");
}

static iree_status_t iree_hal_cuda_graph_command_buffer_atomic_store(
    iree_hal_command_buffer_t* base_command_buffer,
    iree_hal_execution_stage_t source_stage_mask,
    iree_hal_execution_stage_t target_stage_mask,
  ...
  return iree_make_status(
      IREE_STATUS_UNIMPLEMENTED,
      "CUDA graph command buffers do not support atomic stores");
}

static iree_status_t iree_hal_cuda_graph_command_buffer_atomic_rmw(
    iree_hal_command_buffer_t* base_command_buffer,
    iree_hal_execution_stage_t source_stage_mask,
    iree_hal_execution_stage_t target_stage_mask,
  ...
  return iree_make_status(
      IREE_STATUS_UNIMPLEMENTED,
      "CUDA graph command buffers do not support atomic read-modify-write");
}
  ...
  if (IREE_UNLIKELY(constants.data_length != expected_constant_length)) {
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                             "CUDA graph dispatch provides %" PRIhsz
    ...
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                             "CUDA graph dispatch constants must be non-null "
    ...
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                             "CUDA graph dispatch bindings must be non-null "
...
static const iree_hal_command_buffer_vtable_t
    iree_hal_cuda_graph_command_buffer_vtable = {
        ...
        .atomic_wait = iree_hal_cuda_graph_command_buffer_atomic_wait,
        .atomic_store = iree_hal_cuda_graph_command_buffer_atomic_store,
        .atomic_rmw = iree_hal_cuda_graph_command_buffer_atomic_rmw,

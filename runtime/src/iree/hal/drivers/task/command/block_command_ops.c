...
iree_status_t iree_hal_cmd_build_dispatch(
  ...
  if (task_executable->dispatch_attrs) {
    dispatch_attrs = task_executable->dispatch_attrs[export_ordinal];
  }
  ...
  cmd->function = task_executable->dispatch_ptrs
                      ? task_executable->dispatch_ptrs[export_ordinal]
                      : NULL;

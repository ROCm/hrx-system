...
typedef struct iree_hal_vulkan_copy_unaligned_push_constants_t {
  // Source storage-buffer uint32 element containing the first byte.
  uint32_t source_word_index;

  // Byte offset within source_word_index where the copy begins.
  uint32_t source_word_byte_offset;

  // Target storage-buffer uint32 element containing the first byte.
  uint32_t target_word_index;

  // Byte offset within target_word_index where the copy begins.
  uint32_t target_word_byte_offset;

  // Number of bytes to copy.
  uint32_t length_bytes;

  // Number of target words covered by the copy.
  uint32_t target_word_count;
} iree_hal_vulkan_copy_unaligned_push_constants_t;

static_assert(sizeof(iree_hal_vulkan_copy_unaligned_push_constants_t) == 24,
              "push constant layout must match the built-in shader");

// Stays below the minimum Vulkan workgroup-count limit while leaving room for
// descriptor alignment without approaching maxStorageBufferRange.
#define IREE_HAL_VULKAN_COPY_SHADER_CHUNK_LENGTH \
  (16ull * 1024ull * 1024ull - 4096ull)
...
iree_status_t iree_hal_vulkan_builtins_initialize(
  ...
  VkDescriptorSetLayoutBinding copy_layout_bindings[2] = {
      {
          .binding = 0,
          .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
          .descriptorCount = 1,
  ...
  VkDescriptorSetLayoutCreateInfo copy_descriptor_set_layout_create_info = {
      .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO,
      .bindingCount = IREE_ARRAYSIZE(copy_layout_bindings),
      .pBindings = copy_layout_bindings,
  };
  if (iree_status_is_ok(status)) {
    IREE_LEAK_CHECK_DISABLE_PUSH();
    status = iree_vkCreateDescriptorSetLayout(
        IREE_VULKAN_DEVICE(&out_builtins->syms), out_builtins->logical_device,
        &copy_descriptor_set_layout_create_info, /*pAllocator=*/NULL,
        &out_builtins->copy_descriptor_set_layout);
    IREE_LEAK_CHECK_DISABLE_POP();
  }
  if (iree_status_is_ok(status)) {
    VkPushConstantRange copy_push_constant_range = {
        .stageFlags = VK_SHADER_STAGE_COMPUTE_BIT,
        .offset = 0,
        .size =
            (uint32_t)sizeof(iree_hal_vulkan_copy_unaligned_push_constants_t),
    };
    VkPipelineLayoutCreateInfo copy_pipeline_layout_create_info = {
        .sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO,
        .setLayoutCount = 1,
        .pSetLayouts = &out_builtins->copy_descriptor_set_layout,
        .pushConstantRangeCount = 1,
        .pPushConstantRanges = &copy_push_constant_range,
    };
    IREE_LEAK_CHECK_DISABLE_PUSH();
    status = iree_vkCreatePipelineLayout(
        IREE_VULKAN_DEVICE(&out_builtins->syms), out_builtins->logical_device,
        &copy_pipeline_layout_create_info, /*pAllocator=*/NULL,
        &out_builtins->copy_pipeline_layout);
    IREE_LEAK_CHECK_DISABLE_POP();
  }
  if (iree_status_is_ok(status)) {
    status = iree_hal_vulkan_builtins_create_compute_pipeline(
        out_builtins, IREE_SV("copy_unaligned.spv"),
        out_builtins->copy_pipeline_layout, &out_builtins->copy_pipeline);
  }
  if (iree_status_is_ok(status)) {
...
void iree_hal_vulkan_builtins_deinitialize(
  ...
  if (builtins->copy_pipeline) {
    iree_vkDestroyPipeline(IREE_VULKAN_DEVICE(&builtins->syms),
                           builtins->logical_device, builtins->copy_pipeline,
                           /*pAllocator=*/NULL);
  }
  if (builtins->copy_pipeline_layout) {
    iree_vkDestroyPipelineLayout(
        IREE_VULKAN_DEVICE(&builtins->syms), builtins->logical_device,
        builtins->copy_pipeline_layout, /*pAllocator=*/NULL);
  }
  if (builtins->copy_descriptor_set_layout) {
    iree_vkDestroyDescriptorSetLayout(
        IREE_VULKAN_DEVICE(&builtins->syms), builtins->logical_device,
        builtins->copy_descriptor_set_layout, /*pAllocator=*/NULL);
  }
...
static VkDeviceSize iree_hal_vulkan_builtins_copy_shader_chunk_length(
    VkDeviceSize target_offset, VkDeviceSize remaining_length) {
  const VkDeviceSize target_word_byte_offset = target_offset % sizeof(uint32_t);
  const VkDeviceSize chunk_capacity =
      IREE_HAL_VULKAN_COPY_SHADER_CHUNK_LENGTH - target_word_byte_offset;
  return iree_min(chunk_capacity, remaining_length);
}
...
iree_status_t iree_hal_vulkan_builtins_copy_descriptor_set_count(
  ...
  IREE_ASSERT_ARGUMENT(out_descriptor_set_count);
  if (length == 0) {
    *out_descriptor_set_count = 0;
    return iree_ok_status();
  }
  if (length > UINT64_MAX - source_offset ||
      length > UINT64_MAX - target_offset) {
    return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                            "Vulkan built-in copy range overflows");
  }

  const VkDeviceSize source_alignment = source_offset % sizeof(uint32_t);
  const VkDeviceSize target_alignment = target_offset % sizeof(uint32_t);
  if (source_alignment == 0 && target_alignment == 0 &&
      length % sizeof(uint32_t) == 0) {
    *out_descriptor_set_count = 0;
    return iree_ok_status();
  }

  uint64_t descriptor_set_count = 0;
  if (source_alignment == target_alignment) {
    VkDeviceSize start_edge_length = 0;
    if (target_alignment != 0) {
      start_edge_length =
          iree_min((VkDeviceSize)sizeof(uint32_t) - target_alignment, length);
    }
    if (start_edge_length != 0) ++descriptor_set_count;
    const VkDeviceSize remaining_length = length - start_edge_length;
    if (remaining_length % sizeof(uint32_t) != 0) ++descriptor_set_count;
  } else {
    VkDeviceSize copied_length = 0;
    while (copied_length < length) {
      if (descriptor_set_count == UINT32_MAX) {
        return iree_make_status(
            IREE_STATUS_OUT_OF_RANGE,
            "Vulkan built-in copy requires more than UINT32_MAX descriptor "
            "sets");
      }
      const VkDeviceSize chunk_length =
          iree_hal_vulkan_builtins_copy_shader_chunk_length(
              target_offset + copied_length, length - copied_length);
      copied_length += chunk_length;
      ++descriptor_set_count;
    }
  }
  *out_descriptor_set_count = (uint32_t)descriptor_set_count;
  return iree_ok_status();
}

static iree_status_t iree_hal_vulkan_builtins_record_copy_shader_range(
    const iree_hal_vulkan_builtins_t* builtins, VkCommandBuffer command_buffer,
  ...
  if (!descriptor_pool) {
    return iree_make_status(IREE_STATUS_FAILED_PRECONDITION,
                            "Vulkan built-in copy requires a descriptor pool");
  }
  if (length > IREE_HAL_VULKAN_COPY_SHADER_CHUNK_LENGTH ||
      length > UINT32_MAX) {
    return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                            "Vulkan built-in copy shader range is too large");
  }
  if (length > UINT64_MAX - source_offset ||
      length > UINT64_MAX - target_offset) {
    return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
  ...
  const VkDeviceSize descriptor_alignment =
      iree_max((VkDeviceSize)sizeof(uint32_t),
               builtins->min_storage_buffer_offset_alignment);
  const VkDeviceSize descriptor_alignment_mask = descriptor_alignment - 1;
  const VkDeviceSize source_word_offset = source_offset & ~(VkDeviceSize)3;
  const VkDeviceSize source_descriptor_offset =
      source_word_offset & ~descriptor_alignment_mask;
  VkDeviceSize source_range_end = 0;
  if (!iree_device_size_checked_align(source_offset + length, sizeof(uint32_t),
                                      &source_range_end)) {
    return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                            "Vulkan built-in copy source range overflows");
  }
  const VkDeviceSize target_word_offset = target_offset & ~(VkDeviceSize)3;
  const VkDeviceSize target_descriptor_offset =
      target_word_offset & ~descriptor_alignment_mask;
  VkDeviceSize target_range_end = 0;
  if (!iree_device_size_checked_align(target_offset + length, sizeof(uint32_t),
                                      &target_range_end)) {
    return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                            "Vulkan built-in copy target range overflows");
  }
  if (source_range_end > source_buffer_length ||
      target_range_end > target_buffer_length) {
    return iree_make_status(
        IREE_STATUS_OUT_OF_RANGE,
        "Vulkan built-in copy rounded range exceeds its backing buffer");
  }

  const VkDeviceSize source_word_index =
      (source_word_offset - source_descriptor_offset) / sizeof(uint32_t);
  const VkDeviceSize target_word_index =
      (target_word_offset - target_descriptor_offset) / sizeof(uint32_t);
  const VkDeviceSize target_word_count =
      ((target_offset - target_word_offset) + length + sizeof(uint32_t) - 1) /
      sizeof(uint32_t);
  if (source_word_index > UINT32_MAX || target_word_index > UINT32_MAX ||
      target_word_count > UINT32_MAX) {
    return iree_make_status(
        IREE_STATUS_OUT_OF_RANGE,
        "Vulkan built-in copy push constant range exceeds uint32_t");
  }
  ...
  VkDescriptorSetAllocateInfo allocate_info = {
      ...
      .pSetLayouts = &builtins->copy_descriptor_set_layout,
  };
  IREE_RETURN_IF_ERROR(iree_vkAllocateDescriptorSets(
      IREE_VULKAN_DEVICE(&builtins->syms), builtins->logical_device,
  ...
  VkDescriptorBufferInfo buffer_infos[2] = {
      {
          ...
          .offset = source_descriptor_offset,
          .range = source_range_end - source_descriptor_offset,
      },
      {
          .buffer = target_buffer,
          .offset = target_descriptor_offset,
          .range = target_range_end - target_descriptor_offset,
      },
  };
  ...
  iree_vkUpdateDescriptorSets(IREE_VULKAN_DEVICE(&builtins->syms),
                              builtins->logical_device,
                              IREE_ARRAYSIZE(write_infos), write_infos,
                              /*descriptorCopyCount=*/0,
                              /*pDescriptorCopies=*/NULL);

  const iree_hal_vulkan_copy_unaligned_push_constants_t constants = {
      .source_word_index = (uint32_t)source_word_index,
      .source_word_byte_offset = (uint32_t)(source_offset - source_word_offset),
      .target_word_index = (uint32_t)target_word_index,
      .target_word_byte_offset = (uint32_t)(target_offset - target_word_offset),
      .length_bytes = (uint32_t)length,
      .target_word_count = (uint32_t)target_word_count,
  };
  iree_vkCmdBindPipeline(IREE_VULKAN_DEVICE(&builtins->syms), command_buffer,
                         VK_PIPELINE_BIND_POINT_COMPUTE,
                         builtins->copy_pipeline);
  iree_vkCmdBindDescriptorSets(
      IREE_VULKAN_DEVICE(&builtins->syms), command_buffer,
      VK_PIPELINE_BIND_POINT_COMPUTE, builtins->copy_pipeline_layout,
      /*firstSet=*/0, /*descriptorSetCount=*/1, &descriptor_set,
      /*dynamicOffsetCount=*/0, /*pDynamicOffsets=*/NULL);
  iree_vkCmdPushConstants(IREE_VULKAN_DEVICE(&builtins->syms), command_buffer,
                          builtins->copy_pipeline_layout,
                          VK_SHADER_STAGE_COMPUTE_BIT, /*offset=*/0,
                          sizeof(constants), &constants);
  const uint32_t workgroup_count = ((uint32_t)target_word_count + 63u) / 64u;
  iree_vkCmdDispatch(IREE_VULKAN_DEVICE(&builtins->syms), command_buffer,
                     workgroup_count, /*groupCountY=*/1, /*groupCountZ=*/1);
  return iree_ok_status();
}

static void iree_hal_vulkan_builtins_record_copy_aligned_range(
    const iree_hal_vulkan_builtins_t* builtins, VkCommandBuffer command_buffer,
    VkBuffer source_buffer, VkDeviceSize source_offset, VkBuffer target_buffer,
    VkDeviceSize target_offset, VkDeviceSize length) {
  if (length == 0) return;
  ...
  iree_vkCmdCopyBuffer(IREE_VULKAN_DEVICE(&builtins->syms), command_buffer,
                       source_buffer, target_buffer, /*regionCount=*/1,
                       &copy_region);
}
...
iree_status_t iree_hal_vulkan_builtins_record_copy(
  ...
  IREE_ASSERT_ARGUMENT(target_buffer);
  if (length == 0) return iree_ok_status();
  if (source_offset > source_buffer_length ||
      length > source_buffer_length - source_offset ||
      target_offset > target_buffer_length ||
      length > target_buffer_length - target_offset) {
    return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                            "Vulkan built-in copy range exceeds its backing "
                            "buffer");
  }
  ...
  if (source_alignment == 0 && target_alignment == 0 &&
      length % sizeof(uint32_t) == 0) {
    iree_hal_vulkan_builtins_record_copy_aligned_range(
        builtins, command_buffer, source_buffer, source_offset, target_buffer,
        target_offset, length);
    return iree_ok_status();
  }
  ...
  if (source_alignment == target_alignment) {
    ...
    IREE_RETURN_IF_ERROR(iree_hal_vulkan_builtins_record_copy_shader_range(
        builtins, command_buffer, descriptor_pool, source_buffer,
        source_buffer_length, source_offset, target_buffer,
        target_buffer_length, target_offset, start_edge_length));

    const VkDeviceSize remaining_length = length - start_edge_length;
    const VkDeviceSize aligned_length =
        remaining_length & ~(VkDeviceSize)(sizeof(uint32_t) - 1);
    iree_hal_vulkan_builtins_record_copy_aligned_range(
        builtins, command_buffer, source_buffer,
        source_offset + start_edge_length, target_buffer,
        target_offset + start_edge_length, aligned_length);

    const VkDeviceSize end_edge_length = remaining_length - aligned_length;
    return iree_hal_vulkan_builtins_record_copy_shader_range(
        builtins, command_buffer, descriptor_pool, source_buffer,
        source_buffer_length,
        source_offset + start_edge_length + aligned_length, target_buffer,
        target_buffer_length,
        target_offset + start_edge_length + aligned_length, end_edge_length);
  }
  ...
  while (copied_length < length) {
    ...
    IREE_RETURN_IF_ERROR(iree_hal_vulkan_builtins_record_copy_shader_range(
        builtins, command_buffer, descriptor_pool, source_buffer,
        source_buffer_length, source_offset + copied_length, target_buffer,
        target_buffer_length, target_offset + copied_length, chunk_length));
    copied_length += chunk_length;
  }
  return iree_ok_status();
}

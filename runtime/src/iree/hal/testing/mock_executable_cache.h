...
// time. When prepare_executable is called with a matching format, it creates a
// mock executable from the supplied executable params. Formats not in the
// supported list are rejected with INCOMPATIBLE.
...
// executable_format matches any supported format. If so, it delegates to
// iree_hal_mock_executable_create. If not, it returns INCOMPATIBLE.

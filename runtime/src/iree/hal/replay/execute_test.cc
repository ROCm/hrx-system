...
static std::vector<uint8_t> MakeReservedDispatchReplayFile() {
  ...
  payload.reserved0 = 1;
  const iree_const_byte_span_t payload_span =
...
TEST(ReplayExecuteTest, RejectsNonzeroReservedDispatchFields) {
  std::vector<uint8_t> storage = MakeReservedDispatchReplayFile();
  iree_hal_device_group_t* replay_group = CreateSyncDeviceGroup();

...
static iree_status_t iree_net_shm_endpoint_deactivate(
  ...
  if (iree_status_is_ok(status) &&
    ...
    status = iree_net_carrier_deactivate(
  ...
  *out_connection = NULL;
  ...
  if (iree_status_is_ok(status)) {
    status = iree_net_shm_connection_create_with_slots(
        proactor, endpoint_count, host_allocator, &connection);
  }
  if (iree_status_is_ok(status)) {
  ...
  for (uint16_t i = 0; i < connection->max_endpoint_count; ++i) {
    ...
    if (slot->adapter) {
      ...
      iree_status_t status = iree_net_carrier_deactivate(
          slot->adapter->carrier, iree_net_shm_endpoint_carrier_deactivated,
          slot->adapter);
      if (!iree_status_is_ok(status)) {
...
static iree_status_t iree_net_shm_stop_deferred_submit(
  ...
  iree_async_operation_initialize(
      ...
      iree_net_shm_stop_deferred_complete, deferred);
  iree_status_t status =
      iree_async_proactor_submit_one(proactor, &deferred->nop.base);
  if (!iree_status_is_ok(status)) {
    iree_allocator_free(host_allocator, deferred);
...
static iree_status_t iree_net_shm_factory_connect(
  ...
  if (listener) {
    ...
    if (iree_status_is_ok(status)) {
      status = iree_net_shm_connection_create_with_slots(
          listener->proactor, max_endpoints, factory->host_allocator,
          &server_connection);
    }

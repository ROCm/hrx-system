...
typedef struct iree_vm_program_modules_t {
  ...
  // Library modules available to satisfy imports.
...
// Links and failure-atomically publishes one immutable program. Every input
// module must already be linkable and contain canonical resolved ref types.
// Program creation retains and name-sorts the modules, merges structural
// callable contracts, assigns opaque process-storage offsets, resolves all
// imports, and selects only the executable module's `initialize` export.
// Libraries receive no implicit initialization. The operation performs no
// environment lookup or guest execution and allocates exactly one slab.
// |out_program| is set to null before input validation. Failure releases every
// temporary module owner and leaves no partially linked state.
...
// Resolves one module-bound export into a program-bound function value. The
// export's module must participate in |program|. The result borrows |program|
// and remains valid only while the program is live. Failure leaves

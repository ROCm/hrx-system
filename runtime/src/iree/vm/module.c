...
static iree_status_t iree_vm_module_validate_vtable(
  ...
  if (vtable->structure_size < IREE_VM_MODULE_VTABLE_V0_REQUIRED_SIZE) {
    return iree_make_status(IREE_STATUS_INCOMPATIBLE,
                            "module vtable has %" PRIu32
                            " bytes but ABI version zero requires at least %zu",
                            vtable->structure_size,
                            (size_t)IREE_VM_MODULE_VTABLE_V0_REQUIRED_SIZE);
  }
...
static iree_status_t iree_vm_module_validate_metadata_value(
  ...
  switch (value.type) {
    ...
    case IREE_VM_METADATA_VALUE_TYPE_UTF8: {
      iree_string_view_t text = iree_make_string_view(
...
static iree_status_t iree_vm_module_validate_metadata_scope(
  ...
  for (iree_host_size_t i = 0; i < count; ++i) {
    ...
    if (i != 0 && iree_string_view_compare(previous_key, entry.key) >= 0) {
      return iree_make_status(
          IREE_STATUS_INVALID_ARGUMENT,
          "metadata keys must be strictly increasing within each scope");
    }
...
static iree_status_t iree_vm_module_validate_exports(
  ...
  const iree_vm_module_descriptor_t* descriptor = module->descriptor;
  iree_string_view_t previous_export_name = iree_string_view_empty();
  for (iree_host_size_t i = 0; i < descriptor->counts.export_count; ++i) {
...
static iree_status_t iree_vm_module_validate_semantics(
  ...
  return iree_vm_module_validate_metadata_scope(
      module, metadata_scope, module->descriptor->counts.metadata_count);
}
...
IREE_API_EXPORT void iree_vm_call_ref_argument_load_borrow(
  ...
  const iree_vm_ref_t source =
      *iree_vm_call_ref_argument_slot(call, ref_ordinal);
...
IREE_API_EXPORT void iree_vm_call_ref_argument_load_move(
    const iree_vm_call_packet_t* call, uint16_t ref_ordinal,
    iree_vm_ref_t* inout_ref) {
  iree_vm_ref_t* source = iree_vm_call_ref_argument_slot(call, ref_ordinal);
  iree_vm_ref_reset(inout_ref);
  *inout_ref = *source;
...
IREE_API_EXPORT void iree_vm_call_ref_result_store_move(
    const iree_vm_call_packet_t* call, uint16_t ref_ordinal,
    iree_vm_ref_t* inout_ref) {
  iree_vm_ref_t* result = iree_vm_call_ref_result_slot(call, ref_ordinal);

...
// provider execution. No declaration in this file is part of the public ABI.
...
struct iree_vm_frame_t {
  ...
  uint16_t function_ordinal;
  // Whether the framed function may yield.
  bool may_yield;
};
...
struct iree_vm_invocation_t {
  ...
  // Whether root staging contains a nonnull external borrowed ref.
  bool has_external_borrowed_arguments;
  // Whether storage was returned by |iree_vm_invocation_allocate|.
  bool is_allocated;
};
...
// Returns whether |invocation| can accept a new operation.
bool iree_vm_invocation_is_idle(const iree_vm_invocation_t* invocation);

// Removes complete frames above |stack_cursor| in exact LIFO order.
void iree_vm_invocation_unwind_to(iree_vm_invocation_t* invocation,
                                  uint8_t* stack_cursor);

// Validates public span storage without changing invocation or caller bytes.
...
// Releases all valid carriers in a well-formed public argument span.
void iree_vm_invocation_consume_arguments(iree_vm_variant_span_t arguments);

// Validates one root call before process allocation or provider entry. A
...
// Atomically closes an uncancelled operation. NONE means completion won;
// otherwise the returned cancellation reason won.
iree_vm_cancel_reason_t iree_vm_invocation_claim_completion(
    iree_vm_invocation_t* invocation);

// Releases root ownership and returns an already closed operation to idle.
void iree_vm_invocation_finish(iree_vm_invocation_t* invocation);

// Atomically closes, unwinds, and returns one failed operation to idle.

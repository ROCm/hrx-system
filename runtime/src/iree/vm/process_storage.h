...
// Private process representation shared with the invocation core. No
...
struct iree_vm_process_t {
  // Intrusive process owner count, published only after successful sealing.
  iree_atomic_ref_count_t ref_count;
  // Allocator owning the process slab.
  iree_allocator_t host_allocator;
  // Retained immutable linked program.
  iree_vm_program_t* program;
};

// Returns the first byte of the process-state tail.
...
// receive canonical empty spans even though their lifecycle callbacks run.

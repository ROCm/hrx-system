...
typedef struct iree_vm_buffer_release_callback_t {
  ...
  iree_vm_buffer_release_fn_t fn;
  // Unowned callback data transferred with |fn| on successful wrap.
  void* user_data;
...
// Allocates one zero-initialized READ|WRITE buffer. A zero
// |minimum_alignment| selects the runtime's natural maximum alignment. A
// nonzero alignment must be a representable power of two. The result is a
// distinct nonnull object even when |length| is zero. |out_buffer| is set null
// before construction and remains null on failure.
...
// Returns currently effective access. |buffer| must be nonnull. This is an
// instantaneous query rather than a byte-access lease.
IREE_API_EXPORT iree_vm_buffer_access_flags_t
iree_vm_buffer_access(const iree_vm_buffer_t* buffer);

// Returns the immutable logical byte length. |buffer| must be nonnull.
...
// access is absent. |buffer| must be nonnull. A valid empty buffer may also

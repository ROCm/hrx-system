...
template <typename T>
void ExpectApply(uint8_t* address, iree_vm_isa_buffer_atomic_carrier_t carrier,
                 iree_vm_isa_buffer_atomic_kind_t kind, T initial_bits,
                 T operand_bits, T expected_bits) {
  StoreBits(address, initial_bits);
  EXPECT_EQ(
      iree_vm_bytecode_atomic_apply(address, operand_bits, kind, carrier,
                                    IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_RELAXED),
      initial_bits);
...
void ExpectAllApplyKinds(uint8_t* address,
  ...
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_EXCHANGE_INTEGER,
              T{9}, T{4}, T{4});
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_EXCHANGE_FLOAT,
              Bits::FromValue(FloatT{1}), Bits::kSignalingNaN,
              Bits::kSignalingNaN);
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_ADD_INTEGER,
              Limits::max(), T{2}, T{1});
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_ADD_FLOAT,
              Bits::FromValue(FloatT{1.5}), Bits::FromValue(FloatT{2.25}),
              Bits::FromValue(FloatT{3.75}));
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_SUBTRACT_INTEGER,
              T{1}, T{2}, Limits::max());
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_AND_INTEGER,
              T{0xA5}, T{0x3C}, T{0x24});
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_OR_INTEGER,
              T{0xA0}, T{0x0F}, T{0xAF});
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_XOR_INTEGER,
              T{0xAA}, T{0xFF}, T{0x55});
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_MINIMUM_SIGNED,
              SignedBits<T>(std::numeric_limits<SignedT>::max()),
              SignedBits<T>(std::numeric_limits<SignedT>::min()),
              SignedBits<T>(std::numeric_limits<SignedT>::min()));
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_MAXIMUM_SIGNED,
              SignedBits<T>(std::numeric_limits<SignedT>::min()),
              SignedBits<T>(std::numeric_limits<SignedT>::max()),
              SignedBits<T>(std::numeric_limits<SignedT>::max()));
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_MINIMUM_UNSIGNED,
              Limits::max(), T{0}, T{0});
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_MAXIMUM_UNSIGNED,
              T{0}, Limits::max(), Limits::max());
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_MINIMUM_FLOAT,
              T{0}, Bits::kNegativeZero, Bits::kNegativeZero);
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_MAXIMUM_FLOAT,
  ...
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_MINNUM_FLOAT,
              Bits::kQuietNaN, Bits::FromValue(FloatT{3}),
              Bits::FromValue(FloatT{3}));
  ExpectApply(address, carrier, IREE_VM_ISA_BUFFER_ATOMIC_KIND_MAXNUM_FLOAT,
              Bits::FromValue(FloatT{3}), Bits::kSignalingNaN,
              Bits::FromValue(FloatT{3}));
}

template <typename T>
void ExpectEveryApplyOrdering(uint8_t* address,
                              iree_vm_isa_buffer_atomic_carrier_t carrier) {
  for (uint8_t ordering = IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_RELAXED;
    ...
    EXPECT_EQ(iree_vm_bytecode_atomic_apply(
                  address, T{5}, IREE_VM_ISA_BUFFER_ATOMIC_KIND_ADD_INTEGER,
                  carrier, ordering),
              T{7});
...
struct AtomicOrderingPair {
  ...
  iree_vm_isa_buffer_atomic_ordering_t success;
  // Ordering applied when the comparison fails.
  iree_vm_isa_buffer_atomic_ordering_t failure;
};

constexpr AtomicOrderingPair kAtomicOrderingPairs[] = {
    {IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_RELAXED,
     IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_RELAXED},
    {IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_ACQUIRE,
     IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_RELAXED},
    {IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_ACQUIRE,
     IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_ACQUIRE},
    {IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_RELEASE,
     IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_RELAXED},
    {IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_ACQ_REL,
     IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_RELAXED},
    {IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_ACQ_REL,
     IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_ACQUIRE},
    {IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_SEQ_CST,
     IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_RELAXED},
    {IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_SEQ_CST,
     IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_ACQUIRE},
    {IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_SEQ_CST,
...
void ExpectEveryCompareExchangeOrderingPair(
    uint8_t* address, iree_vm_isa_buffer_atomic_carrier_t carrier) {
...
TEST(VMBytecodeInterpreterAtomicTest, AppliesEveryI32KindToRawStorage) {
  if (!iree_vm_bytecode_atomic_carrier_is_supported(
          IREE_VM_ISA_BUFFER_ATOMIC_CARRIER_I32)) {
    GTEST_SKIP() << "i32 atomics are unsupported by this target";
  }
  alignas(4) std::array<uint8_t, 4> storage = {};
  ExpectAllApplyKinds<uint32_t, int32_t, float>(
      storage.data(), IREE_VM_ISA_BUFFER_ATOMIC_CARRIER_I32);
  ExpectEveryApplyOrdering<uint32_t>(storage.data(),
                                     IREE_VM_ISA_BUFFER_ATOMIC_CARRIER_I32);
}
...
TEST(VMBytecodeInterpreterAtomicTest, AppliesEveryI64KindToRawStorage) {
  ...
  ExpectAllApplyKinds<uint64_t, int64_t, double>(
      storage.data(), IREE_VM_ISA_BUFFER_ATOMIC_CARRIER_I64);
  ExpectEveryApplyOrdering<uint64_t>(storage.data(),
                                     IREE_VM_ISA_BUFFER_ATOMIC_CARRIER_I64);
}
...
TEST(VMBytecodeInterpreterAtomicTest, PublishesMemoryWithReleaseAcquire) {
  ...
  std::thread reader([&]() {
    ...
    do {
      observed_flag = iree_vm_bytecode_atomic_apply(
          flag_storage.data(), 0, IREE_VM_ISA_BUFFER_ATOMIC_KIND_ADD_INTEGER,
          IREE_VM_ISA_BUFFER_ATOMIC_CARRIER_I32,
          IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_ACQUIRE);
  ...
  std::thread writer([&]() {
    ...
    (void)iree_vm_bytecode_atomic_apply(
        flag_storage.data(), 1, IREE_VM_ISA_BUFFER_ATOMIC_KIND_EXCHANGE_INTEGER,
        IREE_VM_ISA_BUFFER_ATOMIC_CARRIER_I32,
        IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_RELEASE);
  });
  writer.join();
    ...
    thread = std::thread([&storage, carrier, kIterationCount]() {
      for (uint32_t i = 0; i < kIterationCount; ++i) {
        (void)iree_vm_bytecode_atomic_apply(
            ...
            carrier, IREE_VM_ISA_BUFFER_ATOMIC_ORDERING_RELAXED);
      }
    });
  }

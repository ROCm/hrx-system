...
CORE_MACHINE = NormativeClause(
    ...
    normative_text=(
        ...
        "Each physical instruction also declares a conservative set of may-"
        "effects on non-register architectural state. Unknown, read, write, "
        "allocate, release, and synchronize effects include work scheduled for "
        "deferred execution. Read observes mutable state; write may change it; "
        "allocate creates a distinct resource lifetime; release invalidates such a "
        "lifetime; and synchronize establishes or awaits ordering without by itself "
        "implying a payload read or write. Resource fields identify encoded resource "
        "selectors; "
        "an associated range group's count is implicit and distinct field selectors "
        "do not prove that selected objects are disjoint. The unknown-any effect "
        "aliases every state resource. The buffer domain includes core vm.buffer "
        "and HAL buffer or mapping storage so views and mappings retain their "
        "possible aliasing. An empty set means the instruction has no "
        "such effect; register dataflow, static control, suspension, ref ownership, "
        "and reads of immutable state remain authoritative in their existing "
        "declarations and are not repeated as state effects. The effect summary is "
        "normative but does not replace the instruction's complete success, failure, "

...
class ProjectionTest(unittest.TestCase):
    ...
    def test_generated_file_set_is_deterministic_and_sharded(self) -> None:
        ...
        for path, contents in (wire_outputs | documentation_outputs).items():
    ...
    def test_write_and_check_detect_nested_output_drift(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            output_directory = pathlib.Path(temporary_directory)
            self.assertTrue(write_or_check_wire(output_directory, check_only=False))
            self.assertTrue(write_or_check_wire(output_directory, check_only=True))

            stale_path = output_directory / "wire/core/control.h"
            stale_contents = stale_path.read_text(encoding="utf-8")
            stale_path.write_text("stale\n", encoding="utf-8")
            errors = io.StringIO()
            with contextlib.redirect_stderr(errors):
                self.assertFalse(write_or_check_wire(output_directory, check_only=True))
            self.assertIn("stale generated file", errors.getvalue())

            stale_path.write_text(stale_contents, encoding="utf-8")
            extra_path = output_directory / "wire/core/unexpected.h"
            ...
            with self.assertRaisesRegex(ValueError, "unexpected files"):
                write_or_check_wire(output_directory, check_only=False)

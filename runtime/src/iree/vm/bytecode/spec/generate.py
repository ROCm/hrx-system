...
"""Generates checked-in wire and build-only documentation projections."""
...
_WIRE_GENERATED_DIRECTORIES = (
    "wire/core",
    "wire/hal",
)
_WIRE_GENERATED_FILES = (
    "wire/assertions.c",
...
def _default_output_directory() -> pathlib.Path:
    ...
    if workspace_directory:
        return pathlib.Path(workspace_directory) / "runtime/src/iree/vm/bytecode"
...
def generated_wire_outputs() -> dict[str, str]:
    """Returns the complete deterministic checked-in C wire projection."""
...
def _existing_wire_paths(output_directory: pathlib.Path) -> set[str]:
    paths = {
        path for path in _WIRE_GENERATED_FILES if (output_directory / path).is_file()
    }
    for directory_name in _WIRE_GENERATED_DIRECTORIES:
...
def write_or_check_wire(
    output_directory: pathlib.Path,
    *,
    check_only: bool,
) -> bool:
    """Writes or checks the checked-in C wire projection."""

    return _write_or_check(
        output_directory,
        generated_wire_outputs(),
        _existing_wire_paths(output_directory),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="wire",
        help="Projection product to generate. Defaults to checked-in wire headers.",
    )
    parser.add_argument(
        "--output-directory",
        type=pathlib.Path,
        help=(
            "Projection root. Defaults to the runtime bytecode directory for "
            "wire output; required for build-only documentation output."
        ),
    )
    ...
    if output_directory is None:
        if arguments.output_kind == "documentation":

...
def _selector_rows(
    specification: Specification, table_name: str
) -> tuple[tuple[int, str], ...]:
    table = next(table for table in specification.selectors if table.name == table_name)
    return tuple((value.value, value.name) for value in table.values)


def _integer_conversion_rows(
    specification: Specification,
) -> tuple[tuple[int, int, int], ...]:
    rows: list[tuple[int, int, int]] = []
    dirty_high_bits = 0xA55AA55AA55AA55A
    for selector, name in _selector_rows(specification, "integer.convert"):
...
def _float_truncate_rows(
    specification: Specification,
) -> tuple[tuple[int, int, int], ...]:
    rows: list[tuple[int, int, int]] = []
    for selector, name in _selector_rows(specification, "float.truncate"):
...
def _float_width_rows(
    specification: Specification,
) -> tuple[tuple[int, int, int], ...]:
    rows: list[tuple[int, int, int]] = []
    for selector, name in _selector_rows(specification, "float.width"):
        source_name, target_name = name.split(".to.")
        source_format = _FORMATS[source_name]
        target_format = _FORMATS[target_name]
        for source_bits in _float_source_samples(source_format, target_name):
...
def _integer_to_float_rows(
    specification: Specification,
) -> tuple[tuple[int, int, int], ...]:
    rows: list[tuple[int, int, int]] = []
    for selector, name in _selector_rows(specification, "integer.to.float"):
        source_name, target_name = name.split(".to.")
...
def _float_to_integer_rows(
    specification: Specification,
) -> tuple[tuple[int, int, str, int], ...]:
    rows: list[tuple[int, int, str, int]] = []
    for selector, name in _selector_rows(specification, "float.to.integer"):
        source_name, destination_name = name.split(".to.")
        source_format = _FORMATS[source_name]
        destination_is_signed = destination_name.startswith("s")
        destination_bit_count = int(destination_name[1:])
...
def render_conversion_test_vectors(specification: Specification) -> str:
    ...
    lines.extend(
        ...
        for selector, source, expected in _integer_conversion_rows(specification)
    )
    ...
    lines.extend(
        ...
        for selector, source, expected in _float_truncate_rows(specification)
    )
    ...
    lines.extend(
        ...
        for selector, source, expected in _float_width_rows(specification)
    )
    ...
    lines.extend(
        ...
        for selector, source, expected in _integer_to_float_rows(specification)
    )
    ...
    lines.extend(
        ...
        for selector, source, failure, expected in _float_to_integer_rows(specification)
    )
    lines.extend(
        (
            "#else",

...
TEST(VMBytecodeInterpreterMemoryTest, RepeatsBitPatternsThroughTail) {
  constexpr uint64_t kPattern = UINT64_C(0x8877665544332211);
  constexpr std::array<uint8_t, 4> kPatternWidths = {1, 2, 4, 8};
  for (uint8_t width : kPatternWidths) {
    SCOPED_TRACE(unsigned(width));
    std::array<uint8_t, 19> storage = {};
    storage.fill(0xCD);
    iree_vm_bytecode_fill_pattern(storage.data() + 1, 17, kPattern, width);
    EXPECT_EQ(storage.front(), 0xCD);
    EXPECT_EQ(storage.back(), 0xCD);
    for (iree_host_size_t i = 0; i < 17; ++i) {
      EXPECT_EQ(storage[i + 1],
                static_cast<uint8_t>(kPattern >> ((i % width) * 8u)));
    }
  }
}

TEST(VMBytecodeInterpreterMemoryTest, LoadsEveryMemoryFormatUnaligned) {
  std::array<uint8_t, 66> storage = {};
  for (iree_host_size_t i = 0; i < 64; ++i) {
    storage[i + 1] = static_cast<uint8_t>(i * 17u + 3u);
  }

  for (uint8_t format = IREE_VM_BYTECODE_MEMORY_FORMAT_I8_X1;
       format <= IREE_VM_BYTECODE_MEMORY_FORMAT_I64_X8; ++format) {
    ...
    iree_vm_bytecode_memory_load_lanes(format, storage.data() + 1,
                                       values.data());
...
TEST(VMBytecodeInterpreterMemoryTest, StoresEveryMemoryFormatUnaligned) {
  ...
  for (uint8_t format = IREE_VM_BYTECODE_MEMORY_FORMAT_I8_X1;
    ...
    std::array<uint8_t, 66> storage = {};
    storage.fill(0xCD);
    iree_vm_bytecode_memory_store_lanes(format, values.data(),
                                        storage.data() + 1);
    for (uint8_t lane = 0; lane < lane_count; ++lane) {
      for (uint8_t byte = 0; byte < element_bytes; ++byte) {

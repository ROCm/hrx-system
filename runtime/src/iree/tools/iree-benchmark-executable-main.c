...
static iree_status_t iree_benchmark_executable_from_flags(
  ...
  // has the same ordering contract as normal HAL command buffer execution.
...
int main(int argc, char** argv) {
  ...
  iree_flags_set_usage(
      ...
      "by the executable. This tool records and submits HAL command buffers\n"
      "directly, bypassing higher-level program invocation and I/O setup.\n"
      "\n"
      "Example --flagfile:\n"
      "  --device=task\n"
      "  --executable_target_family=cpu\n"
      "  --executable_target_key=x86_64\n"
      "  "
      "--executable_file=runtime/src/iree/hal/drivers/task/executable/elf/"
      "testdata/"

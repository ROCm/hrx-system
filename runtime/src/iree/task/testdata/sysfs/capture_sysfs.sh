...
echo "    -DCMAKE_C_FLAGS=\"-DIREE_SYSFS_ROOT=\\\"$(realpath "${DEST}")\\\"\" \\"
echo "    -DCMAKE_CXX_FLAGS=\"-DIREE_SYSFS_ROOT=\\\"$(realpath "${DEST}")\\\"\""
echo "  cmake --build build/ --target iree_task_topology_test"
echo "  ctest --test-dir build/ -R iree_task_topology_test --output-on-failure"

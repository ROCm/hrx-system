...
// - HIP_TRACE_KERNEL_FULL_DUMP: Colon-separated list of kernel names for full buffer dumps
//===----------------------------------------------------------------------===//
...
static char* g_kernel_full_dump_list = NULL;  // Colon-separated list of kernel names
...
static const char* buffer_type_name(buffer_type_t type) {
  switch (type) {
    case BUFFER_TYPE_DEVICE: return "device";
    case BUFFER_TYPE_HOST: return "host";
    case BUFFER_TYPE_MANAGED: return "managed";
    default: return "unknown";
  }
}
...
static void kernel_table_init(void) {
  ...
  g_kernel_table.entries = (kernel_info_t*)calloc(
      g_kernel_table.capacity, sizeof(kernel_info_t));
}
...
static void dump_all_buffers_ex(const char* label, bool force_full_dump) {
  ...
  trace_msg(3, "=== Buffer Dump: %s (%zu buffers)%s ===", label, g_buffer_table.count,
            force_full_dump ? " [FULL DUMP]" : "");
  ...
  for (size_t i = 0; i < MAX_TRACKED_BUFFERS; ++i) {
    ...
    if (!force_full_dump && g_trace_dump_max > 0 && read_size > g_trace_dump_max) {
    ...
    if (buf->type == BUFFER_TYPE_DEVICE) {
      ...
      if (err != 0) {
        trace_msg(1, "  [alloc %lu] %s %p size=%zu - MEMCPY FAILED: %d",
                 buf->alloc_id, buffer_type_name(buf->type), buf->ptr, buf->size, err);
        continue;
      }
    ...
    if (g_trace_dump == 2 && !force_full_dump) {
      ...
      fprintf(g_trace_file, "  [alloc %lu] %s %p size=%zu hash=0x%016lx\n",
              buf->alloc_id, buffer_type_name(buf->type), buf->ptr, buf->size, hash);
    } else {
      // Full dump
...
static hipError_t wrap_hipMalloc(void** ptr, size_t size) {
  ...
  trace_msg(2, "hipMalloc(size=%zu) -> ptr=%p, ret=%d", size, ptr ? *ptr : NULL, err);
  return err;
}
...
static hipError_t wrap_hipHostMalloc(void** ptr, size_t size, unsigned int flags) {
  ...
  trace_msg(2, "hipHostMalloc(size=%zu, flags=0x%x) -> ptr=%p, ret=%d", 
...
static hipError_t wrap_hipMemcpy(void* dst, const void* src, size_t sizeBytes,
  ...
  trace_msg(2, "hipMemcpy(dst=%p, src=%p, size=%zu, kind=%s) -> %d",
...
static hipError_t wrap_hipMemcpyAsync(void* dst, const void* src, size_t sizeBytes,
  ...
  trace_msg(2, "hipMemcpyAsync(dst=%p, src=%p, size=%zu, kind=%s, stream=%p) -> %d",
...
static hipError_t wrap_hipMemset(void* dst, int value, size_t sizeBytes) {
  ...
  trace_msg(2, "hipMemset(dst=%p, value=0x%02x, size=%zu) -> %d", dst, value, sizeBytes, err);
  return err;
}
...
static hipError_t wrap_hipMemsetAsync(void* dst, int value, size_t sizeBytes,
  ...
  trace_msg(2, "hipMemsetAsync(dst=%p, value=0x%02x, size=%zu, stream=%p) -> %d",
...
static hipError_t wrap_hipGetDevice(int* deviceId) {
  ...
  trace_msg(2, "hipGetDevice() -> device=%d, ret=%d", deviceId ? *deviceId : -1, err);
  return err;
}

static hipError_t wrap_hipGetDeviceCount(int* count) {
  hipError_t err = g_real->hipGetDeviceCount(count);
  trace_msg(2, "hipGetDeviceCount() -> count=%d, ret=%d", count ? *count : -1, err);
  return err;
}
...
static hipError_t wrap_hipStreamCreate(hipStream_t* stream) {
  ...
  trace_msg(2, "hipStreamCreate() -> stream=%p, ret=%d", stream ? (void*)*stream : NULL, err);
  return err;
}
...
static hipError_t wrap_hipEventCreate(hipEvent_t* event) {
  ...
  trace_msg(2, "hipEventCreate() -> event=%p, ret=%d", event ? (void*)*event : NULL, err);
  return err;
}
...
static hipError_t wrap_hipEventCreateWithFlags(hipEvent_t* event, unsigned int flags) {
  ...
  trace_msg(2, "hipEventCreateWithFlags(flags=0x%x) -> event=%p, ret=%d",
...
static hipError_t wrap_hipEventRecord(hipEvent_t event, hipStream_t stream) {
  ...
  trace_msg(2, "hipEventRecord(event=%p, stream=%p) -> %d", (void*)event, (void*)stream, err);
  return err;
}
...
static bool should_full_dump_kernel(const char* kernel_name) {
  ...
  while (token) {
    ...
    if (strstr(kernel_name, token)) {
      found = true;
      trace_msg(3, "Full dump match: kernel='%s' matches token='%s'", kernel_name, token);
      break;
    }
...
static hipError_t wrap_hipModuleLaunchKernel(
  ...
  trace_msg(2, "hipModuleLaunchKernel(func=%p [%s], grid=(%u,%u,%u), block=(%u,%u,%u), "
            "shared=%u, stream=%p)",
            (void*)f, kernel_name ? kernel_name : "?", 
...
static hipError_t wrap_hipLaunchKernel(const void* function_address,
  ...
  trace_msg(2, "hipLaunchKernel(func=%p [%s], grid=(%u,%u,%u), block=(%u,%u,%u), "
            "shared=%zu, stream=%p)",
            function_address, kernel_name ? kernel_name : "?",
...
static void** wrap___hipRegisterFatBinary(const void* data) {
  ...
  trace_msg(2, "__hipRegisterFatBinary(data=%p) -> handle=%p", data, (void*)result);
  return result;
}

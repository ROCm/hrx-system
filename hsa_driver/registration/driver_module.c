...
IREE_FLAG_LIST(
    ...
    "Path to search for an appropriate libhsa-runtime64.so. If any \n"
...
IREE_FLAG(bool, hsa_allow_inline_execution, false,
          "Allow command buffers to execute inline when possible.");

IREE_FLAG(
    int32_t, hsa_tracing, 0,
...
static iree_status_t iree_hal_hsa_driver_parse_flags(
  ...
  for (iree_host_size_t i = 0; i < dylib_path_list.count; ++i) {
    IREE_RETURN_IF_ERROR(iree_string_pair_builder_add(
        builder, iree_make_string_pair(IREE_SV("hsa_dylib_path"),
                                       dylib_path_list.values[i])));
  }

  // bool and int flags
  IREE_RETURN_IF_ERROR(iree_string_pair_builder_add_int32(
      builder, IREE_SV("hsa_allow_inline_execution"),
      FLAG_hsa_allow_inline_execution));
  IREE_RETURN_IF_ERROR(iree_string_pair_builder_add_int32(
      builder, IREE_SV("hsa_tracing"), FLAG_hsa_tracing));
  IREE_RETURN_IF_ERROR(iree_string_pair_builder_add_int32(
      builder, IREE_SV("hsa_default_index"), FLAG_hsa_default_index));
    ...
    if (raw_dylib_path_env) {
      ...
      do {
        ...
        split_index = iree_string_view_split(dylib_path_env, ';', &value,
                                             &dylib_path_env);
        IREE_RETURN_IF_ERROR(iree_string_pair_builder_add(
            builder, iree_make_string_pair(IREE_SV("hsa_dylib_path"), value)));
      } while (split_index != -1);
    }
  }
...
static iree_status_t iree_hal_hsa_try_parse_bool_option(
  ...
  int32_t int_value = 0;
  IREE_RETURN_IF_ERROR(
      iree_hal_hsa_try_parse_int32_option(name, value, &int_value));
...
static iree_status_t iree_hal_hsa_driver_populate_options(
  ...
  for (iree_host_size_t i = 0; i < pairs_size; ++i) {
    ...
    if (iree_string_view_equal(key, IREE_SV("hsa_dylib_path"))) {
      ++dylib_path_count;
    } else if (iree_string_view_equal(key,
                                      IREE_SV("hsa_allow_inline_execution"))) {
      IREE_RETURN_IF_ERROR(iree_hal_hsa_try_parse_bool_option(
          key, value, &device_params->allow_inline_execution));
    } else if (iree_string_view_equal(key, IREE_SV("hsa_tracing"))) {
      IREE_RETURN_IF_ERROR(iree_hal_hsa_try_parse_int32_option(
          key, value, &device_params->stream_tracing));
    } else if (iree_string_view_equal(key, IREE_SV("hsa_default_index"))) {
      IREE_RETURN_IF_ERROR(iree_hal_hsa_try_parse_int32_option(
  ...
  if (dylib_path_count > 0) {
    ...
    for (iree_host_size_t i = 0; i < pairs_size; ++i) {
      ...
      iree_string_view_t value = pairs[i].value;
      if (iree_string_view_equal(key, IREE_SV("hsa_dylib_path"))) {
        driver_options->hsa_lib_search_paths
            [driver_options->hsa_lib_search_path_count++] = value;
      }
    }
  }
...
static iree_status_t iree_hal_hsa_driver_factory_try_create(
  ...
  iree_status_t status = iree_hal_hsa_driver_parse_flags(&flag_option_builder);

  // Parse key/values into option structs.
  if (iree_status_is_ok(status)) {
    status = iree_hal_hsa_driver_populate_options(
  ...
  if (iree_status_is_ok(status)) {
    status =
        iree_hal_hsa_driver_create(driver_name, &driver_options, &device_params,
                                   host_allocator, out_driver);
  }

  iree_allocator_free(host_allocator, driver_options.hsa_lib_search_paths);

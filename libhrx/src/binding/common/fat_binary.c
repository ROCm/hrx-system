...
static iree_status_t hrx_fat_extract_from_hipk(
    ...
    const iree_hal_streaming_fat_binary_target_t* targets,
    iree_hal_streaming_fat_binary_extract_t* extract) {
  // Project the ranked targets to their match strings; the kpack resolver does
  // its own ISA feature-subset matching.
  ...
  for (iree_host_size_t i = 0;
       ...
       ++i) {
    if (!iree_string_view_is_empty(targets[i].value)) {
      target_archs[target_arch_count++] = targets[i].value;
    }
  }
  if (target_arch_count == 0) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "no target architecture for HIPK out-of-band code object");
  }
  ...
  if (hrx_fat_is_elf(inner)) {
    return hrx_fat_extract_concatenated_elves(inner, extract);
  }

...
static hipError_t iree_hip_resolve_stream_context(
  ...
  *out_stream = NULL;
  ...
  if (result == hipSuccess) *out_context = (*out_stream)->context;
  return result;
}
...
static hipError_t iree_hip_enqueue_memory_fill(hipDeviceptr_t destination,
  ...
  if (result != hipSuccess) return result;
  iree_hal_streaming_context_t* context = stream_object->context;
...
HIPAPI hipError_t hipMemsetAsync(void* dst, int value, size_t sizeBytes,
  ...
  hipError_t result = iree_hip_enqueue_memory_fill(
      (hipDeviceptr_t)dst, sizeBytes, &value, 1, stream);
  IREE_TRACE_ZONE_END(z0);
  HIP_RETURN_ERROR(result);
}
...
HIPAPI hipError_t hipMemsetD8Async(hipDeviceptr_t dstDevice, unsigned char uc,
  ...
  hipError_t result =
      iree_hip_enqueue_memory_fill(dstDevice, N, &uc, 1, stream);
  IREE_TRACE_ZONE_END(z0);
  HIP_RETURN_ERROR(result);
}
...
HIPAPI hipError_t hipMemsetD16Async(hipDeviceptr_t dstDevice, unsigned short us,
  ...
  hipError_t result =
      iree_hip_enqueue_memory_fill(dstDevice, byte_count, &us, 2, stream);
  IREE_TRACE_ZONE_END(z0);
  HIP_RETURN_ERROR(result);
}
...
HIPAPI hipError_t hipMemsetD32Async(hipDeviceptr_t dstDevice, int i, size_t N,
  ...
  hipError_t result =
      iree_hip_enqueue_memory_fill(dstDevice, byte_count, &i, 4, stream);
  IREE_TRACE_ZONE_END(z0);
  HIP_RETURN_ERROR(result);
}
...
HIPAPI hipError_t hipStreamWaitEvent(hipStream_t stream, hipEvent_t event,
  ...
  hipError_t event_result =
      iree_hip_event_registry_lookup_retain(event, &event_object);
...
HIPAPI hipError_t hipExtLaunchKernel(const void* function_address,
  ...
  iree_hal_streaming_event_t* stop_event = NULL;
  if (stopEvent) {
    result = iree_hip_event_registry_lookup_retain(stopEvent, &stop_event);
...
HIPAPI hipError_t hipGraphLaunch(hipGraphExec_t graphExec, hipStream_t stream) {
  ...
  iree_status_t launch_status = iree_hal_streaming_graph_exec_launch(
      exec, stream_obj, &requires_host_completion);
  if (iree_status_is_ok(launch_status) && requires_host_completion) {
    launch_status = iree_hal_streaming_stream_synchronize(stream_obj);
  }
  iree_hal_streaming_stream_release(stream_obj);
  iree_hal_streaming_graph_exec_release(exec);

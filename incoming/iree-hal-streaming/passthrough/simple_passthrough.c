...
//
// This library simply loads the real HIP library with RTLD_GLOBAL and RTLD_DEEPBIND
// so its symbols are used for all lookups. This allows interception of init/shutdown
// while letting all other calls pass through transparently.
//===----------------------------------------------------------------------===//
...
static void passthrough_init(void) {
    ...
    real_hipGetDeviceCount = (hipGetDeviceCount_fn)dlsym(g_real_lib, "hipGetDeviceCount");
    real_hipSetDevice = (hipSetDevice_fn)dlsym(g_real_lib, "hipSetDevice");
    real_hipDeviceSynchronize = (hipDeviceSynchronize_fn)dlsym(g_real_lib, "hipDeviceSynchronize");
...
int hipMemcpy(void* dst, const void* src, size_t size, int kind) {
    ...
    if (g_log_file) {
        ...
        switch(kind) {
            case 0: kind_str = "H2H"; break;
            case 1: kind_str = "H2D"; break;
            case 2: kind_str = "D2H"; break;
            case 3: kind_str = "D2D"; break;
            case 4: kind_str = "Default"; break;
        }

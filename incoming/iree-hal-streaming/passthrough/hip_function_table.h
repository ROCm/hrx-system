...
typedef struct hipExternalSemaphoreSignalParams hipExternalSemaphoreSignalParams;
...
typedef hipError_t (*pfn_hipDeviceGetAttribute)(int* value, hipDeviceAttribute_t attr, int deviceId);
typedef hipError_t (*pfn_hipDeviceGetName)(char* name, int len, int deviceId);
typedef hipError_t (*pfn_hipDeviceGetPCIBusId)(char* pciBusId, int len, int device);
typedef hipError_t (*pfn_hipDeviceGetByPCIBusId)(int* device, const char* pciBusId);
...
typedef hipError_t (*pfn_hipDeviceGetSharedMemConfig)(hipSharedMemConfig* pConfig);
typedef hipError_t (*pfn_hipDeviceSetSharedMemConfig)(hipSharedMemConfig config);
typedef hipError_t (*pfn_hipDeviceGetStreamPriorityRange)(int* leastPriority, int* greatestPriority);
typedef hipError_t (*pfn_hipSetDeviceFlags)(unsigned int flags);
typedef hipError_t (*pfn_hipGetDeviceFlags)(unsigned int* flags);
typedef hipError_t (*pfn_hipDeviceCanAccessPeer)(int* canAccessPeer, int deviceId, int peerDeviceId);
typedef hipError_t (*pfn_hipDeviceEnablePeerAccess)(int peerDeviceId, unsigned int flags);
typedef hipError_t (*pfn_hipDeviceDisablePeerAccess)(int peerDeviceId);
typedef hipError_t (*pfn_hipChooseDevice)(int* device, const hipDeviceProp_t* prop);
typedef hipError_t (*pfn_hipDeviceGetP2PAttribute)(int* value, unsigned int attr, int srcDevice, int dstDevice);

// Memory Management
typedef hipError_t (*pfn_hipMalloc)(void** ptr, size_t size);
typedef hipError_t (*pfn_hipFree)(void* ptr);
typedef hipError_t (*pfn_hipHostMalloc)(void** ptr, size_t size, unsigned int flags);
typedef hipError_t (*pfn_hipHostFree)(void* ptr);
typedef hipError_t (*pfn_hipMemGetInfo)(size_t* free, size_t* total);

// Memory Copy
typedef hipError_t (*pfn_hipMemcpy)(void* dst, const void* src, size_t sizeBytes, hipMemcpyKind kind);
typedef hipError_t (*pfn_hipMemcpyAsync)(void* dst, const void* src, size_t sizeBytes, hipMemcpyKind kind, hipStream_t stream);
typedef hipError_t (*pfn_hipMemset)(void* dst, int value, size_t sizeBytes);
typedef hipError_t (*pfn_hipMemsetAsync)(void* dst, int value, size_t sizeBytes, hipStream_t stream);

// Stream Management
typedef hipError_t (*pfn_hipStreamCreate)(hipStream_t* stream);
typedef hipError_t (*pfn_hipStreamCreateWithFlags)(hipStream_t* stream, unsigned int flags);
...
typedef hipError_t (*pfn_hipStreamWaitEvent)(hipStream_t stream, hipEvent_t event, unsigned int flags);

// Event Management
typedef hipError_t (*pfn_hipEventCreate)(hipEvent_t* event);
typedef hipError_t (*pfn_hipEventCreateWithFlags)(hipEvent_t* event, unsigned int flags);
...
typedef hipError_t (*pfn_hipEventElapsedTime)(float* ms, hipEvent_t start, hipEvent_t stop);
...
typedef hipError_t (*pfn_hipModuleGetFunction)(hipFunction_t* function, hipModule_t module, const char* kname);
typedef hipError_t (*pfn_hipModuleGetGlobal)(hipDeviceptr_t* dptr, size_t* bytes, hipModule_t hmod, const char* name);
typedef hipError_t (*pfn_hipModuleLaunchKernel)(hipFunction_t f, unsigned int gridDimX, unsigned int gridDimY, unsigned int gridDimZ, unsigned int blockDimX, unsigned int blockDimY, unsigned int blockDimZ, unsigned int sharedMemBytes, hipStream_t stream, void** kernelParams, void** extra);
typedef hipError_t (*pfn_hipLaunchKernel)(const void* function_address, dim3 numBlocks, dim3 dimBlocks, void** args, size_t sharedMemBytes, hipStream_t stream);
typedef hipError_t (*pfn_hipExtModuleLaunchKernel)(hipFunction_t f, unsigned int globalWorkSizeX, unsigned int globalWorkSizeY, unsigned int globalWorkSizeZ, unsigned int localWorkSizeX, unsigned int localWorkSizeY, unsigned int localWorkSizeZ, size_t sharedMemBytes, hipStream_t stream, void** kernelParams, void** extra, hipEvent_t startEvent, hipEvent_t stopEvent, unsigned int flags);
...
typedef void (*pfn___hipRegisterFunction)(void** fatCubinHandle, const char* hostFun, char* deviceFun, const char* deviceName, int thread_limit, void* tid, void* bid, dim3* blockDim, dim3* gridDim, int* wSize);
typedef void (*pfn___hipRegisterVar)(void** fatCubinHandle, char* hostVar, char* deviceAddress, const char* deviceName, int ext, size_t size, int constant, int global);

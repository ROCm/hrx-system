...
// CPU test kernels for benchmarking graph execution with real parameter
// metadata
...
static int
nop_kernel(const iree_hal_executable_environment_v0_t *environment,
...
// Parameters: a_ptr (2 constants), b_ptr (2 constants), c_ptr (2 constants), n
// (1 constant)
...
static int many_params_hal(
  ...
  for (uint32_t i = start_idx; i < end_idx; i++) {
    ...
    float sum = buf0[idx] * weight0 + buf1[idx] * weight1 +
                buf2[idx] * weight2 + buf3[idx] * weight3 + buf4[idx] * weight4;
    if (flags & 1)
      sum = -sum;
    if (mode == 1)
      sum = sum * sum;
...
static int
memory_copy(const iree_hal_executable_environment_v0_t *environment,
            const iree_hal_executable_dispatch_state_v0_t *dispatch_state,
            const iree_hal_executable_workgroup_state_v0_t *workgroup_state) {
...
static const iree_hal_executable_dispatch_v0_t entry_points[] = {
    nop_kernel,      simple_add_raw, simple_add_hal,    many_params_raw,
    many_params_hal, memory_copy,    compute_intensive,
};
...
static const iree_hal_executable_dispatch_parameter_v0_t *entry_params[] = {
    nop_kernel_params,        simple_add_raw_params,  simple_add_hal_params,
    many_params_raw_params,   many_params_hal_params, memory_copy_params,
    compute_intensive_params,
};
...
static const iree_hal_executable_dispatch_attrs_v0_t entry_attrs[] = {
    ...
    },
    {
        ...
        .workgroup_size_z = 0,
        .parameter_count =
            sizeof(simple_add_raw_params) / sizeof(simple_add_raw_params[0]),
    },
    {
        // simple_add_hal
        .flags = IREE_HAL_EXECUTABLE_DISPATCH_FLAG_V0_NONE,
        ...
        .workgroup_size_z = 0,
        .parameter_count =
            sizeof(simple_add_hal_params) / sizeof(simple_add_hal_params[0]),
    },
    {
        // many_params_raw
        .flags = IREE_HAL_EXECUTABLE_DISPATCH_FLAG_V0_NONE,
        ...
        .workgroup_size_z = 0,
        .parameter_count =
            sizeof(many_params_raw_params) / sizeof(many_params_raw_params[0]),
    },
    {
        // many_params_hal
        .flags = IREE_HAL_EXECUTABLE_DISPATCH_FLAG_V0_NONE,
        ...
        .workgroup_size_z = 0,
        .parameter_count =
            sizeof(many_params_hal_params) / sizeof(many_params_hal_params[0]),
    },
    {
        // memory_copy
        .flags = IREE_HAL_EXECUTABLE_DISPATCH_FLAG_V0_NONE,
        ...
        .workgroup_size_z = 0,
        .parameter_count =
            sizeof(memory_copy_params) / sizeof(memory_copy_params[0]),
    },
    {
        // compute_intensive
        .flags = IREE_HAL_EXECUTABLE_DISPATCH_FLAG_V0_NONE,
        ...
        .workgroup_size_y = 0,
        .workgroup_size_z = 0,
        .parameter_count = sizeof(compute_intensive_params) /
                           sizeof(compute_intensive_params[0]),
    },
};
...
static const char *entry_point_tags[] = {
    "nop",      "simple_raw", "simple_hal", "many_raw",
    "many_hal", "memcpy",     "compute",
};

...
iree_status_t
iree_hal_streaming_test_module_load(iree_hal_streaming_context_t *context,
...
iree_status_t
iree_hal_streaming_test_module_verify(iree_hal_streaming_module_t *module) {
...
iree_status_t iree_hal_streaming_graph_add_test_node(
  ...
  case IREE_HAL_STREAMING_GRAPH_NODE_TYPE_EMPTY:
    return iree_hal_streaming_graph_add_empty_node(graph, dependencies,
                                                   dependency_count, out_node);
...
iree_status_t iree_hal_streaming_graph_generate_linear_sequence(
  ...
  if (out_first_node)
    *out_first_node = first_node;
  if (out_last_node)
    *out_last_node = prev_node;
  return iree_ok_status();
}
...
iree_status_t iree_hal_streaming_graph_generate_mixed(
  ...
  if (out_node_count)
    *out_node_count = params->node_count;
  return iree_ok_status();
}

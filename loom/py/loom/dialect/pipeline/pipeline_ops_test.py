...
pipeline.def<kernel> public target(@array) @split_k() launch(%lhs: buffer, %rhs: buffer, %bias: buffer, %output: buffer) {
  ...
  %lhs_view = buffer.view %lhs[%base] : buffer -> view<2x4x8x8xi8>
  %rhs_view = buffer.view %rhs[%base] : buffer -> view<2x4x8x8xi8>
  %bias_view = buffer.view %bias[%base] : buffer -> view<8x8xi32>
  %output_view = buffer.view %output[%base] : buffer -> view<8x8xi32>
  %lhs_tiles = pipeline.scatter %lhs_view across %products : view<2x4x8x8xi8>, group -> pipeline.flow<tile<8x8xi8>>
  %rhs_tiles = pipeline.scatter %rhs_view across %products : view<2x4x8x8xi8>, group -> pipeline.flow<tile<8x8xi8>>
  ...
  %folded = pipeline.fold<addi> %partials : pipeline.flow<tile<8x8xi32>>

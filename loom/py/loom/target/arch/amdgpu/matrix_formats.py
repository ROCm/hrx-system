...
class Gfx125xMatrixPhysicalFormat:
    """One physical operand-width family in the GFX12.5 f8/f6/f4 ISA."""
    ...
    def register_count_for(self, element_count: int) -> int:
        ...
        if payload_bit_count % 32 != 0:
            raise ValueError(
                f"GFX12.5 matrix format '{self.token}' payload occupies "
                f"{payload_bit_count} bits, not whole 32-bit registers"
            )
        return payload_bit_count // 32


GFX125X_MATRIX_PHYSICAL_FORMATS = (
    Gfx125xMatrixPhysicalFormat(
        token="f8",

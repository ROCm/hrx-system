...
# ====
# ERROR@+1: "call `tl.sync_grid` requires cooperative-grid launch residency and a Loom grid synchronization contract"
@tilelang_case(
    name="sync_grid_requires_cooperative_grid",
    category="diagnostic",
    tags=("topology", "grid", "barrier"),
)
def sync_grid_requires_cooperative_grid(tir: Any) -> TileLangImportInput:
    dst = tir.Var("dst", "handle")
    dst_buffer = tir.decl_buffer((1,), "int32", name="dst")
    body = tir.Evaluate(tir.call_intrin("handle", tir.op.Op.get("tl.sync_grid")))
    prim_func = tir.PrimFunc(
        [dst],
        body,
        buffer_map={dst: dst_buffer},
...
# ====
# ERROR@+1: "tl.tileop.gemm CDNA FP8 bridge requires k_pack = 2"
@tilelang_case(
    name="tileop_gemm_cdna_fp8_requires_k_pack_2",
    category="diagnostic",
    tags=("tileop", "gemm", "matrix", "fp8", "target"),
)
def tileop_gemm_cdna_fp8_requires_k_pack_2(T: Any) -> TileLangImportInput:
    @T.prim_func  # type: ignore[untyped-decorator]
    def tileop_gemm_cdna_fp8_requires_k_pack_2_kernel(
    ...
    return TileLangImportInput(
        source=tileop_gemm_cdna_fp8_requires_k_pack_2_kernel,
        target="hip -mcpu=gfx942",
        name="tileop_gemm_cdna_fp8_requires_k_pack_2_kernel",
    )
...
def gfx942_warp_reduce_width_mismatch(tir: Any, tvm: Any) -> TileLangImportInput:
    ...
    body = tir.AttrStmt(
        ...
        tir.Evaluate(
            tir.call_intrin(
                "float32",
                tir.op.Op.get("tl.warp_reduce_sum"),
                tir.BufferLoad(src_buffer, [thread_index.var]),
            )
        ),
    )

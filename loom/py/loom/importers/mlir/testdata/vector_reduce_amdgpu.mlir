...
kernel.def target(@rocm_hsaco_fb) export("vector_reduce_amdgpu") @vector_reduce_amdgpu() {
...
} launch(%binding0: buffer, %binding1: buffer) {
  %c0 = index.constant 0 : index
  %c1 = index.constant 1 : index
  %c4 = index.constant 4 : index
  %zero = scalar.constant 0.0 : f32
  %c0_bytes = index.cast %c0 : index to offset
  %input = buffer.view %binding0[%c0_bytes] : buffer -> view<4xf32, #encoding.layout.dense>
  %c0_bytes_2 = index.cast %c0 : index to offset
  %output = buffer.view %binding1[%c0_bytes_2] : buffer -> view<4xf32, #encoding.layout.dense>
  %vec = vector.load %input[%c0] : view<4xf32, #encoding.layout.dense> -> vector<4xf32>
  ...
  vector.store %broadcast, %output[%c0] : vector<4xf32>, view<4xf32, #encoding.layout.dense>
  kernel.return
}
...
kernel.def target(@rocm_hsaco_fb) export("barrier_smoke") @barrier_smoke() {
  %wg_count_x = index.constant 1 : index
  %wg_count_y = index.constant 1 : index
  %wg_count_z = index.constant 1 : index
  %wg_size_x = index.constant 1 : index

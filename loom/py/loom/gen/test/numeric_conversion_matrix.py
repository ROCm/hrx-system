...
def _emit_encode_pair(row: Fp8Format, source_format: _BinaryFormat) -> str:
    ...
    lines = [
        ...
        f"  %output_view = buffer.view %output[%base] : buffer -> view<{exact_width}xi8, #dense>",
        ...
        *_emit_exact_vector(
        ...
        f"  vector.store %difference_head, %output_view[%head_offset] : vector<8xi8>, view<{exact_width}xi8, #dense>",
        *_emit_exact_vector(
            tail,
            source_format.integer_type_spelling,
            source_format.bit_count,
        ...
        f"  vector.store %difference_tail, %output_view[%tail_offset] : vector<{len(tail)}xi8>, view<{exact_width}xi8, #dense>",
        "  kernel.return",
        "}",
        "",
        ...
        f"  %input_view = buffer.view %input_noalias[%base] : buffer -> view<[%bounded_row_count]x8x{source_name}, #dense>",
        "  %output_view = buffer.view %output_noalias[%base] : buffer -> view<[%bounded_row_count]x8xi8, #dense>",
        f"  %schema = encoding.define #matrix_operand<element_format={format_name}, payload_elements=8, payload_packing=dense_lanes, payload_registers=0> : encoding<schema>",
        "  scf.if %in_bounds {",
        f"    %source = vector.load %input_view[%row, %column] : view<[%bounded_row_count]x8x{source_name}, #dense> -> vector<8x{source_name}>",
        ...
        "    vector.store %encoded_bits, %output_view[%row, %column] : vector<8xi8>, view<[%bounded_row_count]x8xi8, #dense>",
        "  }",
        "  kernel.return",
        "}",
        "",
...
def _emit_decode_pair(row: Fp8Format, result_format: _BinaryFormat) -> str:
    ...
    lines = [
        ...
        *_emit_exact_vector(
        ...
        "  %in_bounds = index.cmp ult, %row, %bounded_row_count : index",
        "  %input_noalias, %output_noalias = buffer.assume.noalias %input, %output : buffer, buffer",
        "  %input_view = buffer.view %input_noalias[%base] : buffer -> view<[%bounded_row_count]x8xi8, #dense>",
        f"  %output_view = buffer.view %output_noalias[%base] : buffer -> view<[%bounded_row_count]x8x{result_name}, #dense>",
        f"  %schema = encoding.define #matrix_operand<element_format={format_name}, payload_elements=8, payload_packing=dense_lanes, payload_registers=0> : encoding<schema>",
        "  scf.if %in_bounds {",
        "    %source_bits = vector.load %input_view[%row, %column] : view<[%bounded_row_count]x8xi8, #dense> -> vector<8xi8>",
        ...
        f"    vector.store %decoded, %output_view[%row, %column] : vector<8x{result_name}>, view<[%bounded_row_count]x8x{result_name}, #dense>",
        "  }",
        "  kernel.return",
        "}",
        "",

...
def _test_vm_binary_emits_vmfb_product(name, **kwargs):
    analysis_test(
        name = name,
        impl = _test_vm_binary_emits_vmfb_product_impl,
        target = "//loom/build_tools/bazel/test/testdata/vm_binary:deps_subject",
        **kwargs
    )

def _test_vm_binary_emits_vmfb_product_impl(env, target):
    binary = target[LoomBinaryInfo]
    env.expect.that_str(binary.kind).equals("vm")
    env.expect.that_str(binary.primary_artifact.basename).equals(
        "deps_subject.vmfb",
    )
    env.expect.that_str(binary.linked_module.basename).equals(
    ...
    if binary.target_profiles:
        env.fail(
            "authored VM targets must not report configured target profiles: %r" %
            binary.target_profiles,
        )

    default_files = target[DefaultInfo].files.to_list()
    if len(default_files) != 1:
        env.fail("expected one default VM artifact, got %r" % default_files)
    _expect_basename(env, default_files, "deps_subject.vmfb")
    ...
    compile_action = _find_action(env, actions, "LoomVmBinary")
    for expected_arg in [
        "--backend=vm",
        "--module-name=deps_subject",
        "--compile-report=details",
    ]:
        if expected_arg not in compile_action.argv:
            env.fail(
                "expected %r in VM compile arguments %r" %
                (expected_arg, compile_action.argv),
            )
    _expect_arg_with_suffix(
        env,
    ...
    _expect_basename(
        env,
        target[OutputGroupInfo].compile_reports.to_list(),
        "deps_subject.compile.json",
    )

def _test_vm_binary_sources_are_an_implicit_library(name, **kwargs):
    analysis_test(
        name = name,
        impl = _test_vm_binary_sources_are_an_implicit_library_impl,
        target = "//loom/build_tools/bazel/test/testdata/vm_binary:mixed_subject",
        **kwargs
    )

def _test_vm_binary_sources_are_an_implicit_library_impl(env, target):
    actions = target[TestingAspectInfo].actions
    source_action = _find_action(env, actions, "LoomBinarySources")
    _expect_arg_with_suffix(env, source_action.argv, "", "interface.loom")
    _expect_arg_with_suffix(
    ...
    compile_action = _find_action(env, actions, "LoomVmBinary")
    if "--module-name=mixed_runtime" not in compile_action.argv:
        env.fail(
            "expected explicit VM module name in %r" % compile_action.argv,
        )

def _test_vm_binary_explicit_roots_replace_exports(name, **kwargs):
    analysis_test(
        name = name,
        impl = _test_vm_binary_explicit_roots_replace_exports_impl,
        target = "//loom/build_tools/bazel/test/testdata/vm_binary:explicit_subject",
        **kwargs
    )

def _test_vm_binary_explicit_roots_replace_exports_impl(env, target):
    link_action = _find_action(
        env,
        target[TestingAspectInfo].actions,
        "LoomBinaryLink",
    )
    if "--root=@double_once" not in link_action.argv:
        env.fail("expected explicit VM root in %r" % link_action.argv)
    _expect_arg_with_suffix(
        env,
        link_action.argv,
        "--library=",
...
def loom_binary_rules_test_suite(name):
    test_suite(
        ...
        tests = [
            ...
            _test_vm_binary_explicit_roots_replace_exports,
            _test_vm_binary_sources_are_an_implicit_library,
        ],
    )

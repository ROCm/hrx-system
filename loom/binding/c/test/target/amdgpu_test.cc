...
kernel.def target(@gfx11_generic) @configured_store() {
  ...
  %view = buffer.view %global[%zero_offset] : buffer -> view<1xi32, #encoding.layout.dense>
  view.store %value, %view[%zero_index] : i32, view<1xi32, #encoding.layout.dense>
  kernel.return
}
)");
  ModulePtr module =
...
kernel.def @wave32_root() {
  ...
  %global = buffer.assume.memory_space<global> %output : buffer
  %view = buffer.view %global[%zero_offset] : buffer -> view<1xi32, #encoding.layout.dense>
  view.store %size_i32, %view[%zero_index] : i32, view<1xi32, #encoding.layout.dense>
  kernel.return
}

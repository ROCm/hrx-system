...
// Target-informed scf.for loop-invariant placement policy.
typedef enum loom_scf_for_residency_policy_e {
  LOOM_SCF_FOR_RESIDENCY_POLICY_PRESERVE = 1,
  LOOM_SCF_FOR_RESIDENCY_POLICY_COUNT_ = 2,
} loom_scf_for_residency_policy_t;
...
LOOM_DEFINE_SEGMENTED_OPTIONAL_OPERAND(loom_scf_for_residency_minimum, 5)
...
LOOM_DEFINE_ATTR_ENUM_TYPED(loom_scf_for_residency_policy, 2, loom_scf_for_residency_policy_t)
...
enum loom_scf_for_build_flag_bits_e {
  ...
  LOOM_SCF_FOR_BUILD_FLAG_HAS_RESIDENCY_MINIMUM = 1u << 3,
  LOOM_SCF_FOR_BUILD_FLAG_HAS_RESIDENCY_POLICY = 1u << 4,
};
...
iree_status_t loom_scf_for_build(
    ...
    loom_optional loom_may_consume loom_value_id_t residency_minimum,
    loom_optional uint8_t residency_policy,

...
TEST_F(RepresentationPlanTest, TracksConstrainedComponentsAcrossUnions) {
  ...
  IREE_ASSERT_OK(loom_low_representation_plan_constrain(
      &plan, 1, candidates, IREE_ARRAYSIZE(candidates)));
...
TEST_F(RepresentationPlanTest, IntersectsDomainsAcrossUnionedValues) {
  ...
  IREE_ASSERT_OK(loom_low_representation_plan_constrain(
      &plan, 0, producer_candidates, IREE_ARRAYSIZE(producer_candidates)));
  IREE_ASSERT_OK(loom_low_representation_plan_constrain(
      &plan, 1, consumer_candidates, IREE_ARRAYSIZE(consumer_candidates)));
  IREE_ASSERT_OK(loom_low_representation_plan_union(&plan, 0, 1));
  EXPECT_TRUE(loom_low_representation_plan_solve(&plan, nullptr));
...
TEST_F(RepresentationPlanTest, SumsComponentRuntimeCosts) {
  ...
  IREE_ASSERT_OK(loom_low_representation_plan_constrain(
      &plan, 0, first_candidates, IREE_ARRAYSIZE(first_candidates)));
  IREE_ASSERT_OK(loom_low_representation_plan_constrain(
      &plan, 2, second_candidates, IREE_ARRAYSIZE(second_candidates)));
...
TEST_F(RepresentationPlanTest, ReportsConflictingComponent) {
  auto plan = MakePlan(3);
  const loom_low_representation_candidate_t first_candidates[] = {
  ...
  IREE_ASSERT_OK(loom_low_representation_plan_constrain(
      &plan, 0, first_candidates, IREE_ARRAYSIZE(first_candidates)));
  IREE_ASSERT_OK(loom_low_representation_plan_constrain(
      &plan, 1, narrowing_candidates, IREE_ARRAYSIZE(narrowing_candidates)));
  IREE_ASSERT_OK(loom_low_representation_plan_constrain(
      &plan, 2, emptying_candidates, IREE_ARRAYSIZE(emptying_candidates)));
  IREE_ASSERT_OK(loom_low_representation_plan_union(&plan, 0, 1));
  ...
  EXPECT_EQ(conflict.value_ordinal, 0u);
}
...
TEST_F(RepresentationPlanTest, KeepsIndependentComponentsIndependent) {
  ...
  const loom_low_representation_candidate_t right_candidates[] = {
      Candidate(20, 0)};
  IREE_ASSERT_OK(loom_low_representation_plan_constrain(
      &plan, 0, left_candidates, IREE_ARRAYSIZE(left_candidates)));
  IREE_ASSERT_OK(loom_low_representation_plan_constrain(
      &plan, 3, right_candidates, IREE_ARRAYSIZE(right_candidates)));
  EXPECT_TRUE(loom_low_representation_plan_solve(&plan, nullptr));

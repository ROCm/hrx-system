...
static iree_status_t loom_low_lower_memory_report_row_list_append(
  ...
  if (list->count == list->capacity) {
    iree_host_size_t minimum_capacity =
        LOOM_LOW_LOWER_MEMORY_REPORT_ROW_VEC_DEFAULT_BYTE_LENGTH / sizeof(*row);
...
static bool loom_low_lower_memory_report_mul_u64(uint64_t lhs, uint64_t rhs,
                                                 uint64_t* out_result) {
...
static bool loom_low_lower_memory_expr_keys_equal(
    const loom_low_lower_memory_expr_key_t* left,
    const loom_low_lower_memory_expr_key_t* right) {
...
static bool loom_low_lower_memory_expr_key_append_term(
    loom_low_lower_memory_expr_key_t* key, loom_value_id_t value_id,
    int64_t coefficient) {
  if (coefficient == 0) {
    return true;
  }
  ...
  key->terms[insert_index] = (loom_low_lower_memory_expr_term_t){
      .value_id = value_id,
...
static bool loom_low_lower_memory_expr_key_from_source_plan(
    const loom_low_source_memory_access_plan_t* source_plan,
    int64_t lane_offset, loom_low_lower_memory_expr_key_t* out_key) {
  *out_key = (loom_low_lower_memory_expr_key_t){0};
  ...
  for (uint8_t i = 0; i < source_plan->dynamic_term_count; ++i) {
    ...
    if (term->index == LOOM_VALUE_ID_INVALID || term->stride_value_count != 0 ||
        !loom_low_lower_memory_expr_key_append_term(out_key, term->index,
                                                    term->byte_stride)) {
      return false;
    }
  }
...
static iree_status_t loom_low_lower_memory_expr_ensure_capacity(
    loom_low_lower_context_t* context, iree_host_size_t minimum_capacity) {
  if (minimum_capacity <= context->lowering.memory_expr_entry_capacity) {
    return iree_ok_status();
  }
  ...
  context->lowering.memory_expr_entries =
      (loom_low_lower_memory_expr_entry_t*)entries;
  return iree_ok_status();
}

static iree_status_t loom_low_lower_memory_expr_intern(
    loom_low_lower_context_t* context,
    const loom_low_lower_memory_expr_key_t* key,
    loom_low_memory_expr_id_t* out_expr_id) {
  *out_expr_id = LOOM_LOW_MEMORY_EXPR_ID_NONE;
  for (iree_host_size_t i = 0; i < context->lowering.memory_expr_entry_count;
       ++i) {
    if (loom_low_lower_memory_expr_keys_equal(
            &context->lowering.memory_expr_entries[i].key, key)) {
      *out_expr_id = (loom_low_memory_expr_id_t)i;
      return iree_ok_status();
    }
  }
  if (context->lowering.memory_expr_entry_count >=
      (iree_host_size_t)LOOM_LOW_MEMORY_EXPR_ID_NONE) {
    return iree_make_status(IREE_STATUS_RESOURCE_EXHAUSTED,
                            "too many source-memory report expressions");
  }
  IREE_RETURN_IF_ERROR(loom_low_lower_memory_expr_ensure_capacity(
      context, context->lowering.memory_expr_entry_count + 1));
  const iree_host_size_t entry_index =
      context->lowering.memory_expr_entry_count++;
  context->lowering.memory_expr_entries[entry_index] =
      (loom_low_lower_memory_expr_entry_t){.key = *key};
  *out_expr_id = (loom_low_memory_expr_id_t)entry_index;
  return iree_ok_status();
}
...
iree_status_t loom_low_lower_memory_report_row_populate_source_interval(
  ...
  loom_low_lower_memory_expr_key_t begin_key = {0};
  loom_low_lower_memory_expr_key_t end_key = {0};
  if (!loom_low_lower_memory_expr_key_from_source_plan(
          source_plan, lane_begin_offset, &begin_key) ||
      !loom_low_lower_memory_expr_key_from_source_plan(
          source_plan, lane_end_offset, &end_key)) {
    return iree_ok_status();
  }

  loom_low_memory_expr_id_t begin_expr_id = LOOM_LOW_MEMORY_EXPR_ID_NONE;
  loom_low_memory_expr_id_t end_expr_id = LOOM_LOW_MEMORY_EXPR_ID_NONE;
  IREE_RETURN_IF_ERROR(
      loom_low_lower_memory_expr_intern(context, &begin_key, &begin_expr_id));
  IREE_RETURN_IF_ERROR(
      loom_low_lower_memory_expr_intern(context, &end_key, &end_expr_id));
  row->source_interval.begin_expr_id = begin_expr_id;
  row->source_interval.end_expr_id = end_expr_id;
...
static bool loom_low_lower_memory_report_exact_trip_count(
...
static bool loom_low_lower_memory_report_value_exact_i64(
...
static loom_value_id_t loom_low_lower_memory_report_identity_value(
    const loom_low_lower_context_t* context, loom_value_id_t value_id) {
...
static bool loom_low_lower_memory_report_block_branch_to(
...
static bool loom_low_lower_memory_report_block_branch_edge_to(
  ...
  if (!loom_low_lower_memory_report_block_branch_to(source_block, target_block,
                                                    &branch_op)) {
    return false;
  }
...
static bool loom_low_lower_memory_report_branch_arg(
...
static bool loom_low_lower_memory_report_add_step(
    const loom_low_lower_context_t* context, loom_value_id_t value_id,
  ...
  const loom_value_id_t lhs = loom_low_lower_memory_report_identity_value(
      context, loom_index_add_lhs(defining_op));
  const loom_value_id_t rhs = loom_low_lower_memory_report_identity_value(
      context, loom_index_add_rhs(defining_op));
  if (lhs == iv_id) {
    return loom_low_lower_memory_report_value_exact_i64(context, rhs, out_step);
  }
  if (rhs == iv_id) {
    return loom_low_lower_memory_report_value_exact_i64(context, lhs, out_step);
  }
  return false;
}

static bool loom_low_lower_memory_report_header_upper_bound(
  ...
  *out_inclusive_upper_bound = false;
  ...
  if (!loom_low_lower_memory_report_value_exact_i64(
...
static bool loom_low_lower_memory_report_compute_trip_count(
...
static bool loom_low_lower_memory_report_try_counted_cfg_loop(
    ...
    uint8_t* counted_backedge_edges) {
  ...
  const loom_block_t* true_dest = loom_cfg_cond_br_true_dest(header->last_op);
  const loom_block_t* false_dest = loom_cfg_cond_br_false_dest(header->last_op);
  ...
  if (loom_low_lower_memory_report_block_branch_edge_to(
  ...
  bool step_ok = loom_low_lower_memory_report_add_step(
      context, body_backedge_arg, iv_id, &step);
  ...
  if (!loom_low_lower_memory_report_multiply_block(
      ...
      !loom_low_lower_memory_report_multiply_block(block_execution_counts,
...
static iree_status_t loom_low_lower_source_op_execution_count_plus_one(
  ...
  for (const loom_op_t* parent = source_op ? source_op->parent_op : NULL;
    ...
    uint64_t trip_count = 0;
    if (!loom_low_lower_memory_report_exact_trip_count(
            context->lowering.fact_table, loop, &trip_count) ||
        !loom_low_lower_memory_report_mul_u64(execution_count, trip_count,
                                              &execution_count) ||
  ...
  if (body != NULL && function_block != NULL &&
    ...
    IREE_RETURN_IF_ERROR(
        loom_low_lower_memory_report_ensure_source_block_counts(context));
    if (!context->lowering.source_block_execution_counts_exact ||
        block_index >= body->block_count ||
        !loom_low_lower_memory_report_mul_u64(
            execution_count,
            context->lowering.source_block_execution_counts[block_index],
            &execution_count)) {
      *out_execution_count_plus_one =
          LOOM_LOW_LOWER_MEMORY_REPORT_EXECUTION_COUNT_PLUS_ONE_UNKNOWN;
      return iree_ok_status();

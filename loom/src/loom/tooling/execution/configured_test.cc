...
TEST(ConfiguredExecutionTest, ReturnsConfiguredProviderSet) {
  const loom_run_execution_provider_set_t* providers =
  ...
  ASSERT_EQ(providers->provider_count, expected_provider_count);
  EXPECT_EQ(providers->providers == nullptr, expected_provider_count == 0);

  loom_run_execution_environment_t environment = {};
  IREE_ASSERT_OK(
      loom_run_execution_environment_initialize(providers, &environment));
  ...
  for (iree_host_size_t i = 0; i < expected_provider_count; ++i) {
    ...
    EXPECT_EQ(execution_provider->device_provider, device_provider);

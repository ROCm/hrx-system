...
// `rhs_transposed = true` invocation attr instead accepts rhs [N, K] without
// changing the mathematical result. The returned result is a rank-2 buffer
// view [M, N] with the numeric kind selected by the result contract.
// Floating-point inputs default to f64 reference accumulation and f32 results.
// Integer input storage may be interpreted into a floating accumulator/result
// by providing explicit {lhs, rhs, accumulator, result} attrs. |options| is

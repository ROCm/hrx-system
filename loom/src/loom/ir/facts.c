...
static bool loom_value_facts_find_alias_ordinal(const loom_value_id_t* values,
...
void loom_value_facts_apply_alias_predicates(
  ...
  // Relational predicates constrain both aliases from the known interval of
  // their counterpart. The predicates remain available to symbolic relation
  // analysis; these interval refinements let ordinary arithmetic and target
  // legality consume the same contract.
  for (uint16_t i = 0; i < predicate_count; ++i) {
    const loom_predicate_t* predicate = &predicates[i];

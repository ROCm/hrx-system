...
typedef struct loom_bytecode_body_reader_t {
  ...
  // Function-local SSA value namespace and type-binding state.
  loom_bytecode_value_scope_t values;
  ...
  // Actual body counts used to verify the encoded allocation summary.
  loom_bytecode_body_counts_t counts;
} loom_bytecode_body_reader_t;

static iree_status_t loom_bytecode_body_validate_string_ref(
...
static iree_status_t loom_bytecode_body_validate_type_ref(
...
static iree_status_t loom_bytecode_body_validate_location_ref(
    loom_bytecode_reader_decoder_t* decoder,
    const loom_bytecode_reader_module_view_t* module_view, uint64_t location_id,
    uint64_t offset) {
  if (location_id >= module_view->locations.count) {
    return loom_bytecode_reader_emit_table_ref(
        decoder, IREE_SV("LOCATIONS"), location_id,
        module_view->locations.count, offset);
  }
  return iree_ok_status();
}

static iree_status_t loom_bytecode_body_resolve_op_ref(
    loom_bytecode_reader_decoder_t* decoder,
...
static iree_status_t loom_bytecode_value_scope_lookup_value(
    loom_bytecode_value_scope_t* value_scope, uint64_t value_number,
...
iree_status_t loom_bytecode_value_scope_materialize_definition(
  ...
  if (name_id != 0) {
    ...
    IREE_RETURN_IF_ERROR(loom_bytecode_body_validate_string_ref(
        value_scope->decoder, value_scope->module_view, name_id,
        IREE_SV("value_name"), name_offset, &unused_name));
  }
  uint64_t type_offset = name_offset;
  IREE_RETURN_IF_ERROR(loom_bytecode_body_validate_type_ref(
      value_scope->decoder, type_id, value_scope->output_module->types.count,
      type_offset));
  loom_type_t base_type = value_scope->output_module->types.entries[type_id];

  loom_type_t type = {0};
  loom_type_id_t canonical_type_id = LOOM_TYPE_ID_INVALID;
  IREE_RETURN_IF_ERROR(loom_bytecode_value_scope_bind_type(
      value_scope, cursor, base_type, (loom_type_id_t)type_id,
...
static iree_status_t loom_bytecode_value_scope_prepare_fresh_values(
    loom_bytecode_value_scope_t* value_scope, iree_host_size_t count) {
...
static iree_status_t loom_bytecode_value_scope_reserve_definitions(
    loom_bytecode_value_scope_t* value_scope, uint64_t count,
...
static iree_status_t loom_bytecode_body_reader_read_op(
  ...
  IREE_RETURN_IF_ERROR(loom_bytecode_body_resolve_op_ref(
      body_reader->values.decoder, body_reader->values.module_view,
      op_table_index_plus1, op_offset, &vtable));
  loom_op_kind_t op_kind =
      body_reader->values.module_view->ops.kinds[op_table_index_plus1 - 1];
  ...
  if (location_id != 0) {
    IREE_RETURN_IF_ERROR(loom_bytecode_body_validate_location_ref(
        body_reader->values.decoder, body_reader->values.module_view,
        location_id, loom_bytecode_reader_cursor_absolute_position(cursor)));
  }
  ...
  for (uint64_t i = 0; i < present_attr_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(loom_bytecode_body_validate_string_ref(
        body_reader->values.decoder, body_reader->values.module_view, key_id,
        IREE_SV("attribute_key"), key_offset, &key));
    ...
    if (descriptor->attr_kind == LOOM_ATTR_SCOPED_ENUM) {
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_body_validate_string_ref(
          body_reader->values.decoder, body_reader->values.module_view,
          descriptor_key_id, IREE_SV("descriptor_key"), descriptor_key_offset,
          &descriptor_key));
      loom_low_repr_descriptor_value_t descriptor_value = {0};
      if (!loom_low_repr_resolve_descriptor(
              body_reader->materializer->low_repr_environment,
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_attribute_materialize_ssa(
          &body_reader->materializer->attributes, cursor, descriptor,
          value_kind, &attrs[attr_index],
          body_reader->values.module_view->types.count, &attribute_ssa_scope));
    }
  }
...
loom_bytecode_body_reader_read_block(loom_bytecode_body_reader_t* body_reader,
  ...
  if (has_label) {
    ...
    IREE_RETURN_IF_ERROR(loom_bytecode_body_validate_string_ref(
        body_reader->values.decoder, body_reader->values.module_view, label_id,
        IREE_SV("block_label"), label_offset, &unused_label));
...
iree_status_t loom_bytecode_body_materialize_symbol_regions(
  ...
  iree_arena_initialize(materializer->block_pool, &body_arena);
  ...
  if (iree_status_is_ok(status)) {
    ...
    for (iree_host_size_t root_ordinal = 0;
      ...
      status = loom_bytecode_reader_read_uvarint(
          materializer->attributes.decoder, &cursor, &region_index);
      if (!iree_status_is_ok(status)) {
        break;
      }
      if (region_index >= parent_op->region_count || region_index > UINT8_MAX) {
        status = loom_bytecode_value_scope_emit_invalid(
            &body_reader.values, region_index_offset,
      ...
      body_reader.values.predefined_values = NULL;
      body_reader.values.predefined_value_start =
          body_reader.values.next_value_number;
      body_reader.values.predefined_value_count = 0;
      body_reader.expect_predefined_entry_args = false;
      for (uint8_t i = 0; i < predefined_region_count; ++i) {
        if (predefined_regions[i].region_index != region_index) {
          continue;
        }
        body_reader.values.predefined_values = predefined_regions[i].values;
        body_reader.values.predefined_value_count = predefined_regions[i].count;
  ...
  if (iree_status_is_ok(status)) {
    status = loom_bytecode_reader_expect_empty(materializer->attributes.decoder,
                                               &cursor, IREE_SV("IR"));
  }

...
static iree_status_t loom_bytecode_reader_read_source_trivia(
  ...
  IREE_RETURN_IF_ERROR(
      loom_bytecode_reader_read_uvarint(reader, cursor, &source_trivia));
  if (loom_bytecode_reader_has_errors(reader)) return iree_ok_status();
  uint64_t count =
...
static iree_status_t loom_bytecode_reader_read_signed_enum_set(
  ...
  IREE_RETURN_IF_ERROR(
      loom_bytecode_reader_read_u8(reader, cursor, &word_count));
  if (loom_bytecode_reader_has_errors(reader)) return iree_ok_status();
  if (word_count > LOOM_SIGNED_ENUM_SET_MAX_WORD_COUNT) {
    return loom_bytecode_reader_emit_count_exceeds(
        reader, IREE_SV("signed_enum_set_word_count"), word_count,
  ...
  for (iree_host_size_t i = 0; i < (iree_host_size_t)word_count * 2; ++i) {
    IREE_RETURN_IF_ERROR(
        loom_bytecode_reader_read_u64_le(reader, cursor, &words[i]));
    if (loom_bytecode_reader_has_errors(reader)) return iree_ok_status();
  }
...
static iree_status_t loom_bytecode_reader_skip_attr_value_at_depth(
  ...
  switch (kind) {
    ...
    case LOOM_BYTECODE_ATTR_ENUM_ARRAY: {
      ...
      return loom_bytecode_reader_read_span(reader, cursor, count, &unused);
    }
    ...
    case LOOM_BYTECODE_ATTR_SYMBOL_SET: {
      ...
      if (count > UINT16_MAX) {
        return loom_bytecode_reader_emit_count_exceeds(
            reader, collection_name, count, UINT16_MAX, count_offset);
      }
    ...
    case LOOM_BYTECODE_ATTR_PARAMETERIZED: {
      ...
      for (uint64_t i = 0; i < present_count; ++i) {
        ...
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
            reader, cursor, &parameter_name_id));
        if (loom_bytecode_reader_has_errors(reader)) return iree_ok_status();
...
static iree_status_t loom_bytecode_reader_read_parameterized_type(
  ...
  if (present_count > descriptor->parameter_count) {
    return loom_bytecode_reader_emit_count_exceeds(
        reader, IREE_SV("parameterized_type_parameters"), present_count,
...
static iree_status_t loom_bytecode_reader_read_types(
  ...
  if (read_mode == LOOM_BYTECODE_TYPE_READ_MATERIALIZE &&
    ...
    return loom_bytecode_reader_emit_invalid_field(
        reader, IREE_SV("TYPES"), IREE_SV("header"), 0, IREE_SV("type_count"),
        count_offset,
...
static iree_status_t loom_bytecode_reader_skip_symbol_payload(
  ...
  if (kind <= LOOM_BYTECODE_SYMBOL_FUNC_UKERNEL) {
    ...
    uint64_t result_count = 0;
    IREE_RETURN_IF_ERROR(
        loom_bytecode_reader_read_uvarint(reader, cursor, &workload_arg_count));
    IREE_RETURN_IF_ERROR(
        loom_bytecode_reader_read_uvarint(reader, cursor, &arg_count));
    IREE_RETURN_IF_ERROR(
        loom_bytecode_reader_read_uvarint(reader, cursor, &result_count));
...
static iree_status_t loom_bytecode_reader_read_symbols(
  ...
  for (uint64_t symbol_index = 0; symbol_index < count; ++symbol_index) {
    ...
    if (kind <= LOOM_BYTECODE_SYMBOL_FUNC_UKERNEL) {
      ...
      uint64_t result_count = 0;
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
          reader, &cursor, &workload_arg_count));
      IREE_RETURN_IF_ERROR(
          loom_bytecode_reader_read_uvarint(reader, &cursor, &arg_count));
      IREE_RETURN_IF_ERROR(
          loom_bytecode_reader_read_uvarint(reader, &cursor, &result_count));
...
static iree_status_t loom_bytecode_reader_read_provider_imports(
  ...
  IREE_RETURN_IF_ERROR(
      loom_bytecode_reader_read_uvarint(reader, &cursor, &provider_count));
  IREE_RETURN_IF_ERROR(
      loom_bytecode_reader_read_uvarint(reader, &cursor, &total_anchor_count));
  if (loom_bytecode_reader_has_errors(reader)) return iree_ok_status();
  if (provider_count > LOOM_BYTECODE_MAX_PROVIDER_IMPORT_COUNT ||
      provider_count > IREE_HOST_SIZE_MAX) {
    return loom_bytecode_reader_emit_count_exceeds(
        reader, IREE_SV("PROVIDER_IMPORTS"), provider_count,
  ...
  if (total_anchor_count > UINT32_MAX ||
    ...
    return loom_bytecode_reader_emit_invalid_field(
        reader, IREE_SV("PROVIDER_IMPORTS"), IREE_SV("header"), 0,
  ...
  for (iree_host_size_t provider_index = 0;
    ...
    IREE_RETURN_IF_ERROR(
        loom_bytecode_reader_read_uvarint(reader, &cursor, &provider_id));
    ...
    if (provider_index > 0 &&
      ...
      return loom_bytecode_reader_emit_invalid_field(
          reader, IREE_SV("PROVIDER_IMPORTS"), IREE_SV("provider"),
    ...
    IREE_RETURN_IF_ERROR(
        loom_bytecode_reader_read_uvarint(reader, &cursor, &anchor_count));
    if (loom_bytecode_reader_has_errors(reader)) return iree_ok_status();
    ...
    for (iree_host_size_t local_anchor_index = 0;
      ...
      IREE_RETURN_IF_ERROR(
          loom_bytecode_reader_read_uvarint(reader, &cursor, &symbol_index));
      if (loom_bytecode_reader_has_errors(reader)) return iree_ok_status();
      if (symbol_index >= reader->symbol_count) {
        return loom_bytecode_reader_emit_invalid_field(
            reader, IREE_SV("PROVIDER_IMPORTS"), IREE_SV("anchor"),
  ...
  for (iree_host_size_t symbol_index = 0; symbol_index < reader->symbol_count;
    ...
    if ((anchor_usage_bits[symbol_index / 64] &
      ...
      return loom_bytecode_reader_emit_invalid_field(
          reader, IREE_SV("PROVIDER_IMPORTS"), IREE_SV("anchor"), symbol_index,
          IREE_SV("symbol_index"), provider_imports_section->absolute_offset,
...
static iree_status_t loom_bytecode_reader_read_symbol_references(
  ...
  IREE_RETURN_IF_ERROR(
      loom_bytecode_reader_read_uvarint(reader, &cursor, &symbol_count));
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      reader, &cursor, &total_dependency_count));
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      reader, &cursor, &total_contract_demand_count));
  if (loom_bytecode_reader_has_errors(reader)) return iree_ok_status();
  if (symbol_count != reader->symbol_count) {
    return loom_bytecode_reader_emit_invalid_field(
        reader, IREE_SV("SYMBOL_REFERENCES"), IREE_SV("header"), 0,
  ...
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      reader, &cursor, &module_dependency_count));
  if (loom_bytecode_reader_has_errors(reader)) return iree_ok_status();
  if (module_dependency_count > total_dependency_count) {
    return loom_bytecode_reader_emit_invalid_field(
        reader, IREE_SV("SYMBOL_REFERENCES"), IREE_SV("module"), 0,
  ...
  for (uint64_t i = 0; i < module_dependency_count; ++i, ++dependency_index) {
    ...
    IREE_RETURN_IF_ERROR(
        loom_bytecode_reader_read_uvarint(reader, &cursor, &dependency));
    if (loom_bytecode_reader_has_errors(reader)) return iree_ok_status();
    if (dependency >= symbol_count) {
      return loom_bytecode_reader_emit_invalid_field(
          reader, IREE_SV("SYMBOL_REFERENCES"), IREE_SV("module_dependency"), i,
          IREE_SV("target_symbol_index"), dependency_offset,
  ...
  for (iree_host_size_t symbol_index = 0; symbol_index < reader->symbol_count;
    ...
    IREE_RETURN_IF_ERROR(
        loom_bytecode_reader_read_uvarint(reader, &cursor, &dependency_count));
    if (loom_bytecode_reader_has_errors(reader)) return iree_ok_status();
    if (dependency_count > total_dependency_count - dependency_index) {
      return loom_bytecode_reader_emit_invalid_field(
          reader, IREE_SV("SYMBOL_REFERENCES"), IREE_SV("symbol"), symbol_index,
          IREE_SV("dependency_count"), dependency_count_offset,
          IREE_SV("row_dependency_count_exceeds_declared_total"));
    }
    ...
    for (uint64_t i = 0; i < dependency_count; ++i, ++dependency_index) {
      ...
      if (dependency >= symbol_count) {
        return loom_bytecode_reader_emit_invalid_field(
            reader, IREE_SV("SYMBOL_REFERENCES"), IREE_SV("dependency"), i,
            IREE_SV("target_symbol_index"), dependency_offset,
            IREE_SV("dependency_symbol_index_is_out_of_range"));
      }
      if (module_metadata) {
    ...
    IREE_RETURN_IF_ERROR(
        loom_bytecode_reader_read_uvarint(reader, &cursor, &contract_count));
    if (loom_bytecode_reader_has_errors(reader)) return iree_ok_status();
    if (contract_count > total_contract_demand_count - contract_demand_index) {
      return loom_bytecode_reader_emit_invalid_field(
          reader, IREE_SV("SYMBOL_REFERENCES"), IREE_SV("symbol"), symbol_index,
          IREE_SV("contract_demand_count"), contract_count_offset,
    ...
    for (uint64_t i = 0; i < contract_count; ++i, ++contract_demand_index) {
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
          reader, &cursor, &contract_string_id));

...
typedef struct loom_bytecode_body_reader_t {
  ...
  // Number of region arguments that must reuse predefined values.
...
static iree_status_t loom_bytecode_reader_emit_invalid_ir_body(
  ...
  return loom_bytecode_reader_emit_error(&body_reader->reader->decoder,
...
static iree_status_t loom_bytecode_reader_validate_type_id_bounded(
    loom_bytecode_module_reader_t* reader, uint64_t type_id,
    uint64_t type_count, uint64_t offset) {
...
static iree_status_t loom_bytecode_reader_validate_type_id(
    loom_bytecode_module_reader_t* reader, uint64_t type_id, uint64_t offset) {
  return loom_bytecode_reader_validate_type_id_bounded(
      reader, type_id, reader->view.types.count, offset);
}
  ...
  for (uint64_t predicate_index = 0; predicate_index < predicate_count;
    ...
    if (predicate_kind >= LOOM_PREDICATE_COUNT_) {
      return loom_bytecode_reader_emit_enum_value(
          &reader->decoder, IREE_SV("predicate_kind"), predicate_kind,
          LOOM_PREDICATE_COUNT_, kind_offset);
    }
    IREE_RETURN_IF_ERROR(
        loom_bytecode_reader_read_u8(&reader->decoder, cursor, &arg_count));
    for (uint8_t arg_index = 0; arg_index < arg_count; ++arg_index) {
      uint8_t tag = 0;
      uint64_t tag_offset =
          loom_bytecode_reader_cursor_absolute_position(cursor);
      IREE_RETURN_IF_ERROR(
          loom_bytecode_reader_read_u8(&reader->decoder, cursor, &tag));
      if (tag == 1) {
        uint64_t encoded_value = 0;
        uint64_t value_offset =
            loom_bytecode_reader_cursor_absolute_position(cursor);
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
            &reader->decoder, cursor, &encoded_value));
        ...
        int64_t value = 0;
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_svarint(&reader->decoder,
                                                               cursor, &value));
        (void)value;
      } else {
        return loom_bytecode_reader_emit_enum_value(
            &reader->decoder, IREE_SV("predicate_arg_tag"), tag, 3, tag_offset);
      }
    }
  }
  return iree_ok_status();
...
static uint8_t loom_bytecode_reader_find_parameter_index(
    const loom_attr_descriptor_t* parameter_descriptors,
    uint8_t parameter_count, iree_string_view_t parameter_name,
    uint8_t start_index) {
  for (uint8_t i = start_index; i < parameter_count; ++i) {
    if (iree_string_view_equal(
            parameter_name,
            loom_attr_descriptor_name(&parameter_descriptors[i]))) {
      return i;
    }
  }
...
static iree_status_t loom_bytecode_reader_read_signed_enum_set(
  ...
  IREE_RETURN_IF_ERROR(
      loom_bytecode_reader_read_u8(&reader->decoder, cursor, &word_count));
  if (word_count > LOOM_SIGNED_ENUM_SET_MAX_WORD_COUNT) {
    return loom_bytecode_reader_emit_count_exceeds(
        &reader->decoder, IREE_SV("signed_enum_set_word_count"), word_count,
        LOOM_SIGNED_ENUM_SET_MAX_WORD_COUNT, payload_offset);
  }

  uint64_t words[LOOM_SIGNED_ENUM_SET_MAX_WORD_COUNT * 2] = {0};
  for (iree_host_size_t i = 0; i < (iree_host_size_t)word_count * 2; ++i) {
    IREE_RETURN_IF_ERROR(
        loom_bytecode_reader_read_u64_le(&reader->decoder, cursor, &words[i]));
  }

  const uint64_t* negative_words = words + word_count;
  for (iree_host_size_t i = 0; i < word_count; ++i) {
    if ((words[i] & negative_words[i]) != 0) {
      return loom_bytecode_reader_emit_invalid_field(
          &reader->decoder, cursor->range_name, IREE_SV("attribute"), 0,
          IREE_SV("signed_enum_set"), payload_offset,
  ...
  if (word_count > 0 && words[word_count - 1] == 0 &&
      negative_words[word_count - 1] == 0) {
    return loom_bytecode_reader_emit_invalid_field(
        &reader->decoder, cursor->range_name, IREE_SV("attribute"), 0,
        IREE_SV("signed_enum_set"), payload_offset,
  ...
  const loom_signed_enum_set_t set =
      loom_make_signed_enum_set(words, word_count);
  for (iree_host_size_t value = 0; value < 256; ++value) {
    if (!loom_signed_enum_set_contains_positive(set, (uint8_t)value) &&
  ...
  if (out_attr != NULL) {
    ...
    if (word_count > 0) {
      IREE_RETURN_IF_ERROR(iree_arena_allocate_array(
          &reader->output_module->arena, (iree_host_size_t)word_count * 2,
          sizeof(*owned_words), (void**)&owned_words));
      memcpy(owned_words, words,
             (iree_host_size_t)word_count * 2 * sizeof(*owned_words));
    }
    *out_attr = loom_attr_signed_enum_set(owned_words, word_count);
  }
  return iree_ok_status();
}
...
static iree_status_t loom_bytecode_reader_skip_attr_value_at_depth(
  ...
  switch (kind) {
    case LOOM_BYTECODE_ATTR_I64: {
      int64_t value = 0;
      IREE_RETURN_IF_ERROR(
          loom_bytecode_reader_read_svarint(&reader->decoder, cursor, &value));
      return iree_ok_status();
    }
    case LOOM_BYTECODE_ATTR_F64: {
      uint64_t bits = 0;
      IREE_RETURN_IF_ERROR(
          loom_bytecode_reader_read_u64_le(&reader->decoder, cursor, &bits));
      return iree_ok_status();
    }
    ...
    case LOOM_BYTECODE_ATTR_SCOPED_ENUM: {
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
          &reader->decoder, cursor, &string_id));
      iree_string_view_t unused = iree_string_view_empty();
    ...
    case LOOM_BYTECODE_ATTR_ENUM: {
      uint8_t unused = 0;
      return loom_bytecode_reader_read_u8(&reader->decoder, cursor, &unused);
    }
    case LOOM_BYTECODE_ATTR_BOOL: {
      uint8_t value = 0;
      uint64_t offset = loom_bytecode_reader_cursor_absolute_position(cursor);
      IREE_RETURN_IF_ERROR(
          loom_bytecode_reader_read_u8(&reader->decoder, cursor, &value));
      if (value > 1) {
        return loom_bytecode_reader_emit_enum_value(
            &reader->decoder, IREE_SV("bool_attribute"), value, 2, offset);
      }
      return iree_ok_status();
    }
    ...
    case LOOM_BYTECODE_ATTR_ENUM_ARRAY: {
      ...
      if (count > UINT16_MAX) {
        return loom_bytecode_reader_emit_count_exceeds(
            &reader->decoder, IREE_SV("enum_array"), count, UINT16_MAX,
            count_offset);
      }
      iree_const_byte_span_t unused = iree_const_byte_span_empty();
      return loom_bytecode_reader_read_span(&reader->decoder, cursor, count,
                                            &unused);
    }
    case LOOM_BYTECODE_ATTR_SIGNED_ENUM_SET:
      return loom_bytecode_reader_read_signed_enum_set(
          reader, cursor, descriptor, /*out_attr=*/NULL);
    case LOOM_BYTECODE_ATTR_BYTES: {
      uint64_t byte_length = 0;
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
          &reader->decoder, cursor, &byte_length));
      iree_const_byte_span_t unused = iree_const_byte_span_empty();
      return loom_bytecode_reader_read_span(&reader->decoder, cursor,
                                            byte_length, &unused);
    }
    case LOOM_BYTECODE_ATTR_SYMBOL: {
      ...
      uint64_t offset = loom_bytecode_reader_cursor_absolute_position(cursor);
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
          &reader->decoder, cursor, &symbol_name_id));
      iree_string_view_t unused = iree_string_view_empty();
      return loom_bytecode_reader_validate_string_ref(
    ...
    case LOOM_BYTECODE_ATTR_SYMBOL_SET: {
      ...
      if (count > UINT16_MAX) {
        return loom_bytecode_reader_emit_count_exceeds(
            &reader->decoder, collection_name, count, UINT16_MAX, count_offset);
      }
      ...
      for (uint64_t i = 0; i < count; ++i) {
        ...
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_validate_string_ref(
            reader, name_id, IREE_SV("symbol_collection_element"), name_offset,
            &name));
        if (is_set && i > 0 &&
    ...
    case LOOM_BYTECODE_ATTR_TYPE: {
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(&reader->decoder,
                                                             cursor, &type_id));
      return loom_bytecode_reader_validate_type_id_bounded(
          reader, type_id, available_type_count, offset);
    }
    ...
    case LOOM_BYTECODE_ATTR_DICT: {
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
          &reader->decoder, cursor, &entry_count));
      ...
      for (uint64_t entry_index = 0; entry_index < entry_count; ++entry_index) {
        ...
        if (entry_index > 0 &&
          ...
          return loom_bytecode_reader_emit_invalid_field(
              &reader->decoder, cursor->range_name, IREE_SV("dict"),
              entry_index, IREE_SV("key_id"), key_offset,
        ...
        if (value_kind >= LOOM_BYTECODE_ATTR_COUNT) {
          return loom_bytecode_reader_emit_enum_value(
              &reader->decoder, IREE_SV("attribute_kind"), value_kind,
              LOOM_BYTECODE_ATTR_COUNT, value_kind_offset);
        }
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_skip_attr_value_at_depth(
            reader, cursor, NULL, value_kind, predicate_value_args_are_strings,
    ...
    case LOOM_BYTECODE_ATTR_ENCODING: {
      ...
      uint64_t offset = loom_bytecode_reader_cursor_absolute_position(cursor);
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
          &reader->decoder, cursor, &encoding_id));
    ...
    case LOOM_BYTECODE_ATTR_PARAMETERIZED: {
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
          &reader->decoder, cursor, &family_name_id));
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
          &reader->decoder, cursor, &present_count));
      if (present_count > family_descriptor->parameter_count) {
        return loom_bytecode_reader_emit_count_exceeds(
            &reader->decoder, IREE_SV("parameterized_parameters"),
      ...
      for (uint64_t i = 0; i < present_count; ++i) {
        ...
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
            &reader->decoder, cursor, &parameter_name_id));
        ...
        if (parameter_index == LOOM_ATTR_INDEX_NONE) {
          ...
          return loom_bytecode_reader_emit_invalid_field(
              &reader->decoder, cursor->range_name, IREE_SV("attribute"), i,
    ...
    case LOOM_BYTECODE_ATTR_PARAMETERIZED_ARRAY: {
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
          &reader->decoder, cursor, &element_count));
      if (element_count > UINT16_MAX) {
        return loom_bytecode_reader_emit_count_exceeds(
            &reader->decoder, IREE_SV("parameterized_attribute_array"),
    ...
    default:
      return loom_bytecode_reader_emit_enum_value(
          &reader->decoder, IREE_SV("attribute_kind"), kind,
...
static iree_status_t loom_bytecode_reader_skip_attr_value(
  ...
  return loom_bytecode_reader_skip_attr_value_at_depth(
      reader, cursor, descriptor, kind, predicate_value_args_are_strings,
      available_type_count, /*aggregate_depth=*/0);
}
...
static iree_status_t loom_bytecode_reader_read_predicate_list_attr(
  ...
  if (predicate_count > UINT16_MAX || predicate_count > IREE_HOST_SIZE_MAX) {
    return loom_bytecode_reader_emit_count_exceeds(
        &reader->decoder, IREE_SV("predicate_list"), predicate_count,
  ...
  for (uint64_t predicate_index = 0; predicate_index < predicate_count;
    ...
    if (predicate->kind >= LOOM_PREDICATE_COUNT_) {
      return loom_bytecode_reader_emit_enum_value(
          &reader->decoder, IREE_SV("predicate_kind"), predicate->kind,
          LOOM_PREDICATE_COUNT_, kind_offset);
    }
    IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_u8(&reader->decoder, cursor,
                                                      &predicate->arg_count));
    ...
    if (predicate->arg_count != expected_arg_count ||
      ...
      return loom_bytecode_reader_emit_invalid_field(
          &reader->decoder, cursor->range_name, IREE_SV("predicate"),
          predicate_index, IREE_SV("arg_count"), kind_offset + 1,
    ...
    for (uint8_t arg_index = 0; arg_index < predicate->arg_count; ++arg_index) {
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_u8(
          &reader->decoder, cursor, &predicate->arg_tags[arg_index]));
      switch (predicate->arg_tags[arg_index]) {
        ...
        case LOOM_PRED_ARG_CONST: {
          IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_svarint(
              &reader->decoder, cursor, &predicate->args[arg_index]));
          break;
        }
        default:
          return loom_bytecode_reader_emit_enum_value(
              &reader->decoder, IREE_SV("predicate_arg_tag"),
...
static iree_status_t loom_bytecode_reader_read_parameterized_attr_payload(
  ...
  if (present_count > family_descriptor->parameter_count) {
    return loom_bytecode_reader_emit_count_exceeds(
        &reader->decoder, IREE_SV("parameterized_parameters"), present_count,
  ...
  for (uint64_t i = 0; i < present_count && iree_status_is_ok(status); ++i) {
    uint64_t parameter_offset =
        loom_bytecode_reader_cursor_absolute_position(cursor);
    uint64_t parameter_name_id = 0;
    status = loom_bytecode_reader_read_uvarint(&reader->decoder, cursor,
                                               &parameter_name_id);
    if (!iree_status_is_ok(status)) {
      break;
    }
    ...
    uint8_t value_kind = 0;
    status =
        loom_bytecode_reader_read_u8(&reader->decoder, cursor, &value_kind);
    if (!iree_status_is_ok(status)) {
      break;
    }
    if (value_kind >= LOOM_BYTECODE_ATTR_COUNT) {
  ...
  switch (kind) {
    ...
    case LOOM_BYTECODE_ATTR_BOOL: {
      ...
      IREE_RETURN_IF_ERROR(
          loom_bytecode_reader_read_u8(&reader->decoder, cursor, &value));
      if (value > 1) {
        return loom_bytecode_reader_emit_enum_value(&reader->decoder,
    ...
    case LOOM_BYTECODE_ATTR_I64_ARRAY: {
      ...
      if (count > UINT16_MAX || count > IREE_HOST_SIZE_MAX) {
        return loom_bytecode_reader_emit_count_exceeds(
            &reader->decoder, IREE_SV("i64_array"), count, UINT16_MAX,
            count_offset);
      }
      int64_t* values = NULL;
      if (count > 0) {
      ...
      for (uint64_t i = 0; i < count; ++i) {
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_svarint(
            &reader->decoder, cursor, &values[i]));
      }
    ...
    case LOOM_BYTECODE_ATTR_ENUM_ARRAY: {
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_span(
          &reader->decoder, cursor, count, &span));
      uint8_t* values = NULL;
    ...
    case LOOM_BYTECODE_ATTR_SIGNED_ENUM_SET:
      return loom_bytecode_reader_read_signed_enum_set(reader, cursor,
                                                       descriptor, out_attr);
    case LOOM_BYTECODE_ATTR_BYTES: {
      ...
      if (byte_length > UINT32_MAX || byte_length > IREE_HOST_SIZE_MAX) {
        return loom_bytecode_reader_emit_count_exceeds(
            &reader->decoder, IREE_SV("bytes_attribute"), byte_length,
            UINT32_MAX, byte_length_offset);
      }
      iree_const_byte_span_t span = iree_const_byte_span_empty();
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_span(
          &reader->decoder, cursor, byte_length, &span));
      uint8_t* bytes = NULL;
      if (span.data_length > 0) {
        IREE_RETURN_IF_ERROR(iree_arena_allocate(
    ...
    case LOOM_BYTECODE_ATTR_TYPE: {
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(&reader->decoder,
                                                             cursor, &type_id));
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_validate_type_id_bounded(
          reader, type_id, available_type_count, type_offset));
    ...
    case LOOM_BYTECODE_ATTR_DICT: {
      ...
      if (entry_count > UINT16_MAX || entry_count > IREE_HOST_SIZE_MAX) {
        return loom_bytecode_reader_emit_count_exceeds(
            &reader->decoder, IREE_SV("dict_entries"), entry_count, UINT16_MAX,
            count_offset);
      }
      ...
      for (uint64_t entry_index = 0; entry_index < entry_count; ++entry_index) {
        ...
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_attr_value_at_depth(
            reader, cursor, body_reader, NULL, value_kind,
            &entries[entry_index].value, available_type_count,
            aggregate_depth + 1));
      }
...
static iree_status_t loom_bytecode_reader_read_attr_value(
    ...
    const loom_attr_descriptor_t* descriptor, uint8_t kind,
    loom_attribute_t* out_attr, iree_host_size_t available_type_count) {
  return loom_bytecode_reader_read_attr_value_at_depth(
      reader, cursor, body_reader, descriptor, kind, out_attr,
      available_type_count, /*aggregate_depth=*/0);
}

static iree_status_t loom_bytecode_reader_validate_parameterized_type(
    loom_bytecode_module_reader_t* reader,
  ...
  iree_string_view_t family_name = iree_string_view_empty();
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_validate_string_ref(
      reader, family_name_id, IREE_SV("parameterized_type_family"),
      family_offset, &family_name));
  ...
  if (!type_descriptor || !type_descriptor->parameterized) {
    return loom_bytecode_reader_emit_invalid_field(
        &reader->decoder, IREE_SV("TYPES"), IREE_SV("type"), type_id,
        IREE_SV("family_name"), family_offset,
  ...
  if (present_count > descriptor->parameter_count) {
    return loom_bytecode_reader_emit_count_exceeds(
        &reader->decoder, IREE_SV("parameterized_type_parameters"),
        present_count, descriptor->parameter_count, present_count_offset);
  }
  ...
  IREE_RETURN_IF_ERROR(
      iree_arena_allocate(reader->arena, allocation_size, (void**)&fact));
  *fact = (loom_bytecode_parameterized_type_fact_t){
  ...
  for (iree_host_size_t i = 0; i < (iree_host_size_t)present_count; ++i) {
    ...
    iree_string_view_t parameter_name = iree_string_view_empty();
    IREE_RETURN_IF_ERROR(loom_bytecode_reader_validate_string_ref(
        reader, parameter_name_id, IREE_SV("parameterized_type_parameter"),
        parameter_offset, &parameter_name));
    const uint8_t parameter_index = loom_bytecode_reader_find_parameter_index(
        descriptor->parameter_descriptors, descriptor->parameter_count,
        parameter_name, next_parameter_index);
    if (parameter_index == LOOM_ATTR_INDEX_NONE) {
      const bool declared_before_cursor =
          loom_bytecode_reader_find_parameter_index(
              descriptor->parameter_descriptors, descriptor->parameter_count,
              parameter_name, /*start_index=*/0) != LOOM_ATTR_INDEX_NONE;
      return loom_bytecode_reader_emit_invalid_field(
          &reader->decoder, IREE_SV("TYPES"), IREE_SV("type"), type_id,
          IREE_SV("parameter_name"), parameter_offset,
          declared_before_cursor
    ...
    IREE_RETURN_IF_ERROR(loom_bytecode_reader_skip_attr_value(
        reader, cursor, parameter_descriptor, value_kind,
        /*predicate_value_args_are_strings=*/true, type_id));
  }
...
static iree_status_t loom_bytecode_reader_materialize_encodings(
  ...
  for (uint64_t instance_index = 0; instance_index < instance_count;
    ...
    if (param_count > UINT8_MAX || param_count > IREE_HOST_SIZE_MAX) {
      return loom_bytecode_reader_emit_count_exceeds(
          &reader->decoder, IREE_SV("encoding_params"), param_count, UINT8_MAX,
          param_count_offset);
    }
    ...
    if (encoding_id != instance_index + 1) {
      return loom_bytecode_reader_emit_invalid_field(
          &reader->decoder, IREE_SV("ENCODINGS"), IREE_SV("instance"),
...
static iree_status_t loom_bytecode_reader_build_shaped_type(
  ...
  if (element_type >= LOOM_SCALAR_TYPE_COUNT_) {
    return loom_bytecode_reader_emit_enum_value(
        &reader->decoder, IREE_SV("element_type"), element_type,
        LOOM_SCALAR_TYPE_COUNT_, offset);
  }
  if (rank > LOOM_TYPE_MAX_RANK) {
    return loom_bytecode_reader_emit_invalid_field(
        &reader->decoder, IREE_SV("TYPES"), IREE_SV("type"), 0, IREE_SV("rank"),
        offset, IREE_SV("rank_exceeds_loom_type_maximum"));
  }
  if (kind == LOOM_TYPE_VECTOR && rank == 0) {
    return loom_bytecode_reader_emit_invalid_field(
        &reader->decoder, IREE_SV("TYPES"), IREE_SV("type"), 0, IREE_SV("rank"),
        offset, IREE_SV("vector_rank_zero"));
  }
  ...
  switch (attachment) {
    case LOOM_BYTECODE_ENCODING_ATTACHMENT_NONE:
      if (encoding_instance != 0) {
        return loom_bytecode_reader_emit_invalid_field(
            &reader->decoder, IREE_SV("TYPES"), IREE_SV("type"), 0,
    ...
    default:
      return loom_bytecode_reader_emit_enum_value(
          &reader->decoder, IREE_SV("encoding_attachment"), attachment,
...
static iree_status_t loom_bytecode_reader_validate_types(
    loom_bytecode_module_reader_t* reader,
    const loom_bytecode_reader_section_t* section) {
  loom_bytecode_reader_cursor_t cursor;
  loom_bytecode_reader_cursor_initialize(
  ...
  if (count > LOOM_BYTECODE_MAX_TYPE_COUNT || count > IREE_HOST_SIZE_MAX) {
    return loom_bytecode_reader_emit_count_exceeds(
        &reader->decoder, IREE_SV("TYPES"), count, LOOM_BYTECODE_MAX_TYPE_COUNT,
        count_offset);
  }
  if (count > 0) {
    IREE_RETURN_IF_ERROR(
        iree_arena_allocate_array(reader->arena, (iree_host_size_t)count,
                                  ...
                                  (void**)&reader->view.types.entries));
  }
  ...
  for (uint64_t type_index = 0; type_index < count; ++type_index) {
    ...
    loom_bytecode_type_plan_entry_t* plan_entry =
        &reader->view.types.entries[type_index];
    plan_entry->bytecode_offset = type_offset;
    uint8_t kind_byte = 0;
    IREE_RETURN_IF_ERROR(
        loom_bytecode_reader_read_u8(&reader->decoder, &cursor, &kind_byte));
    ...
    switch (kind) {
      ...
      case LOOM_TYPE_SCALAR: {
        ...
        uint64_t element_offset =
            loom_bytecode_reader_cursor_absolute_position(&cursor);
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_u8(
            &reader->decoder, &cursor, &element_type));
        if (element_type >= LOOM_SCALAR_TYPE_COUNT_) {
          return loom_bytecode_reader_emit_enum_value(
              &reader->decoder, IREE_SV("scalar_type"), element_type,
      ...
      case LOOM_TYPE_VIEW: {
        ...
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_u8(
            &reader->decoder, &cursor, &element_type));
        IREE_RETURN_IF_ERROR(
            loom_bytecode_reader_read_u8(&reader->decoder, &cursor, &rank));
        uint64_t attachment_offset =
            loom_bytecode_reader_cursor_absolute_position(&cursor);
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_u8(
            &reader->decoder, &cursor, &attachment));
        uint64_t encoding_instance = 0;
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
            &reader->decoder, &cursor, &encoding_instance));
        uint64_t dims[LOOM_TYPE_MAX_RANK] = {0};
        if (rank > LOOM_TYPE_MAX_RANK) {
          return loom_bytecode_reader_emit_invalid_field(
              &reader->decoder, IREE_SV("TYPES"), IREE_SV("type"), type_index,
        ...
        for (uint8_t i = 0; i < rank; ++i) {
          ...
          uint64_t dim_offset =
              loom_bytecode_reader_cursor_absolute_position(&cursor);
          IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_u8(
              &reader->decoder, &cursor, &is_dynamic));
          if (is_dynamic == 0) {
            uint64_t size = 0;
            IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
                &reader->decoder, &cursor, &size));
            if (size > LOOM_DIM_MAX_STATIC_SIZE) {
              return loom_bytecode_reader_emit_invalid_field(
                  &reader->decoder, IREE_SV("TYPES"), IREE_SV("type"),
                  type_index, IREE_SV("dim_size"), dim_offset,
            ...
            return loom_bytecode_reader_emit_enum_value(
                &reader->decoder, IREE_SV("is_dynamic"), is_dynamic, 2,
                dim_offset);
          }
        }
      ...
      case LOOM_TYPE_FUNCTION: {
        ...
        *fact = (loom_bytecode_function_type_fact_t){
        ...
        for (iree_host_size_t i = 0; i < total_count; ++i) {
          ...
          IREE_RETURN_IF_ERROR(loom_bytecode_reader_validate_type_id_bounded(
              reader, type_id, type_index, ref_offset));
      ...
      case LOOM_TYPE_DIALECT: {
        ...
        if (param_count > UINT16_MAX || param_count > IREE_HOST_SIZE_MAX) {
          return loom_bytecode_reader_emit_count_exceeds(
              &reader->decoder, IREE_SV("dialect_type_params"), param_count,
              UINT16_MAX, type_offset);
        }
        if (param_count == 0) {
      ...
      case LOOM_TYPE_PARAMETERIZED: {
        loom_bytecode_parameterized_type_fact_t* fact = NULL;
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_validate_parameterized_type(
            reader, &cursor, (loom_type_id_t)type_index, &fact));
        type_fact = &fact->base;
        break;
      }
      case LOOM_TYPE_REGISTER: {
        ...
        uint64_t payload1 = 0;
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
            &reader->decoder, &cursor, &payload0));
        uint64_t payload1_offset =
            loom_bytecode_reader_cursor_absolute_position(&cursor);
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
            &reader->decoder, &cursor, &payload1));
        uint64_t has_value_type_offset =
            loom_bytecode_reader_cursor_absolute_position(&cursor);
        uint8_t has_value_type = 0;
        IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_u8(
            &reader->decoder, &cursor, &has_value_type));
        if (payload0 == 0) {
          return loom_bytecode_reader_emit_invalid_field(
              &reader->decoder, IREE_SV("TYPES"), IREE_SV("type"), type_index,
        ...
        if (has_value_type > 1) {
          return loom_bytecode_reader_emit_enum_value(
              &reader->decoder, IREE_SV("register_has_value_type"),
              has_value_type, 2, has_value_type_offset);
        }
        if (has_value_type) {
          ...
          IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
              &reader->decoder, &cursor, &value_type_id));
          IREE_RETURN_IF_ERROR(loom_bytecode_reader_validate_type_id_bounded(
              reader, value_type_id, type_index, value_type_offset));
          loom_bytecode_typed_register_fact_t* fact = NULL;
          IREE_RETURN_IF_ERROR(
              iree_arena_allocate(reader->arena, sizeof(*fact), (void**)&fact));
      ...
      case LOOM_TYPE_STORAGE: {
        ...
        uint64_t space_offset =
            loom_bytecode_reader_cursor_absolute_position(&cursor);
        IREE_RETURN_IF_ERROR(
            loom_bytecode_reader_read_u8(&reader->decoder, &cursor, &space));
        if (space >= LOOM_STORAGE_SPACE_COUNT_) {
          return loom_bytecode_reader_emit_enum_value(
              &reader->decoder, IREE_SV("storage_space"), space,
      ...
      case LOOM_TYPE_ENCODING: {
        ...
        uint64_t role_offset =
            loom_bytecode_reader_cursor_absolute_position(&cursor);
        IREE_RETURN_IF_ERROR(
            loom_bytecode_reader_read_u8(&reader->decoder, &cursor, &role));
        if (role >= LOOM_ENCODING_ROLE_COUNT_) {
          return loom_bytecode_reader_emit_enum_value(
              &reader->decoder, IREE_SV("encoding_role"), role,
  ...
  reader->view.types.facts = first_fact;
  return loom_bytecode_reader_expect_empty(&reader->decoder, &cursor,
                                           IREE_SV("TYPES"));
}

static iree_status_t loom_bytecode_reader_materialize_parameterized_type(
    loom_bytecode_module_reader_t* reader,
  ...
  if (descriptor->parameter_count > 0) {
    IREE_RETURN_IF_ERROR(
        iree_arena_allocate_array(reader->arena, descriptor->parameter_count,
  ...
  loom_bytecode_reader_cursor_t cursor;
  loom_bytecode_reader_cursor_initialize(
      reader->bytecode.data + (iree_host_size_t)fact->parameters_offset,
  ...
  for (uint8_t i = 0; i < fact->present_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
        &reader->decoder, &cursor, &unused_parameter_name_id));
    ...
    IREE_RETURN_IF_ERROR(
        loom_bytecode_reader_read_u8(&reader->decoder, &cursor, &value_kind));
    IREE_ASSERT(value_kind < LOOM_BYTECODE_ATTR_COUNT);
    IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_attr_value(
        reader, &cursor, /*body_reader=*/NULL,
        &descriptor->parameter_descriptors[parameter_index], value_kind,
        &parameters[parameter_index], fact->base.type_id));
  }
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_expect_empty(
      &reader->decoder, &cursor, IREE_SV("parameterized_type_parameters")));

  return loom_module_make_parameterized_type(
      reader->output_module, descriptor, parameters,
      descriptor->parameter_count, out_type);
}

static iree_status_t loom_bytecode_reader_materialize_type_fact(
    loom_bytecode_module_reader_t* reader,
  ...
  switch (fact->kind) {
    case LOOM_TYPE_FUNCTION: {
      ...
      IREE_RETURN_IF_ERROR(
          iree_arena_allocate(reader->arena, allocation_size, (void**)&data));
      ...
      for (iree_host_size_t i = 0; i < type_count; ++i) {
        data->types[i] =
            reader->output_module->types.entries[function_fact->type_ids[i]];
      }
    ...
    case LOOM_TYPE_DIALECT: {
      ...
      IREE_RETURN_IF_ERROR(iree_arena_allocate_array(
          reader->arena, dialect_fact->parameter_count, sizeof(*parameters),
          (void**)&parameters));
      for (uint16_t i = 0; i < dialect_fact->parameter_count; ++i) {
        parameters[i] =
            reader->output_module->types.entries[dialect_fact->type_ids[i]];
      }
    ...
    case LOOM_TYPE_PARAMETERIZED:
      return loom_bytecode_reader_materialize_parameterized_type(
          reader, (const loom_bytecode_parameterized_type_fact_t*)fact,
          out_type);
    case LOOM_TYPE_REGISTER: {
      ...
      IREE_RETURN_IF_ERROR(
          iree_arena_allocate(reader->arena, sizeof(*data), (void**)&data));
      *data = (loom_register_type_data_t){
          ...
          .value_type = reader->output_module->types
...
static iree_status_t loom_bytecode_reader_materialize_types(
    loom_bytecode_module_reader_t* reader) {
  const loom_bytecode_type_fact_t* fact = reader->view.types.facts;
  for (iree_host_size_t type_index = 0; type_index < reader->view.types.count;
       ++type_index) {
    ...
    if (fact && fact->type_id == type_index) {
      status = loom_bytecode_reader_materialize_type_fact(reader, fact, &type);
      fact = fact->next;
    } else {
      type = reader->view.types.entries[type_index].direct_type;
    }
    loom_type_id_t type_id = LOOM_TYPE_ID_INVALID;
    if (iree_status_is_ok(status)) {
      status =
          loom_module_intern_type_id(reader->output_module, type, &type_id);
    }
    if (iree_status_is_ok(status) && type_id != type_index) {
      status = loom_bytecode_reader_emit_invalid_field(
          &reader->decoder, IREE_SV("TYPES"), IREE_SV("type"), type_index,
          IREE_SV("type"),
          reader->view.types.entries[type_index].bytecode_offset,
...
static iree_status_t loom_bytecode_body_reader_bind_type(
  ...
  for (uint8_t i = 0; i < rank; ++i) {
    ...
    IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_svarint(
        &body_reader->reader->decoder, cursor, &value_number));
    if (value_number < 0) {
      return loom_bytecode_reader_emit_invalid_ir_body(
  ...
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &encoding_binding));
...
static iree_status_t loom_bytecode_body_reader_define_value(
  ...
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &name_id));
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &type_id));
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &dim_binding_count));
  ...
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_validate_type_id_bounded(
      body_reader->reader, type_id,
      body_reader->reader->output_module->types.count, type_offset));
  loom_type_t base_type =
      body_reader->reader->output_module->types.entries[type_id];
  ...
  const bool is_predefined =
      body_reader->next_value_number >= body_reader->predefined_value_start &&
      body_reader->next_value_number - body_reader->predefined_value_start <
...
static iree_status_t loom_bytecode_body_reader_reserve_value_defs(
  ...
  for (uint64_t value_number = start_value_number;
    ...
    loom_value_id_t value_id = LOOM_VALUE_ID_INVALID;
    if (value_number >= body_reader->predefined_value_start &&
        value_number - body_reader->predefined_value_start <
            body_reader->predefined_value_count) {
      value_id =
          body_reader->predefined_values[value_number -
                                         body_reader->predefined_value_start];
      if (value_id >= body_reader->reader->output_module->values.count) {
        return loom_bytecode_reader_emit_invalid_ir_body(
            body_reader, body_reader->body_offset,
            IREE_SV("predefined_value_is_not_present_in_the_module"));
...
static iree_status_t loom_bytecode_body_reader_read_op(
  ...
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &op_table_index_plus1));
  ...
  uint64_t instance_flags_offset =
      loom_bytecode_reader_cursor_absolute_position(cursor);
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_u8(
      &body_reader->reader->decoder, cursor, &instance_flags));
  ...
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &location_id));
  if (location_id != 0) {
  ...
  uint64_t result_count = 0;
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &operand_count));
  ...
  if (operand_segment_count > 0) {
    ...
    for (uint8_t i = 0; i < operand_segment_count; ++i) {
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
          &body_reader->reader->decoder, cursor, &segment_count));
  ...
  uint64_t successor_count_offset =
      loom_bytecode_reader_cursor_absolute_position(cursor);
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &successor_count));
  ...
  if (successor_count > 0) {
    ...
    for (uint64_t i = 0; i < successor_count; ++i) {
      ...
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
          &body_reader->reader->decoder, cursor, &block_index));
  ...
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &result_count));
  ...
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &tied_result_count));
  ...
  for (uint64_t i = 0; i < tied_result_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
        &body_reader->reader->decoder, cursor, &result_index));
    IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
        &body_reader->reader->decoder, cursor, &operand_index));
  ...
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &present_attr_count));
  ...
  for (uint64_t i = 0; i < present_attr_count; ++i) {
    ...
    uint64_t key_id = 0;
    IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
        &body_reader->reader->decoder, cursor, &key_id));
    iree_string_view_t key = iree_string_view_empty();
    IREE_RETURN_IF_ERROR(loom_bytecode_reader_validate_string_ref(
    ...
    uint64_t value_kind_offset =
        loom_bytecode_reader_cursor_absolute_position(cursor);
    IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_u8(
        &body_reader->reader->decoder, cursor, &value_kind));
    if (value_kind >= LOOM_BYTECODE_ATTR_COUNT) {
      return loom_bytecode_reader_emit_enum_value(
          &body_reader->reader->decoder, IREE_SV("attribute_kind"), value_kind,
          LOOM_BYTECODE_ATTR_COUNT, value_kind_offset);
    }
    if (descriptor->attr_kind == LOOM_ATTR_SCOPED_ENUM) {
      ...
      uint64_t descriptor_key_offset =
          loom_bytecode_reader_cursor_absolute_position(cursor);
      IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
          &body_reader->reader->decoder, cursor, &descriptor_key_id));
  ...
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &region_count));
...
static iree_status_t loom_bytecode_body_reader_read_block(
  ...
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_u8(
      &body_reader->reader->decoder, cursor, &has_label));
  if (has_label > 1) {
    return loom_bytecode_reader_emit_invalid_ir_body(
  ...
  if (has_label) {
    uint64_t label_id = 0;
    IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
        &body_reader->reader->decoder, cursor, &label_id));
  ...
  uint64_t arg_count_offset =
      loom_bytecode_reader_cursor_absolute_position(cursor);
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &arg_count));
  ...
  if (body_reader->expect_predefined_entry_args) {
    ...
    if (arg_count != body_reader->predefined_value_count) {
      return loom_bytecode_reader_emit_invalid_ir_body(
          body_reader, arg_count_offset,
  ...
  uint64_t op_count = 0;
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &op_count));
...
static iree_status_t loom_bytecode_body_reader_read_region(
  ...
  uint64_t source_flags_offset =
      loom_bytecode_reader_cursor_absolute_position(cursor);
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &source_flags));
  ...
  uint64_t block_count_offset =
      loom_bytecode_reader_cursor_absolute_position(cursor);
  IREE_RETURN_IF_ERROR(loom_bytecode_reader_read_uvarint(
      &body_reader->reader->decoder, cursor, &block_count));
...
static iree_status_t loom_bytecode_reader_read_symbol_regions(
  ...
  iree_status_t status = loom_bytecode_reader_read_uvarint(
      &reader->decoder, &cursor, &value_count);
  if (iree_status_is_ok(status)) {
    status = loom_bytecode_reader_read_uvarint(&reader->decoder, &cursor,
  ...
  if (iree_status_is_ok(status)) {
    ...
    for (uint64_t root_ordinal = 0; root_ordinal < root_region_count;
      ...
      if (first_region_index != LOOM_REGION_INDEX_NONE && root_ordinal == 0 &&
          region_index != first_region_index) {
        status = loom_bytecode_reader_emit_invalid_ir_body(
            &body_reader, region_index_offset,
      ...
      body_reader.predefined_values = NULL;
      body_reader.predefined_value_start = body_reader.next_value_number;
      body_reader.predefined_value_count = 0;
      body_reader.expect_predefined_entry_args = false;
      for (uint8_t i = 0; i < predefined_region_count; ++i) {
        if (predefined_regions[i].region_index != region_index) {
          continue;
        }
        body_reader.predefined_values = predefined_regions[i].values;
        body_reader.predefined_value_count = predefined_regions[i].count;
        body_reader.expect_predefined_entry_args = true;
        break;
      }
      loom_region_t* region = NULL;
    ...
    if (iree_status_is_ok(status)) {
      for (uint8_t i = 0; i < predefined_region_count; ++i) {
        if (predefined_regions[i].region_index < parent_op->region_count &&
            seen_regions[predefined_regions[i].region_index]) {
          continue;
        }
        status = loom_bytecode_reader_emit_invalid_ir_body(
            &body_reader, body_reader.body_offset,
            IREE_SV("symbol_region_payload_is_missing_a_signature_region"));
        break;
      }
    }
  }
  ...
  if (iree_status_is_ok(status)) {
    status = loom_bytecode_reader_expect_empty(&reader->decoder, &cursor,
                                               IREE_SV("IR"));
  }
...
static iree_status_t loom_bytecode_reader_validate_func_enum(
  ...
  if (!iree_any_bit_set(descriptor->flags, LOOM_ATTR_OPEN_ENUM) &&
    ...
    return loom_bytecode_reader_emit_enum_value(
        &reader->decoder, field_name, value,
...
static iree_status_t loom_bytecode_reader_skip_value_def(
  ...
  IREE_RETURN_IF_ERROR(
      loom_bytecode_reader_validate_type_id(reader, type_id, name_offset));
...
static iree_status_t loom_bytecode_reader_materialize_function_symbol(
  ...
  if (workload_arg_count > UINT16_MAX || arg_count > UINT16_MAX ||
      result_count > UINT16_MAX || workload_arg_count > IREE_HOST_SIZE_MAX ||
  ...
  const bool is_kernel = loom_symbol_definition_implements(
      vtable->symbol_def, LOOM_SYMBOL_INTERFACE_KERNEL);
  if (!is_kernel && workload_arg_count != 0) {
    return loom_bytecode_reader_emit_invalid_field(
        &reader->decoder, IREE_SV("SYMBOLS"), IREE_SV("symbol"), symbol_id,
        IREE_SV("kernel_workload_arg_count"),
        loom_bytecode_reader_cursor_absolute_position(cursor),
  ...
  for (uint64_t i = 0; i < workload_arg_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(loom_bytecode_body_reader_define_value(
        &signature_reader, cursor, &workload_arg_id));
  }
  for (uint64_t i = 0; i < arg_count; ++i) {
...
static iree_status_t loom_bytecode_reader_validate_module(
  ...
  IREE_RETURN_IF_ERROR(
      loom_bytecode_reader_validate_types(reader, reader->view.sections.types));
  IREE_RETURN_IF_ERROR(
...
static iree_status_t loom_bytecode_reader_normalize_diagnosed_error(
...
iree_status_t loom_bytecode_read_metadata(
  ...
  loom_bytecode_reader_decoder_initialize(
      ...
      filename, &reader.result.error_count, &reader.decoder);

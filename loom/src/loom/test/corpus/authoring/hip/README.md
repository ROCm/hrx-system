...
| wave size, template, specialization, fallback | targetless `func.apply`, typed `func.template ... requires`, `#target.subgroup.size` | `target_provider_selection.loom` |
...
```loom
%input_words = buffer.view %input_noalias[%base] : buffer -> view<4xi32, #encoding.layout.dense>

%w0 = vector.load %input_words[0] : view<4xi32, #encoding.layout.dense> -> vector<1xi32>
%w1 = vector.load %input_words[1] : view<4xi32, #encoding.layout.dense> -> vector<1xi32>
%w2 = vector.load %input_words[2] : view<4xi32, #encoding.layout.dense> -> vector<1xi32>
%w3 = vector.load %input_words[3] : view<4xi32, #encoding.layout.dense> -> vector<1xi32>

%wide = vector.load %input_words[0] : view<4xi32, #encoding.layout.dense> -> vector<4xi32>
```
...
%scratch_view = buffer.view %scratch[%base] : buffer -> view<64xi32, #encoding.layout.dense>

%loaded = vector.load %input_view[%lane] : view<64xi32, #encoding.layout.dense> -> vector<1xi32>
vector.store %loaded, %scratch_view[%lane] : vector<1xi32>, view<64xi32, #encoding.layout.dense>
kernel.barrier<workgroup> scope(workgroup) ordering(acq_rel)
%reversed = vector.load %scratch_view[%reverse_lane] : view<64xi32, #encoding.layout.dense> -> vector<1xi32>
vector.store %reversed, %output_view[%lane] : vector<1xi32>, view<64xi32, #encoding.layout.dense>
```
...
vector.store %loaded, %scratch_a_view[%row, %column] : vector<1xi32>, view<8x8xi32, #encoding.layout.dense>
kernel.barrier<workgroup> scope(workgroup) ordering(acq_rel)
%transposed = vector.load %scratch_a_view[%column, %row] : view<8x8xi32, #encoding.layout.dense> -> vector<1xi32>
vector.store %transposed, %scratch_b_view[%row, %column] : vector<1xi32>, view<8x8xi32, #encoding.layout.dense>
kernel.barrier<workgroup> scope(workgroup) ordering(acq_rel)
%roundtrip = vector.load %scratch_b_view[%column, %row] : view<8x8xi32, #encoding.layout.dense> -> vector<1xi32>
```
...
```loom
%scratch = buffer.alloca<workgroup> align(16) %scratch_bytes : buffer
%scratch_view = buffer.view %scratch[%base] : buffer -> view<64x4xi32, #encoding.layout.dense>

%loaded = vector.load %input_view[%lane, 0] : view<64x4xi32, #encoding.layout.dense> -> vector<4xi32>
vector.store %loaded, %scratch_view[%lane, 0] : vector<4xi32>, view<64x4xi32, #encoding.layout.dense>
kernel.barrier<workgroup> scope(workgroup) ordering(acq_rel)
%roundtrip = vector.load %scratch_view[%lane, 0] : view<64x4xi32, #encoding.layout.dense> -> vector<4xi32>
vector.store %roundtrip, %output_view[%lane, 0] : vector<4xi32>, view<64x4xi32, #encoding.layout.dense>
```
...
```loom
%q8_word = vector.load %q8_words[0] : view<1xi32, #encoding.layout.dense> -> vector<1xi32>
...
specialization, fallback, targetless, `func.apply`, `func.template`, `requires`,
...
```loom
func.template<hip.recipe.scale_i32> requires [#target.subgroup.size<64>] priority(20) @scale_i32_subgroup_64(%value: i32) -> (i32) { ... }
...
%copy = kernel.async.cluster.gather %source to %destination using %participants
    ...
    : view<16xi8, #encoding.layout.dense> to view<16xi8, #encoding.layout.dense>, i32 -> kernel.async.token

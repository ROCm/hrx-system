...
kernel.def @classified() {
  ...
  IREE_ASSERT_OK(loom_kernel_class_classifier_build(
      ...
      &classifier));
  ASSERT_EQ(classifier.decision_count, 3u);
  ...
  std::vector<loom_value_id_t> site_argument_values(kSiteCount * 3);
  ...
  IREE_ASSERT_OK(loom_value_fact_table_initialize(&site_facts, &analysis_arena,
                                                  kSiteCount * 3));
  site_facts.count = kSiteCount * 3;
  for (iree_host_size_t i = 0; i < kSiteCount; ++i) {
    const iree_host_size_t value_base = i * 3;
  ...
  ASSERT_EQ(collection.accepted_decision_count, 2u);
  ...
  for (loom_decision_class_ordinal_t class_ordinal = 0;
    ...
    EXPECT_EQ(zero_result_call_count, 1u);

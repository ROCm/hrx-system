...
static iree_status_t iree_benchmark_loom_run_dispatch_sample_work_item(
  ...
  iree_benchmark_loom_hal_work_item_state_t work_item_state = {0};
  iree_status_t status = iree_benchmark_loom_prepare_hal_work_item(
      hal_options, work_item, compile_context, inout_correctness_sample_count,
...
static iree_status_t iree_benchmark_loom_run_hal_case_end_to_end_work_item(
    const iree_benchmark_loom_work_plan_execution_options_t* options,
    const iree_benchmark_loom_hal_setup_options_t* hal_options,
    const iree_benchmark_loom_work_item_t* work_item,
    iree_benchmark_loom_hal_compile_context_t* compile_context,
  ...
  if (iree_status_is_ok(status) && work_item_state.runnable) {
    ...
    status = iree_benchmark_loom_measure_case_end_to_end_work_item(
        options->module_plan, options->work_plan, work_item,
    ...
    if (iree_status_is_ok(status)) {
      ...
      status = iree_benchmark_loom_emit_work_item_result_aliases(
          ...
          &benchmark_result, work_item_state.correctness_sample_count,
          work_item_state.correctness_failed_sample_count, options->event_sink,
...
iree_status_t iree_benchmark_loom_run_work_plan(
  ...
  if (work_plan->hal_compile_item_count != 0) {
    status = iree_allocator_malloc_array(
        ...
        sizeof(*hal_compile_contexts), (void**)&hal_compile_contexts);
    if (iree_status_is_ok(status)) {
      memset(hal_compile_contexts, 0,
             work_plan->hal_compile_item_count * sizeof(*hal_compile_contexts));
    }
  }
  const iree_benchmark_loom_hal_setup_options_t hal_options = {
      .run = options->run,
  ...
  for (iree_host_size_t work_item_index = 0;
    ...
    switch ((iree_benchmark_loom_work_item_kind_t)work_item->kind) {
      case IREE_BENCHMARK_LOOM_WORK_ITEM_CASE_END_TO_END: {
        if (work_item->hal_compile_item_index ==
          ...
          status = iree_benchmark_loom_run_hal_case_end_to_end_work_item(
              options, &hal_options, work_item,
              &hal_compile_contexts[work_item->hal_compile_item_index],
              inout_correctness_sample_count,
              inout_correctness_failed_sample_count,
              inout_failed_benchmark_count);
        }

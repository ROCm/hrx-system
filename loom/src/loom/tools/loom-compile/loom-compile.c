...
static void loom_compile_print_agents_markdown(FILE* stream) {
  fprintf(
      ...
      "source_issued_bytes:.source_low.memory.dispatch_issued.total_bytes, "
      "source_logical_bytes:.source_low.memory.dispatch_source.total_bytes, "
      ...
      "{function,memory_space,operation,strategy,storage,packet_count,"
      "dispatch_source,dispatch_issued,scalar_packet_count,"
      "vector_packet_count} | "
      "with_entries(select(.value != null))' report.json\n"
      ...
      "`target_legalization.rows[]`. Summary and detail reports aggregate\n"
      ...
      "Source-low memory economics are summarized under `.source_low.memory`,\n"
      "including `.roots[]`,\n"
      "`.arguments[]`, and `.strategies[]` when attribution is available.\n"
      "`economics.memory` is the target-low dynamic-instruction issued-memory\n"
      "estimate, including issued byte counts and nonzero instruction-family\n"
      "counts such as `global_load_count` and `buffer_load_count`. It is\n"
      "omitted when the compiler cannot prove that dynamic instruction mix;\n"
      "use `.source_low.memory` for workload-scaled source memory traffic\n"
      "when evaluating dynamic kernels.\n"
      ...
      "`fp8_packed_bf16_decode_repair_zero`. Do not infer those facts from\n"
      "raw scalar type spelling.\n"
      ...
      "index-width, and matrix-profile facts. Detail rows carry `function`,\n"

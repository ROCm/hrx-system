...
static void loom_compile_print_agents_markdown(FILE* stream) {
  fprintf(
      ...
      "bytes only when dynamic packet counts are proven. Unknown dynamic\n"
      "coverage is explicit, for example `unknown_dynamic_packet_count`.\n"
      "Detailed attribution remains under `.source_low.memory`, including\n"
      "`.roots[]`, `.arguments[]`, and `.strategies[]` when available.\n"

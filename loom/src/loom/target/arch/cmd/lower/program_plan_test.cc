...
kernel.def @increment(%element_count: index) {
  ...
  %source_view = buffer.view %source[%base] : buffer -> view<128xi32, #encoding.layout.dense>
  %target_view = buffer.view %target[%base] : buffer -> view<128xi32, #encoding.layout.dense>
  %value = view.load %source_view[%workgroup] : view<128xi32, #encoding.layout.dense> -> i32
  %result = scalar.addi %value, %c1 : i32
  view.store %result, %target_view[%workgroup] : i32, view<128xi32, #encoding.layout.dense>
  kernel.return
}
...
} launch(%lhs: view<3xi32, #encoding.layout.dense>, %rhs: view<4xi32, #encoding.layout.dense>, %target: buffer) {
  %element = index.constant 0 : index
  %base = index.constant 0 : offset
  %lhs_value = view.load %lhs[%element] : view<3xi32, #encoding.layout.dense> -> i32
  %rhs_value = view.load %rhs[%element] : view<4xi32, #encoding.layout.dense> -> i32
  %sum = scalar.addi %lhs_value, %rhs_value : i32
  %target_view = buffer.view %target[%base] : buffer -> view<1xi32, #encoding.layout.dense>
  view.store %sum, %target_view[%element] : i32, view<1xi32, #encoding.layout.dense>
  kernel.return
}

command.program.def public @parameterized() launch(%parameters: buffer, %target: buffer) {
  %layer = index.constant 3 : index
  %lhs = command.parameter %parameters, "blk.{}.lhs"[%layer] : view<3xi32, #encoding.layout.dense>
  %rhs = command.parameter %parameters, "shared.rhs" : view<4xi32, #encoding.layout.dense>
  kernel.launch @combine(%lhs, %rhs, %target) : (view<3xi32, #encoding.layout.dense>, view<4xi32, #encoding.layout.dense>, buffer)
  command.return
}
)");
  ASSERT_NE(source_module, nullptr);

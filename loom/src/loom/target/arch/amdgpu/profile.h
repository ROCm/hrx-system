...
// Structured AMDHSA facts retained by an AMDGPU target profile.
typedef struct loom_amdgpu_amdhsa_profile_facts_t {
  // Processor record selected for this profile.
  const loom_amdgpu_processor_info_t* processor;

  // gfx1250 silicon revision, or UNSPECIFIED for other processors.
  loom_amdgpu_gfx1250_revision_t gfx1250_revision;

  // SRAM ECC mode selected for this profile.
  loom_amdgpu_target_feature_selection_t sramecc;

  // XNACK mode selected for this profile.
  loom_amdgpu_target_feature_selection_t xnack;
} loom_amdgpu_amdhsa_profile_facts_t;
...
typedef struct loom_amdgpu_target_profile_t {
  ...
  // Structured AMDHSA processor and feature identity.
  loom_amdgpu_amdhsa_profile_facts_t amdhsa;

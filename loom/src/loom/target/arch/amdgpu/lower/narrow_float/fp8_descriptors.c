...
#define LOOM_AMDGPU_FP8_SCALED_DESCRIPTOR_REF_ROW(             \
    source_type, result_type, scalef32_pair_ref, e8m0_pk8_ref) \
  {source_type, result_type, scalef32_pair_ref, e8m0_pk8_ref}
...
static bool loom_amdgpu_fp8_scaled_descriptor_ref_row(
  ...
  *out_row_index = 0;
  *out_row = NULL;
  for (size_t i = 0; i < IREE_ARRAYSIZE(kLoomAmdgpuFp8ScaledDescriptorRefRows);
       ++i) {
    const loom_amdgpu_fp8_scaled_descriptor_ref_row_t* row =
        &kLoomAmdgpuFp8ScaledDescriptorRefRows[i];
    if (row->source_element_type == source_element_type &&
        row->result_element_type == result_element_type) {
      *out_row_index = i;
      *out_row = row;

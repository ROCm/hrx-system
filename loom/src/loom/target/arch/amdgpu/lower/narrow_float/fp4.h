...
// Returns true when vector.decode has an exact portable packed E2M1 schema:
// unscaled or E4M3FN-scaled F16, or group-32 E8M0-scaled BF16. The active
// descriptor set must provide the complete selected packet recipe.
bool loom_amdgpu_vector_decode_can_lower_as_fp4_conversion(
    const loom_module_t* module, const loom_value_fact_table_t* fact_table,
    const loom_low_descriptor_set_t* descriptor_set,
    const loom_op_t* source_op);

// Selects a portable packed E2M1 vector.decode plan.
iree_status_t loom_amdgpu_select_fp4_decode_plan(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    loom_amdgpu_vector_16bit_float_conversion_plan_t* out_plan,
    bool* out_selected);

// Lowers one selected portable packed E2M1 vector.decode plan.

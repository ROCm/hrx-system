...
static iree_status_t loom_amdgpu_emit_fp8_normal_packed_bf16_payload(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan,
    loom_value_id_t low_source_no_sign_pair,
    loom_value_id_t low_packed_exponent_bias, loom_type_t vgpr_type,
    loom_value_id_t* out_payload) {
  const loom_scalar_type_fp8_format_t* format = &plan->format;
  const uint32_t payload_shift = 7u - format->mantissa_bits;
  IREE_ASSERT_NE(low_packed_exponent_bias, LOOM_VALUE_ID_INVALID);
  if (iree_any_bit_set(
          plan->flags,
          LOOM_AMDGPU_FP8_DECODE_PLAN_FLAG_HAS_LSHL_ADD_U32_SHIFT_IMM)) {
    return loom_amdgpu_emit_resolved_vgpr_lshl_add_u32(
        context, source_op, &plan->lshl_add_u32_shift_imm_descriptor,
        low_source_no_sign_pair, low_packed_exponent_bias, payload_shift,
  ...
  return loom_amdgpu_emit_vgpr_binary(
      ...
      low_packed_exponent_bias, vgpr_type, out_payload);
}

static iree_status_t loom_amdgpu_emit_fp8_normal_packed_f16_payload(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan,
    loom_value_id_t low_source_no_sign_pair,
    loom_value_id_t low_packed_exponent_bias, loom_type_t vgpr_type,
    loom_value_id_t* out_payload) {
  const loom_scalar_type_fp8_format_t* format = &plan->format;
  const uint32_t payload_shift = 10u - format->mantissa_bits;
  const uint32_t exponent_bias = (15u - format->exponent_bias) << 10;
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_mask_from_condition(
  ...
  *out_mask = LOOM_VALUE_ID_INVALID;
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_const_u32(
      context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_S_MOV_B32,
      LOOM_AMDGPU_FP8_DECODE_BF16_PAIR_ASHR_15_MASK, sgpr_type, &low_shift));
  return loom_amdgpu_emit_fp8_packed_bf16_mask_from_condition_with_shift(
      context, source_op, plan, low_condition, low_shift, vgpr_type, out_mask);
}
...
typedef struct loom_amdgpu_fp8_packed_bf16_pair_state_t {
  // Pair base shared by packed 16-bit FP8 decode paths.
  loom_amdgpu_fp8_packed_u16_pair_base_t base;
  // BF16 payload produced by the fast normal-value expansion.
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_pair_state(
    ...
    const loom_amdgpu_fp8_decode_plan_t* plan,
    const loom_amdgpu_fp8_packed_u16_pair_source_t* pair_source,
    loom_value_id_t low_zero, loom_value_id_t low_packed_exponent_bias,
  ...
  *out_state = (loom_amdgpu_fp8_packed_bf16_pair_state_t){
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_normal_packed_bf16_payload(
      context, source_op, plan, out_state->base.source_no_sign,
      low_packed_exponent_bias, vgpr_type, &out_state->normal_payload));
  return iree_ok_status();
}
...
iree_status_t loom_amdgpu_try_emit_fp8_pair_to_packed_f16_finite(
  ...
  return loom_amdgpu_try_emit_fp8_pairs_to_packed_f16_finite(
      ...
      sgpr_type, out_low_packet, out_selected);
}
...
iree_status_t loom_amdgpu_try_emit_fp8_pairs_to_packed_f16_finite(
  ...
  *out_selected = false;
  ...
  if (!loom_amdgpu_can_emit_fp8_pair_to_packed_f16_finite(plan, value_flags)) {
    return iree_ok_status();
  }
  for (iree_host_size_t i = 0; i < pair_count; ++i) {
    if (pair_sources[i].byte_offset >
        IREE_ARRAYSIZE(kLoomAmdgpuFp8DecodeBf16BytePairSelectors) - 1) {
      return iree_ok_status();
    }
  }
  ...
  const uint32_t packed_exponent_bias = exponent_bias | (exponent_bias << 16);
  loom_value_id_t low_packed_exponent_bias = LOOM_VALUE_ID_INVALID;
  if (packed_exponent_bias != 0) {
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_const_u32(
        context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_V_MOV_B32,
        packed_exponent_bias, vgpr_type, &low_packed_exponent_bias));
  }

  for (iree_host_size_t i = 0; i < pair_count; ++i) {
    loom_amdgpu_fp8_packed_u16_pair_base_t pair_base;
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_u16_pair_base(
        context, source_op, plan, &pair_sources[i], low_zero, vgpr_type,
        &pair_base));

    loom_value_id_t low_normal_payload = LOOM_VALUE_ID_INVALID;
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_normal_packed_f16_payload(
        context, source_op, plan, pair_base.source_no_sign,
        low_packed_exponent_bias, vgpr_type, &low_normal_payload));

    loom_value_id_t low_finite_payload = LOOM_VALUE_ID_INVALID;
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_u16_zero_repaired_payload(
        context, source_op, plan, pair_base.source_no_sign, low_normal_payload,
        value_flags, low_one_mask, vgpr_type, &low_finite_payload));

    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_u16_pair_sign(
        context, source_op, plan, &pair_base, low_finite_payload, vgpr_type,
        &out_low_packets[i]));
  }
  *out_selected = true;
  return iree_ok_status();
...
loom_amdgpu_emit_fp8_packed_bf16_pair_nan_repair_with_quiet_nan(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    loom_value_id_t low_packet, loom_value_id_t low_nan_mask,
    loom_value_id_t low_quiet_nan, loom_type_t vgpr_type,
    loom_value_id_t* out_packet) {
  *out_packet = LOOM_VALUE_ID_INVALID;
  return loom_amdgpu_emit_fp8_packed_bf16_pair_select(
      context, source_op, low_packet, low_quiet_nan, low_nan_mask, vgpr_type,
      out_packet);
}
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_pair_nan_repair(
  ...
  *out_packet = LOOM_VALUE_ID_INVALID;
  ...
  return loom_amdgpu_emit_fp8_packed_bf16_pair_nan_repair_with_quiet_nan(
      context, source_op, low_packet, low_nan_mask, low_quiet_nan, vgpr_type,
      out_packet);
}
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_subnormal_repair_payloads(
  ...
  for (iree_host_size_t i = 0; i < pair_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(
        loom_amdgpu_emit_fp8_packed_bf16_mask_from_condition_with_shift(
            context, source_op, plan, low_repair_condition, low_mask_shift,
            vgpr_type, &low_repair_mask));
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_select(
        context, source_op, pair_states[i].normal_payload,
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_nan_repair_packets(
  ...
  for (iree_host_size_t i = 0; i < pair_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(
        loom_amdgpu_emit_fp8_packed_bf16_mask_from_condition_with_shift(
            context, source_op, plan, nan_conditions[i], low_mask_shift,
            vgpr_type, &nan_mask));
    IREE_RETURN_IF_ERROR(
        loom_amdgpu_emit_fp8_packed_bf16_pair_nan_repair_with_quiet_nan(
            context, source_op, no_nan_packets[i], nan_mask, low_quiet_nan,
            vgpr_type, &out_packets[i]));
  }
  return iree_ok_status();
}
...
loom_amdgpu_emit_fp8_packed_bf16_ieee_top_exponent_repair_packets(
    ...
    loom_type_t sgpr_type, loom_value_id_t* out_packets) {
  ...
  for (iree_host_size_t i = 0; i < pair_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(
        loom_amdgpu_emit_fp8_packed_bf16_mask_from_condition_with_shift(
            ...
            low_mask_shift, vgpr_type, &low_top_exponent_mask));
    ...
    IREE_RETURN_IF_ERROR(
        loom_amdgpu_emit_fp8_packed_bf16_mask_from_condition_with_shift(
            context, source_op, plan, low_nan_condition, low_mask_shift,
            vgpr_type, &low_nan_mask));
    ...
    IREE_RETURN_IF_ERROR(
        loom_amdgpu_emit_fp8_packed_bf16_pair_nan_repair_with_quiet_nan(
            context, source_op, low_infinity_packet, low_nan_mask,
            low_quiet_nan, vgpr_type, &low_top_exponent_packet));
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_select(
        context, source_op, no_top_exponent_packets[i], low_top_exponent_packet,
        low_top_exponent_mask, vgpr_type, &out_packets[i]));
  }
  return iree_ok_status();
}
...
loom_amdgpu_emit_fp8_packed_bf16_top_exponent_repair_packets(
  ...
  if (repair_inf && repair_nan) {
    return loom_amdgpu_emit_fp8_packed_bf16_ieee_top_exponent_repair_packets(
        ...
        vgpr_type, sgpr_type, out_packets);
  }
  ...
  if (repair_inf) {
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_inf_repair_packets(
        context, source_op, plan, pair_states, no_top_exponent_packets,
        pair_count, vgpr_type, sgpr_type, inf_repaired_packets));
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_top_exponent_group_if(
  ...
  IREE_RETURN_IF_ERROR(
      loom_amdgpu_emit_fp8_packed_bf16_top_exponent_repair_packets(
          ...
          top_exponent_conditions, vgpr_type, sgpr_type, repair_packets));
  loom_op_t* repair_branch_op = NULL;
  IREE_RETURN_IF_ERROR(
      loom_low_br_build(builder, split.continuation_block, repair_packets,
                        pair_count, source_op->location, &repair_branch_op));
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_ieee_special_group_if(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan,
    const loom_amdgpu_fp8_packed_bf16_pair_state_t* pair_states,
    iree_host_size_t pair_count,
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_vgpr_binary(
      context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_V_OR_B32,
      low_combined_repair_condition, low_combined_top_exponent_condition,
      vgpr_type, &low_combined_condition));
  ...
  loom_value_id_t repair_payloads[LOOM_AMDGPU_MAX_PACKED_32BIT_REGISTERS] = {0};
  loom_value_id_t
      no_top_exponent_packets[LOOM_AMDGPU_MAX_PACKED_32BIT_REGISTERS] = {0};
  loom_value_id_t repair_packets[LOOM_AMDGPU_MAX_PACKED_32BIT_REGISTERS] = {0};
  loom_builder_set_block(builder, split.repair_block);
  IREE_RETURN_IF_ERROR(
      loom_amdgpu_emit_fp8_packed_bf16_subnormal_repair_payloads(
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_signs(
      ...
      vgpr_type, no_top_exponent_packets));
  IREE_RETURN_IF_ERROR(
      loom_amdgpu_emit_fp8_packed_bf16_top_exponent_repair_packets(
          context, source_op, plan, pair_states, no_top_exponent_packets,
          pair_count, top_exponent_repairs, has_hot_top_exponent_conditions,
...
iree_status_t loom_amdgpu_try_emit_fp8_pair_to_packed_bf16(
  ...
  return loom_amdgpu_try_emit_fp8_pairs_to_packed_bf16(
      context, source_op, plan, &pair_source, 1, value_flags, vgpr_type,
      sgpr_type, mask_type, out_low_packet, out_selected);
}
  ...
  *out_selected = false;
  ...
  for (iree_host_size_t i = 0; i < pair_count; ++i) {
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_state(
        context, source_op, plan, &pair_sources[i], low_zero,
        low_packed_exponent_bias, vgpr_type, &pair_states[i]));
  }

  loom_value_id_t finite_payloads[LOOM_AMDGPU_MAX_PACKED_32BIT_REGISTERS] = {0};
  if (use_exact_repair) {
    if (use_combined_ieee_special_repair_split) {
      IREE_RETURN_IF_ERROR(
          loom_amdgpu_emit_fp8_packed_bf16_ieee_special_group_if(
              context, source_op, plan, pair_states, pair_count,
              top_exponent_repairs, vgpr_type, sgpr_type, mask_type,
              low_zero_sgpr64, out_low_packets));
      *out_selected = true;
      return iree_ok_status();
  ...
  if (needs_inf_repair) {
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_inf_repair_packets(
        context, source_op, plan, pair_states, no_nan_packets, pair_count,
        vgpr_type, sgpr_type, no_inf_packets));
  } else {
    for (iree_host_size_t i = 0; i < pair_count; ++i) {
  ...
  if (use_nan_repair_split) {
    ...
    for (iree_host_size_t i = 0; i < pair_count; ++i) {
      ...
      IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_nan_repair(
          context, source_op, no_inf_packets[i], low_nan_mask, vgpr_type,
          &out_low_packets[i]));
    }
  } else {
    for (iree_host_size_t i = 0; i < pair_count; ++i) {

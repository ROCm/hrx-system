...
enum {
  ...
  LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_REASON_COUNT = 16u,
...
static const iree_string_view_t kLoomAmdgpuFp8PackedBf16RepairReasons
    [LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_REASON_COUNT] = {
...
static const iree_string_view_t kLoomAmdgpuFp8PackedF16RepairReasons
    [LOOM_AMDGPU_FP8_PACKED_F16_REPAIR_REASON_COUNT] = {
        [LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_NONE] =
            IREE_SVL("fp8_packed_f16_decode"),
        [LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_ZERO] =
            IREE_SVL("fp8_packed_f16_decode_repair_zero"),
        [LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_SUBNORMAL] =
            IREE_SVL("fp8_packed_f16_decode_repair_subnormal"),
        [LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_ZERO |
         LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_SUBNORMAL] =
            IREE_SVL("fp8_packed_f16_decode_repair_zero_subnormal"),
};
...
loom_amdgpu_fp8_decode_cmp_src1_inline_descriptor(
  ...
  switch (descriptor_ref) {
    ...
    case LOOM_AMDGPU_DESCRIPTOR_REF_V_CMP_NE_I32:
      return plan->compare_ne_i32_src1_inline_descriptor.descriptor != NULL
                 ? &plan->compare_ne_i32_src1_inline_descriptor
                 : NULL;
...
typedef struct loom_amdgpu_fp8_subnormal_bf16_byte_table_values_t {
  // SGPR table words indexed by output byte and packed word.
  loom_value_id_t words[LOOM_AMDGPU_FP8_BF16_BYTE_COUNT]
                       [LOOM_AMDGPU_FP8_BF16_BYTE_TABLE_WORD_COUNT];
} loom_amdgpu_fp8_subnormal_bf16_byte_table_values_t;

static iree_status_t loom_amdgpu_emit_fp8_subnormal_bf16_byte_table_values(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan, loom_type_t sgpr_type,
    loom_amdgpu_fp8_subnormal_bf16_byte_table_values_t* out_tables) {
  *out_tables = (loom_amdgpu_fp8_subnormal_bf16_byte_table_values_t){0};
  for (uint32_t byte_index = 0; byte_index < LOOM_AMDGPU_FP8_BF16_BYTE_COUNT;
       ++byte_index) {
    for (uint32_t word_index = 0;
         word_index < LOOM_AMDGPU_FP8_BF16_BYTE_TABLE_WORD_COUNT;
         ++word_index) {
      IREE_RETURN_IF_ERROR(loom_amdgpu_emit_const_u32(
          context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_S_MOV_B32,
          plan->subnormal_bf16_byte_table_words[byte_index][word_index],
          sgpr_type, &out_tables->words[byte_index][word_index]));
    }
  }
  return iree_ok_status();
}
...
loom_amdgpu_emit_fp8_subnormal_packed_bf16_bits_from_byte_tables(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan,
    const loom_amdgpu_fp8_subnormal_bf16_byte_table_values_t* tables,
    loom_value_id_t low_source_no_sign_pair, loom_type_t vgpr_type,
    loom_value_id_t* out_bf16_bits) {
  *out_bf16_bits = LOOM_VALUE_ID_INVALID;
  loom_value_id_t low_payload_bytes[LOOM_AMDGPU_FP8_BF16_BYTE_COUNT] = {0};
  for (uint32_t byte_index = 0; byte_index < LOOM_AMDGPU_FP8_BF16_BYTE_COUNT;
       ++byte_index) {
...
typedef struct loom_amdgpu_fp8_subnormal_f16_byte_table_values_t {
  // SGPR table words indexed by output byte and packed word.
  loom_value_id_t words[LOOM_AMDGPU_FP8_U16_BYTE_COUNT]
                       [LOOM_AMDGPU_FP8_U16_BYTE_TABLE_WORD_COUNT];
} loom_amdgpu_fp8_subnormal_f16_byte_table_values_t;

static iree_status_t loom_amdgpu_emit_fp8_subnormal_f16_byte_table_values(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan, loom_type_t sgpr_type,
    loom_amdgpu_fp8_subnormal_f16_byte_table_values_t* out_tables) {
  *out_tables = (loom_amdgpu_fp8_subnormal_f16_byte_table_values_t){0};
  for (uint32_t byte_index = 0; byte_index < LOOM_AMDGPU_FP8_U16_BYTE_COUNT;
    ...
    for (uint32_t word_index = 0;
      ...
      IREE_RETURN_IF_ERROR(loom_amdgpu_emit_const_u32(
          context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_S_MOV_B32,
          plan->subnormal_f16_byte_table_words[byte_index][word_index],
          sgpr_type, &out_tables->words[byte_index][word_index]));
    }
  }
  return iree_ok_status();
...
static iree_status_t
loom_amdgpu_emit_fp8_subnormal_packed_f16_bits_from_byte_tables(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan,
    const loom_amdgpu_fp8_subnormal_f16_byte_table_values_t* tables,
    loom_value_id_t low_source_no_sign_pair, loom_type_t vgpr_type,
    loom_value_id_t* out_f16_bits) {
  *out_f16_bits = LOOM_VALUE_ID_INVALID;
...
static iree_status_t
loom_amdgpu_emit_fp8_packed_bf16_pair_lt_condition_with_bias(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan, loom_value_id_t low_value_pair,
...
static iree_status_t
loom_amdgpu_emit_fp8_packed_bf16_pair_ge_condition_with_bias(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan, loom_value_id_t low_value_pair,
    loom_value_id_t low_bias, loom_type_t vgpr_type,
    loom_value_id_t* out_condition) {
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_mask_from_condition(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan, loom_value_id_t low_condition,
    loom_type_t vgpr_type, loom_type_t sgpr_type, loom_value_id_t* out_mask) {
  *out_mask = LOOM_VALUE_ID_INVALID;
  loom_amdgpu_fp8_packed_bf16_repair_state_t repair_state = {
  ...
  return loom_amdgpu_emit_fp8_packed_bf16_mask_from_condition_with_shift(
      context, source_op, plan, low_condition, repair_state.low_mask_shift,
      vgpr_type, out_mask);
}
...
loom_amdgpu_emit_fp8_packed_bf16_pair_eq_condition_with_bias(
  ...
  *out_condition = LOOM_VALUE_ID_INVALID;
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_vgpr_binary_immediate(
      context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_V_XOR_B32_LIT,
      low_value_pair, lane_value, vgpr_type, &low_delta));
  return loom_amdgpu_emit_fp8_packed_bf16_pair_lt_condition_with_bias(
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_pair_eq_mask(
  ...
  *out_mask = LOOM_VALUE_ID_INVALID;
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_eq_condition(
      context, source_op, plan, low_value_pair, lane_value, vgpr_type,
      sgpr_type, &low_condition));
  return loom_amdgpu_emit_fp8_packed_bf16_mask_from_condition(
      context, source_op, plan, low_condition, vgpr_type, sgpr_type, out_mask);
}
...
typedef struct loom_amdgpu_fp8_packed_u16_zero_group_split_t {
  // Block entered when at least one active lane has non-zero magnitude.
  loom_block_t* non_zero_block;
  // Block entered when every active lane has zero magnitude.
  loom_block_t* zero_block;
} loom_amdgpu_fp8_packed_u16_zero_group_split_t;
...
static bool loom_amdgpu_fp8_packed_u16_zero_group_split_is_profitable(
    const loom_amdgpu_fp8_packed_u16_pair_source_t* pair_sources,
    iree_host_size_t pair_count) {
  enum {
    kLoomAmdgpuFp8ZeroGroupSplitMinPairCount = 8,
  };
  // The split adds a uniform branch and OR tree. It repays that static cost for
  // fragment-width payloads but bloats narrower vector materialization.
  if (pair_count < kLoomAmdgpuFp8ZeroGroupSplitMinPairCount) {
    return false;
  }
  for (iree_host_size_t i = 0; i < pair_count; ++i) {
    if (pair_sources[i].live_lane_count != 2u) {
      return false;
    }
  }
  return true;
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_repair_split(
  ...
  *out_split = (loom_amdgpu_fp8_packed_bf16_repair_split_t){0};
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_decode_cmp_u32_lit(
      context, source_op, plan, LOOM_AMDGPU_DESCRIPTOR_REF_V_CMP_NE_I32,
      low_combined_condition, 0, vgpr_type, mask_type,
      &low_has_condition_lanes));
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_split_condition(
    ...
    loom_type_t vgpr_type, loom_value_id_t* out_condition) {
  *out_condition = LOOM_VALUE_ID_INVALID;
  IREE_ASSERT_GT(condition_count, 0u);
  loom_value_id_t low_combined_condition = low_conditions[0];
  for (iree_host_size_t i = 1; i < condition_count; ++i) {
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_vgpr_binary(
        context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_V_OR_B32,
        low_combined_condition, low_conditions[i], vgpr_type,
...
static iree_status_t loom_amdgpu_emit_fp8_packed_u16_zero_payloads(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    iree_host_size_t pair_count, loom_type_t vgpr_type,
    loom_value_id_t* out_zero_payloads) {
  loom_value_id_t low_zero_payload = LOOM_VALUE_ID_INVALID;
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_const_u32(
      context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_V_MOV_B32, 0, vgpr_type,
      &low_zero_payload));
  for (iree_host_size_t i = 0; i < pair_count; ++i) {
    out_zero_payloads[i] = low_zero_payload;
  }
  return iree_ok_status();
}
...
static iree_status_t loom_amdgpu_emit_fp8_packed_u16_zero_group_split(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan,
    const loom_amdgpu_fp8_packed_bf16_pair_state_t* pair_states,
    iree_host_size_t pair_count, loom_type_t vgpr_type, loom_type_t mask_type,
    loom_value_id_t low_zero_sgpr64,
    loom_amdgpu_fp8_packed_u16_zero_group_split_t* out_split) {
  *out_split = (loom_amdgpu_fp8_packed_u16_zero_group_split_t){0};
  IREE_ASSERT_GT(pair_count, 0);

  loom_value_id_t low_group_or = pair_states[0].base.source_no_sign;
  for (iree_host_size_t i = 1; i < pair_count; ++i) {
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_vgpr_binary(
        context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_V_OR_B32, low_group_or,
        pair_states[i].base.source_no_sign, vgpr_type, &low_group_or));
  }
  loom_value_id_t low_has_non_zero_lanes = LOOM_VALUE_ID_INVALID;
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_decode_cmp_u32_lit(
      context, source_op, plan, LOOM_AMDGPU_DESCRIPTOR_REF_V_CMP_NE_I32,
      low_group_or, 0, vgpr_type, mask_type, &low_has_non_zero_lanes));

  uint32_t wavefront_size = 0;
  IREE_RETURN_IF_ERROR(loom_amdgpu_target_wavefront_size(
      loom_low_lower_context_bundle(context), &wavefront_size));

  loom_value_id_t low_has_non_zero_scc = LOOM_VALUE_ID_INVALID;
  if (wavefront_size == 32) {
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_lane_mask_nonzero_scc(
        context, source_op, low_has_non_zero_lanes, wavefront_size,
        &low_has_non_zero_scc));
  } else {
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_decode_sgpr64_nonzero_scc(
        context, source_op, plan, low_has_non_zero_lanes, low_zero_sgpr64,
        &low_has_non_zero_scc));
  }

  loom_builder_t* builder = loom_low_lower_context_builder(context);
  IREE_ASSERT(builder->ip.before_op == NULL);
  loom_block_t* repair_block = builder->ip.block;
  loom_region_t* low_region = repair_block->parent_region;
  IREE_RETURN_IF_ERROR(loom_region_insert_block(
      builder->module, low_region, (uint16_t)(repair_block->region_index + 1),
      &out_split->non_zero_block));
  IREE_RETURN_IF_ERROR(loom_region_insert_block(
      builder->module, low_region,
      (uint16_t)(out_split->non_zero_block->region_index + 1),
      &out_split->zero_block));

  loom_op_t* cond_branch_op = NULL;
  return loom_low_cond_br_build(
      builder, low_has_non_zero_scc, out_split->non_zero_block,
      out_split->zero_block, source_op->location, &cond_branch_op);
}
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_pair_sign(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan,
    const loom_amdgpu_fp8_packed_bf16_pair_state_t* state,
    loom_value_id_t low_finite_payload, loom_type_t vgpr_type,
    loom_value_id_t* out_packet) {
  return loom_amdgpu_emit_fp8_packed_u16_pair_sign(
      context, source_op, plan, &state->base, low_finite_payload, vgpr_type,
      out_packet);
}
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_pair_signs(
  ...
  for (iree_host_size_t i = 0; i < pair_count; ++i) {
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_sign(
        context, source_op, plan, &pair_states[i], finite_payloads[i],
        vgpr_type, &out_packets[i]));
  }
  return iree_ok_status();
}

static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_pair_nan_mask(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan,
    const loom_amdgpu_fp8_packed_bf16_pair_state_t* state,
    loom_type_t vgpr_type, loom_type_t sgpr_type, loom_value_id_t* out_mask) {
  *out_mask = LOOM_VALUE_ID_INVALID;
  ...
  if (format->special_policy ==
    ...
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_eq_mask(
        ...
        vgpr_type, sgpr_type, &low_nan_mask));
  } else {
    ...
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_ge_condition(
        ...
        sgpr_type, &low_nan_condition));
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_mask_from_condition(
        context, source_op, plan, low_nan_condition, vgpr_type, sgpr_type,
        &low_nan_mask));
  }
  *out_mask = low_nan_mask;
  return iree_ok_status();
}
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_combined_eq_condition(
  ...
  *out_condition = LOOM_VALUE_ID_INVALID;
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_vgpr_binary_immediate(
      context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_V_XOR_B32_LIT,
      pair_states[0].base.source_no_sign, packed_value, vgpr_type,
      &low_min_delta));
  for (iree_host_size_t i = 1; i < pair_count; ++i) {
    loom_value_id_t low_delta = LOOM_VALUE_ID_INVALID;
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_vgpr_binary_immediate(
        ...
        &low_delta));
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_decode_resolved_vgpr_binary(
        context, source_op, &plan->pk_min_u16_descriptor, low_min_delta,
        low_delta, vgpr_type, &low_min_delta));
  }
  loom_value_id_t low_condition = LOOM_VALUE_ID_INVALID;
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_combined_ge_condition(
    ...
    loom_type_t sgpr_type, loom_value_id_t* out_condition) {
  *out_condition = LOOM_VALUE_ID_INVALID;
  loom_value_id_t low_max_source_no_sign = pair_states[0].base.source_no_sign;
  for (iree_host_size_t i = 1; i < pair_count; ++i) {
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_decode_resolved_vgpr_binary(
        context, source_op, &plan->pk_max_u16_descriptor,
        low_max_source_no_sign, pair_states[i].base.source_no_sign, vgpr_type,
        &low_max_source_no_sign));
  }
  loom_value_id_t low_condition = LOOM_VALUE_ID_INVALID;
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_ge_condition(
...
loom_amdgpu_emit_fp8_packed_bf16_combined_non_normal_condition(
  ...
  *out_condition = LOOM_VALUE_ID_INVALID;
  ...
  for (iree_host_size_t i = 0; i < pair_count; ++i) {
    ...
    if (low_max_shifted == LOOM_VALUE_ID_INVALID) {
      low_max_shifted = low_shifted;
    } else {
      IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_decode_resolved_vgpr_binary(
          context, source_op, &plan->pk_max_u16_descriptor, low_max_shifted,
          low_shifted, vgpr_type, &low_max_shifted));
    }
  }
  loom_value_id_t low_condition = LOOM_VALUE_ID_INVALID;
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_ge_condition(
...
static iree_status_t
loom_amdgpu_emit_fp8_packed_bf16_pair_nan_repair_with_quiet_nan(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan, loom_value_id_t low_packet,
    loom_value_id_t low_nan_mask, loom_value_id_t low_quiet_nan,
    loom_type_t vgpr_type, loom_value_id_t* out_packet) {
  *out_packet = LOOM_VALUE_ID_INVALID;
  return loom_amdgpu_emit_fp8_packed_bf16_pair_select(
      context, source_op, plan, low_packet, low_quiet_nan, low_nan_mask,
      vgpr_type, out_packet);
}

static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_pair_nan_repair(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan, loom_value_id_t low_packet,
    loom_value_id_t low_nan_mask, loom_type_t vgpr_type,
    loom_value_id_t* out_packet) {
  *out_packet = LOOM_VALUE_ID_INVALID;
  loom_value_id_t low_quiet_nan = LOOM_VALUE_ID_INVALID;
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_const_u32(
      ...
      &low_quiet_nan));
  return loom_amdgpu_emit_fp8_packed_bf16_pair_nan_repair_with_quiet_nan(
      context, source_op, plan, low_packet, low_nan_mask, low_quiet_nan,
      vgpr_type, out_packet);
}
...
loom_amdgpu_emit_fp8_packed_bf16_combined_subnormal_condition(
  ...
  *out_condition = LOOM_VALUE_ID_INVALID;
  if (pair_count > 1 &&
      iree_any_bit_set(plan->flags,
                       LOOM_AMDGPU_FP8_DECODE_PLAN_FLAG_HAS_PK_MIN_U16)) {
    loom_value_id_t low_min_source_no_sign = pair_states[0].base.source_no_sign;
    for (iree_host_size_t i = 1; i < pair_count; ++i) {
      IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_decode_resolved_vgpr_binary(
          context, source_op, &plan->pk_min_u16_descriptor,
          low_min_source_no_sign, pair_states[i].base.source_no_sign, vgpr_type,
          &low_min_source_no_sign));
    }
    loom_value_id_t low_condition = LOOM_VALUE_ID_INVALID;
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_subnormal_repair_payloads(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan,
    const loom_amdgpu_fp8_packed_bf16_pair_state_t* pair_states,
    iree_host_size_t pair_count, loom_type_t vgpr_type, loom_type_t sgpr_type,
  ...
  loom_amdgpu_fp8_subnormal_bf16_byte_table_values_t subnormal_tables;
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_subnormal_bf16_byte_table_values(
      context, source_op, plan, sgpr_type, &subnormal_tables));
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_repair_group_if(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan,
    const loom_amdgpu_fp8_packed_bf16_pair_state_t* pair_states,
    ...
    loom_value_id_t low_zero_sgpr64, bool use_zero_group_split,
    loom_value_id_t* out_finite_payloads) {
  ...
  loom_amdgpu_fp8_packed_u16_zero_group_split_t zero_group_split = {0};
  if (use_zero_group_split) {
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_u16_zero_group_split(
        context, source_op, plan, pair_states, pair_count, vgpr_type, mask_type,
        low_zero_sgpr64, &zero_group_split));
    loom_builder_set_block(builder, zero_group_split.non_zero_block);
  }
  loom_amdgpu_fp8_packed_bf16_repair_state_t repair_state = {
      .low_mask_shift = LOOM_VALUE_ID_INVALID,
  };
  IREE_RETURN_IF_ERROR(
      loom_amdgpu_emit_fp8_packed_bf16_subnormal_repair_payloads(
          context, source_op, plan, pair_states, pair_count, vgpr_type,
          sgpr_type, &repair_state, /*low_subnormal_markers=*/NULL,
          repair_payloads));
  ...
  if (use_zero_group_split) {
    loom_builder_set_block(builder, zero_group_split.zero_block);
    loom_value_id_t zero_payloads[LOOM_AMDGPU_MAX_PACKED_32BIT_REGISTERS] = {0};
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_u16_zero_payloads(
        context, source_op, pair_count, vgpr_type, zero_payloads));
    loom_op_t* zero_branch_op = NULL;
    IREE_RETURN_IF_ERROR(
        loom_low_br_build(builder, split.continuation_block, zero_payloads,
                          pair_count, source_op->location, &zero_branch_op));
  }
...
static iree_status_t loom_amdgpu_emit_fp8_packed_f16_subnormal_repair_payloads(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan,
    const loom_amdgpu_fp8_packed_bf16_pair_state_t* pair_states,
    iree_host_size_t pair_count, loom_type_t vgpr_type, loom_type_t sgpr_type,
    loom_amdgpu_fp8_packed_bf16_repair_state_t* repair_state,
    loom_value_id_t* out_repair_payloads) {
  loom_amdgpu_fp8_subnormal_f16_byte_table_values_t subnormal_tables;
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_subnormal_f16_byte_table_values(
      context, source_op, plan, sgpr_type, &subnormal_tables));
  loom_value_id_t low_subnormal_bias = LOOM_VALUE_ID_INVALID;
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_const_u32(
      context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_S_MOV_B32,
...
static iree_status_t loom_amdgpu_emit_fp8_packed_f16_repair_group_if(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_fp8_decode_plan_t* plan,
    const loom_amdgpu_fp8_packed_bf16_pair_state_t* pair_states,
    iree_host_size_t pair_count, loom_value_id_t low_combined_repair_condition,
  ...
  IREE_RETURN_IF_ERROR(
      loom_amdgpu_emit_fp8_packed_f16_subnormal_repair_payloads(
          context, source_op, plan, pair_states, pair_count, vgpr_type,
          sgpr_type, &repair_state, repair_payloads));
  loom_op_t* repair_branch_op = NULL;
  IREE_RETURN_IF_ERROR(
      loom_low_br_build(builder, split.continuation_block, repair_payloads,
                        pair_count, source_op->location, &repair_branch_op));
...
iree_status_t loom_amdgpu_emit_fp8_pairs_to_packed_f16_finite(
  ...
  const bool use_zero_group_split =
      !value_non_zero && use_repair_split &&
      loom_amdgpu_fp8_decode_plan_has_packed_zero_repair(plan) &&
      loom_amdgpu_fp8_packed_u16_zero_group_split_is_profitable(pair_sources,
                                                                pair_count);
  ...
  if (use_exact_repair) {
    if (use_repair_split) {
      ...
      IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_f16_repair_group_if(
          context, source_op, plan, pair_states, pair_count,
          low_combined_repair_condition, vgpr_type, sgpr_type, mask_type,
          low_zero_sgpr64, use_zero_group_split, finite_payloads));
    } else {
      loom_amdgpu_fp8_packed_bf16_repair_state_t repair_state = {
          .low_mask_shift = LOOM_VALUE_ID_INVALID,
      };
      IREE_RETURN_IF_ERROR(
          loom_amdgpu_emit_fp8_packed_f16_subnormal_repair_payloads(
              context, source_op, plan, pair_states, pair_count, vgpr_type,
              sgpr_type, &repair_state, finite_payloads));
    }
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_nan_repair_packets(
  ...
  for (iree_host_size_t i = 0; i < pair_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(
        loom_amdgpu_emit_fp8_packed_bf16_pair_nan_repair_with_quiet_nan(
            context, source_op, plan, no_nan_packets[i], nan_mask,
            low_quiet_nan, vgpr_type, &out_packets[i]));
  }
  return iree_ok_status();
}
...
loom_amdgpu_emit_fp8_packed_bf16_ieee_top_exponent_repair_packets(
  ...
  for (iree_host_size_t i = 0; i < pair_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(
        loom_amdgpu_emit_fp8_packed_bf16_pair_nan_repair_with_quiet_nan(
            context, source_op, plan, low_infinity_packet, low_nan_mask,
            low_quiet_nan, vgpr_type, &low_top_exponent_packet));
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_select(
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_top_exponent_condition(
    ...
    const loom_amdgpu_fp8_decode_plan_t* plan,
    const loom_amdgpu_fp8_packed_bf16_pair_state_t* state,
    loom_amdgpu_fp8_packed_bf16_repairs_t top_exponent_repairs,
    loom_type_t vgpr_type, loom_type_t sgpr_type,
    loom_value_id_t* out_condition) {
  *out_condition = LOOM_VALUE_ID_INVALID;
  const uint32_t top_exponent_no_sign =
  ...
  const bool repair_inf = iree_any_bit_set(
      top_exponent_repairs, LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_INF);
  const bool repair_nan = iree_any_bit_set(
      top_exponent_repairs, LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_NAN);
...
static bool loom_amdgpu_fp8_packed_bf16_combined_non_normal_threshold(
  ...
  *out_threshold = 0;
  ...
  if (plan->format.special_policy == LOOM_SCALAR_TYPE_FP8_SPECIAL_POLICY_IEEE) {
    if (top_exponent_repairs != (LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_INF |
                                 LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_NAN)) {
      return false;
    }
...
static iree_status_t loom_amdgpu_emit_fp8_packed_bf16_special_group_if(
  ...
  if (use_zero_group_split) {
    loom_builder_set_block(builder, zero_group_split.zero_block);
    loom_value_id_t zero_payloads[LOOM_AMDGPU_MAX_PACKED_32BIT_REGISTERS] = {0};
    loom_value_id_t zero_packets[LOOM_AMDGPU_MAX_PACKED_32BIT_REGISTERS] = {0};
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_u16_zero_payloads(
        context, source_op, pair_count, vgpr_type, zero_payloads));
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_signs(
        context, source_op, plan, pair_states, zero_payloads, pair_count,
        vgpr_type, zero_packets));
    loom_op_t* zero_branch_op = NULL;
    IREE_RETURN_IF_ERROR(
        loom_low_br_build(builder, split.continuation_block, zero_packets,
                          pair_count, source_op->location, &zero_branch_op));
  }
...
iree_status_t loom_amdgpu_emit_fp8_pairs_to_packed_bf16(
  ...
  const bool needs_inf_repair =
      ...
      plan->format.special_policy == LOOM_SCALAR_TYPE_FP8_SPECIAL_POLICY_IEEE &&
      !value_not_inf;
  const loom_amdgpu_fp8_packed_bf16_repairs_t top_exponent_repairs =
      (needs_nan_repair ? LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_NAN
                        : LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_NONE) |
      (needs_inf_repair ? LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_INF
                        : LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_NONE);
  ...
  const bool use_top_exponent_repair_split =
      plan->format.special_policy == LOOM_SCALAR_TYPE_FP8_SPECIAL_POLICY_IEEE &&
      top_exponent_repairs != LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_NONE &&
    ...
    if (use_combined_special_repair_split) {
      IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_special_group_if(
          ...
          use_combined_ieee_special_repair_split
              ...
              : LOOM_AMDGPU_FP8_PACKED_BF16_REPAIR_NONE,
          vgpr_type, sgpr_type, mask_type, low_zero_sgpr64,
          use_zero_group_split, out_low_packets));
      return iree_ok_status();
      ...
      IREE_RETURN_IF_ERROR(
          loom_amdgpu_emit_fp8_packed_bf16_combined_subnormal_condition(
              context, source_op, plan, pair_states, pair_count, vgpr_type,
              sgpr_type, &low_combined_repair_condition));
      IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_repair_group_if(
          context, source_op, plan, pair_states, pair_count,
          low_combined_repair_condition, vgpr_type, sgpr_type, mask_type,
          low_zero_sgpr64, use_zero_group_split, finite_payloads));
    } else {
      loom_amdgpu_fp8_subnormal_bf16_byte_table_values_t subnormal_tables;
      IREE_RETURN_IF_ERROR(
          loom_amdgpu_emit_fp8_subnormal_bf16_byte_table_values(
              context, source_op, plan, sgpr_type, &subnormal_tables));
      for (iree_host_size_t i = 0; i < pair_count; ++i) {
        loom_value_id_t low_subnormal_payload = LOOM_VALUE_ID_INVALID;
        IREE_RETURN_IF_ERROR(
  ...
  if (use_nan_repair_split) {
    ...
    for (iree_host_size_t i = 0; i < pair_count; ++i) {
      loom_value_id_t low_nan_mask = LOOM_VALUE_ID_INVALID;
      IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_nan_mask(
          context, source_op, plan, &pair_states[i], vgpr_type, sgpr_type,
          &low_nan_mask));
      IREE_RETURN_IF_ERROR(loom_amdgpu_emit_fp8_packed_bf16_pair_nan_repair(
          context, source_op, plan, no_inf_packets[i], low_nan_mask, vgpr_type,
          &out_low_packets[i]));
    }
  } else {
    for (iree_host_size_t i = 0; i < pair_count; ++i) {
  ...
  *out_lane = LOOM_VALUE_ID_INVALID;
  ...
  if (use_finite_sign_insert) {
    ...
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_resolved_vgpr_binary_immediate(
        context, source_op, &plan->bfi_b32_src0_literal_descriptor,
        low_shifted_sign_source, low_finite_payload,
        LOOM_AMDGPU_FP8_DECODE_F32_SIGN_INSERT_MASK, vgpr_type, out_lane));
    return iree_ok_status();
  }

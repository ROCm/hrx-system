...
static bool loom_amdgpu_fragment_memory_payload_storage_register_count(
  ...
  *out_register_count = 0;
  ...
  uint32_t register_count = 0;
  if (!loom_amdgpu_fragment_memory_packed_float_storage(
          payload_type, LOOM_AMDGPU_FRAGMENT_PACKED_B8_ELEMENT_BIT_COUNT,
          &unused_payload_bit_count, &register_count) &&
      !loom_amdgpu_fragment_memory_packed_float_storage(
          payload_type, LOOM_AMDGPU_FRAGMENT_PACKED_B16_ELEMENT_BIT_COUNT,
          &unused_payload_bit_count, &register_count)) {
    return false;
  }
  if (register_count == 0 || register_count > UINT16_MAX) {
...
static iree_string_view_t loom_amdgpu_fragment_memory_packet_fallback_reason(
  ...
  if (plan->payload_form ==
    ...
    if (iree_any_bit_set(
      ...
      if (missing_value_finite && missing_value_not_subnormal) {
        return IREE_SV("fp8_full_bf16_decode_missing_finite_not_subnormal");
      }
      if (missing_value_finite) {
        return IREE_SV("fp8_full_bf16_decode_missing_finite");
      }
      if (missing_value_not_subnormal) {
        return IREE_SV("fp8_full_bf16_decode_missing_not_subnormal");
      }
      if (iree_any_bit_set(
              packet->flags,
              LOOM_AMDGPU_FRAGMENT_MEMORY_PACKET_FLAG_FP8_MISSING_TARGET_PACKETS)) {
        return IREE_SV("fp8_full_bf16_decode_missing_target_packets");
      }
  ...
  if (iree_all_bits_set(
          packet->flags,
          LOOM_AMDGPU_FRAGMENT_MEMORY_PACKET_FLAG_CROSSLANE_PACKED_B16_STORE)) {
    return IREE_SV("crosslane_packed_bf16_store");
  }
  const uint16_t remaining = plan->register_count - packet->register_index;
  for (iree_host_size_t i = 0;
       i < IREE_ARRAYSIZE(kLoomAmdgpuFragmentMemoryNarrowedStoreCandidates);
  ...
  for (uint16_t result_register_index = 0;
    ...
    IREE_RETURN_IF_ERROR(
        loom_amdgpu_emit_fragment_memory_fp8_to_packed_bf16_register(
            ...
            &decode_plan, decode_value_flags, native_f32_pair_type,
            &bf16_pack_descriptors, vgpr_type, mask_type,

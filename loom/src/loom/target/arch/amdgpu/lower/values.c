...
static bool loom_amdgpu_vector_fp8_query_adjacent_pair_storage(
    const loom_amdgpu_vector_16bit_float_conversion_plan_t* plan,
    uint32_t lane_index,
...
static bool loom_amdgpu_vector_fp8_all_pairs_have_adjacent_storage(
  ...
  for (uint32_t pair_index = 0; pair_index < pair_count; ++pair_index) {
    ...
    if (!loom_amdgpu_vector_fp8_query_adjacent_pair_storage(
            plan, pair_index * 2u, &pair_storage)) {
      return false;
    }
  }
  return true;
...
static bool loom_amdgpu_vector_fp8_query_adjacent_pair_set(
    const loom_amdgpu_vector_16bit_float_conversion_plan_t* plan,
    const loom_amdgpu_vector_fp8_decode_value_flag_cache_t* value_flag_cache,
...
static iree_status_t loom_amdgpu_try_lower_vector_fp8_pair_to_packed_bf16(
  ...
  *out_selected = false;
  ...
  return loom_amdgpu_try_emit_fp8_pair_to_packed_bf16(
      context, source_op, decode_plan, source_register,
      pair_storage.byte_offset, value_flags, result_lane_type, *sgpr_type,
      *mask_type, out_low_packet, out_selected);
}
...
static iree_status_t loom_amdgpu_try_lower_vector_fp8_pairs_to_packed_bf16(
  ...
  *out_selected = false;
  ...
  if (!loom_amdgpu_vector_fp8_query_adjacent_pair_set(
...
static iree_status_t loom_amdgpu_lower_vector_fp8_to_packed_f16(
  ...
  loom_amdgpu_fp8_decode_plan_t decode_plan = {0};
  loom_value_id_t packed_registers[LOOM_AMDGPU_MAX_PACKED_16BIT_FLOAT_LANES];
  bool selected_packed_f16 = false;
  IREE_RETURN_IF_ERROR(loom_amdgpu_try_lower_vector_fp8_pairs_to_packed_f16(
      context, source_op, plan, &value_flag_cache, &decode_plan, low_source,
      source_lane_type, result_lane_type, &sgpr_type, &mask_type,
      plan->result_register_count, packed_registers, &selected_packed_f16));
  if (selected_packed_f16) {
    return loom_amdgpu_bind_low_register_range(context, source_op, plan->result,
                                               packed_registers,
                                               plan->result_register_count);
  }

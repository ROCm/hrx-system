...
static bool loom_amdgpu_type_is_vector1_f32(loom_type_t type) {
...
static bool loom_amdgpu_vector_decode_scale_source(
  ...
  *out_scale_source = LOOM_VALUE_ID_INVALID;
  ...
  if (scale_source == LOOM_VALUE_ID_INVALID ||
      ...
      !loom_amdgpu_type_is_vector1_f32(
          loom_module_value_type(module, scale_source))) {
    return false;
  }
...
bool loom_amdgpu_vector_decode_can_lower_as_fp8_conversion(
  ...
  if (!has_auxiliary) {
    return loom_amdgpu_direct_fp8_unscaled_schema_matches(
...
static void loom_amdgpu_vector_16bit_float_conversion_plan_from_accepted_op(
  ...
  *out_plan = (loom_amdgpu_vector_16bit_float_conversion_plan_t){0};
  ...
  if (kind == LOOM_AMDGPU_VECTOR_16BIT_FLOAT_CONVERSION_KIND_EXTF ||
    ...
    if (scale_source == LOOM_VALUE_ID_INVALID &&
        loom_value_fact_table_query_static_lane_origin(fact_table, module,
                                                       source, &lane_origin)) {
...
static iree_status_t loom_amdgpu_ensure_fp8_software_decode(
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_make_sgpr_range_type(context, 2, mask_type));
  return loom_amdgpu_select_fp8_decode_plan(context, source_element_type,
                                            decode_plan);
}
...
static iree_status_t loom_amdgpu_lower_vector_fp8_to_f32_software_lane(
    ...
    loom_amdgpu_fp8_decode_plan_t* decode_plan, loom_value_id_t low_source,
...
static bool loom_amdgpu_vector_fp8_adjacent_pair_storage(
    const loom_amdgpu_vector_16bit_float_conversion_plan_t* plan,
    uint32_t lane_index, uint32_t* out_register_index,
...
static bool loom_amdgpu_vector_fp8_all_pairs_have_adjacent_storage(
    const loom_amdgpu_vector_16bit_float_conversion_plan_t* plan) {
  if ((plan->lane_count & 1u) != 0) {
    return false;
  }
  const uint32_t pair_count = plan->lane_count / 2u;
  for (uint32_t pair_index = 0; pair_index < pair_count; ++pair_index) {
    uint32_t source_register_index = 0;
    uint32_t byte_offset = 0;
    if (!loom_amdgpu_vector_fp8_adjacent_pair_storage(
            plan, pair_index * 2u, &source_register_index, &byte_offset)) {
      return false;
    }
  }
  return true;
...
static iree_status_t loom_amdgpu_try_lower_vector_fp8_pair_to_f32_native(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_vector_16bit_float_conversion_plan_t* plan,
    const loom_low_lower_resolved_descriptor_t* descriptor,
    loom_value_id_t low_source, loom_type_t source_lane_type,
    loom_type_t result_lane_type, loom_type_t result_pair_type,
    uint32_t lane_index, loom_value_id_t* out_low_lanes, bool* out_selected) {
  ...
  *out_selected = false;
  ...
  uint32_t byte_offset = 0;
  if (!loom_amdgpu_vector_fp8_adjacent_pair_storage(
          plan, lane_index, &register_index, &byte_offset)) {
    return iree_ok_status();
  }
  ...
  if (byte_offset != 0) {
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_vgpr_shift(
        context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_V_LSHRREV_B32_LIT,
        byte_offset * 8u, source_register, source_lane_type, &source_register));
  }
  ...
  IREE_RETURN_IF_ERROR(loom_low_lower_emit_resolved_descriptor_op(
      context, descriptor, operands, IREE_ARRAYSIZE(operands),
      loom_named_attr_slice_empty(), &result_pair_type, 1,
      /*tied_results=*/NULL, /*tied_result_count=*/0, source_op->location,
      &low_op));
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_extract_register_unit(
      ...
      /*register_offset=*/0, result_lane_type, &out_low_lanes[0]));
  IREE_RETURN_IF_ERROR(loom_amdgpu_extract_register_unit(
      context, source_op, converted_pair, /*register_count=*/2,
      /*register_offset=*/1, result_lane_type, &out_low_lanes[1]));
  *out_selected = true;
  return iree_ok_status();
}
...
static iree_status_t loom_amdgpu_lookup_vector_scalef32_source(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_vector_16bit_float_conversion_plan_t* plan,
...
static iree_status_t loom_amdgpu_resolve_fp8_scalef32_descriptor_if_present(
    loom_low_lower_context_t* context,
    const loom_amdgpu_vector_16bit_float_conversion_plan_t* plan,
    loom_low_lower_resolved_descriptor_t* out_descriptor,
    bool* out_is_present) {
  ...
  *out_is_present = false;
  loom_amdgpu_descriptor_ref_t descriptor_ref = LOOM_AMDGPU_DESCRIPTOR_REF_NONE;
  if (!loom_amdgpu_fp8_scalef32_descriptor_ref(plan->source_element_type,
...
static iree_status_t loom_amdgpu_try_lower_vector_fp8_scalef32_pair_native(
    loom_low_lower_context_t* context, const loom_op_t* source_op,
    const loom_amdgpu_vector_16bit_float_conversion_plan_t* plan,
    const loom_low_lower_resolved_descriptor_t* descriptor,
    loom_value_id_t low_source, loom_value_id_t low_scale,
    loom_type_t source_lane_type, loom_type_t result_lane_type,
    uint32_t lane_index, loom_value_id_t* out_low_pair, bool* out_selected) {
  ...
  *out_selected = false;
  ...
  IREE_RETURN_IF_ERROR(loom_low_lower_emit_resolved_descriptor_op(
      ...
      /*tied_results=*/NULL, /*tied_result_count=*/0, source_op->location,
      &low_op));
  *out_low_pair = loom_value_slice_get(loom_low_op_results(low_op), 0);
  *out_selected = true;
  return iree_ok_status();
}
...
loom_amdgpu_try_lower_vector_fp8_pair_to_packed_16bit_native(
    ...
    loom_value_id_t low_source, loom_type_t source_lane_type,
    loom_type_t result_lane_type, uint32_t lane_index,
    loom_value_id_t* out_low_pair, bool* out_selected) {
  *out_low_pair = LOOM_VALUE_ID_INVALID;
  *out_selected = false;
  ...
  *out_selected = false;
  ...
  const loom_amdgpu_fp8_decode_value_flags_t value_flags =
      ...
      loom_amdgpu_vector_fp8_decode_value_flags(value_flag_cache,
                                                lane_index + 1u);
  if (!loom_amdgpu_can_emit_fp8_pair_to_packed_bf16(decode_plan, value_flags)) {
    return iree_ok_status();
  }
...
static iree_status_t loom_amdgpu_try_lower_vector_fp8_pairs_to_packed_bf16(
    ...
    loom_amdgpu_fp8_decode_plan_t* decode_plan, loom_value_id_t low_source,
    loom_type_t source_lane_type, loom_type_t result_lane_type,
    loom_type_t* sgpr_type, loom_type_t* mask_type, uint32_t pair_count,
  ...
  *out_selected = false;
  if (pair_count == 0 || pair_count > LOOM_AMDGPU_MAX_PACKED_32BIT_REGISTERS ||
      ...
      plan->lane_count != pair_count * 2u) {
    return iree_ok_status();
  }

  uint32_t source_register_indices[LOOM_AMDGPU_MAX_PACKED_32BIT_REGISTERS] = {
      0};
  uint32_t byte_offsets[LOOM_AMDGPU_MAX_PACKED_32BIT_REGISTERS] = {0};
  ...
  for (uint32_t register_index = 0; register_index < pair_count;
    ...
    const uint32_t lane_base = register_index * 2u;
    if (!loom_amdgpu_vector_fp8_adjacent_pair_storage(
            plan, lane_base, &source_register_indices[register_index],
            &byte_offsets[register_index])) {
      return iree_ok_status();
    }
    loom_amdgpu_fp8_decode_value_flags_t pair_value_flags =
        loom_amdgpu_vector_fp8_decode_value_flags(value_flag_cache, lane_base) &
        loom_amdgpu_vector_fp8_decode_value_flags(value_flag_cache,
                                                  lane_base + 1u);
    value_flags =
        have_value_flags ? value_flags & pair_value_flags : pair_value_flags;
  ...
  for (uint32_t register_index = 0; register_index < pair_count;
    ...
    const uint32_t source_register_index =
        source_register_indices[register_index];
  ...
  for (uint32_t register_index = 0; register_index < pair_count;
       ++register_index) {
    pair_sources[register_index] = (loom_amdgpu_fp8_packed_bf16_pair_source_t){
        .source_register =
            source_registers[source_register_indices[register_index]],
        .byte_offset = byte_offsets[register_index],
    };
  }
  return loom_amdgpu_try_emit_fp8_pairs_to_packed_bf16(
      context, source_op, decode_plan, pair_sources, pair_count, value_flags,
      result_lane_type, *sgpr_type, *mask_type, out_low_packets, out_selected);
}
...
static iree_status_t loom_amdgpu_try_lower_vector_fp8_pairs_to_f32(
    ...
    loom_type_t source_lane_type, loom_type_t result_lane_type,
    loom_type_t* sgpr_type, loom_type_t* mask_type,
    loom_value_id_t* out_low_lanes, bool* out_selected) {
  *out_selected = false;
  ...
  bool selected_packed_bf16 = false;
  IREE_RETURN_IF_ERROR(loom_amdgpu_try_lower_vector_fp8_pairs_to_packed_bf16(
      ...
      packed_bf16, &selected_packed_bf16));
  if (!selected_packed_bf16) {
    return iree_ok_status();
  }
...
static iree_status_t loom_amdgpu_emit_vector_fp8_to_f32_lanes(
  ...
  for (uint32_t i = 0; i < plan->lane_count;) {
    ...
    IREE_RETURN_IF_ERROR(loom_amdgpu_try_lower_vector_fp8_pair_to_f32(
        ...
        packed_decode_lanes, &selected_packed_decode));
    if (selected_packed_decode) {
...
static iree_status_t loom_amdgpu_emit_vector_fp8_scalef32_to_f32_lanes(
  ...
  for (uint32_t lane_index = 0; lane_index < plan->lane_count; ++lane_index) {
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_f32_mul(
        context, source_op, lanes[lane_index], low_scale, result_lane_type,
        &lanes[lane_index]));
  }
  return iree_ok_status();
}
  ...
  if (descriptor_is_present &&
    ...
    for (uint32_t lane_index = 0; lane_index < plan->lane_count;
      ...
      if (!selected) {
        return iree_make_status(
            IREE_STATUS_INTERNAL,
            "selected scaled FP8 vector decode without adjacent source lanes");
      }
      ...
      IREE_RETURN_IF_ERROR(loom_amdgpu_extract_register_unit(
          context, source_op, converted_pair, /*register_count=*/2,
          /*register_offset=*/0, result_lane_type, &lanes[lane_index]));
      IREE_RETURN_IF_ERROR(loom_amdgpu_extract_register_unit(
          context, source_op, converted_pair, /*register_count=*/2,
          /*register_offset=*/1, result_lane_type, &lanes[lane_index + 1u]));
    }
    return loom_amdgpu_bind_low_register_range(context, source_op, plan->result,
                                               lanes, plan->lane_count);
  }
  ...
  if (plan->result_element_type == LOOM_SCALAR_TYPE_BF16) {
    ...
    for (uint32_t register_index = 0;
      ...
      if (lane_base + 1u < plan->lane_count) {
        high_lane = f32_lanes[lane_base + 1u];
      } else {
        IREE_RETURN_IF_ERROR(loom_amdgpu_emit_const_u32(
            context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_V_MOV_B32, 0,
            result_lane_type, &high_lane));
      }
      IREE_RETURN_IF_ERROR(
          loom_amdgpu_emit_f32_pair_to_packed_bf16_with_descriptors(
              context, source_op, &bf16_pack_descriptors, f32_lanes[lane_base],
              high_lane, result_lane_type, &packed_registers[register_index]));
    }
  } else {
...
static iree_status_t loom_amdgpu_lower_vector_fp8_to_packed_bf16(
  ...
  for (uint32_t register_index = 0;
    ...
    IREE_RETURN_IF_ERROR(loom_amdgpu_try_lower_vector_fp8_pair_to_packed_bf16(
        ...
        &packed_registers[register_index], &selected_packed_decode));
    if (selected_packed_decode) {
      continue;
    }
...
static iree_status_t loom_amdgpu_lower_vector_fp8_to_packed_f16(
  ...
  if (native_pair_present && (plan->lane_count & 1u) == 0) {
    bool all_pairs_adjacent = true;
    for (uint32_t register_index = 0;
         register_index < plan->result_register_count; ++register_index) {
      uint32_t source_register_index = 0;
      uint32_t byte_offset = 0;
      if (!loom_amdgpu_vector_fp8_adjacent_pair_storage(
              plan, register_index * 2u, &source_register_index,
              &byte_offset)) {
        all_pairs_adjacent = false;
        break;
      }
    }
    if (all_pairs_adjacent) {
      loom_value_id_t
          packed_registers[LOOM_AMDGPU_MAX_PACKED_16BIT_FLOAT_LANES];
      for (uint32_t register_index = 0;
           register_index < plan->result_register_count; ++register_index) {
        bool selected = false;
        IREE_RETURN_IF_ERROR(
        ...
        if (!selected) {
          return iree_make_status(
              IREE_STATUS_INTERNAL,
              "selected native FP8-to-F16 pair conversion without adjacent "
              "source lanes");
        }
      }
      return loom_amdgpu_bind_low_register_range(context, source_op,
...
static iree_status_t loom_amdgpu_lower_vector_16bit_float_extf(
  ...
  if (plan->source_element_type == LOOM_SCALAR_TYPE_F8E4M3 ||
    ...
    if (plan->result_element_type == LOOM_SCALAR_TYPE_F16) {
      return loom_amdgpu_lower_vector_fp8_to_packed_f16(
          context, source_op, plan, low_source, source_lane_type,
          result_lane_type);
    }
    return loom_amdgpu_lower_vector_fp8_to_f32(context, source_op, plan,
...
static iree_status_t loom_amdgpu_emit_scalar_fp8_to_bf16(
  ...
  loom_amdgpu_fp8_decode_plan_t decode_plan = {0};
  IREE_RETURN_IF_ERROR(
      loom_amdgpu_select_fp8_decode_plan(context, source_type, &decode_plan));

...
static iree_status_t loom_aie2p_tile_image_build_symbol_tables(
  ...
  *out_entry_address = 0;
  ...
  if (entry_symbol_index >= object->symbol_count ||
      ...
      symbol_layouts[entry_symbol_index].section_index >=
  ...
  if (!iree_checked_add_u64(
          assembly->sections[symbol_layouts[entry_symbol_index].section_index]
              .address,
          symbol_layouts[entry_symbol_index].section_offset, &entry_address) ||
      entry_address > UINT32_MAX) {
    return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                            "AIE2P tile entry address exceeds ELF32");
  }
  *out_entry_address = (uint32_t)entry_address;
  ...
  for (iree_host_size_t i = 0; i < object->symbol_count; ++i) {
    ...
    if (!iree_checked_add_u64(layout->section_offset, symbol->size,
                              &symbol_end) ||
        symbol_end > loom_native_elf_section_byte_length(section) ||
        !iree_checked_add_u64(section->address, layout->section_offset,
                              &symbol_address) ||
        symbol_address > UINT32_MAX || symbol->size > UINT32_MAX ||
    ...
    loom_aie2p_tile_image_store_u32(record + 4, (uint32_t)symbol_address);
...
static iree_status_t loom_aie2p_tile_image_assign_addresses(
...
iree_status_t loom_aie2p_tile_image_write(
  ...
  IREE_RETURN_IF_ERROR(
      loom_aie2p_tile_image_assign_addresses(contribution, layout, &assembly));
  IREE_RETURN_IF_ERROR(
  ...
  iree_host_size_t output_section_count = 0;
  if (!iree_host_size_checked_add(assembly.section_count, 2,
                                  &output_section_count) ||
      assembly.section_count >= UINT32_MAX) {
    return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                            "AIE2P tile has too many sections");
  }
  loom_native_elf_section_t* sections = NULL;
  IREE_RETURN_IF_ERROR(
      iree_arena_allocate_array(scratch_arena, output_section_count,
                                sizeof(*sections), (void**)&sections));
  memcpy(sections, assembly.sections,
         assembly.section_count * sizeof(*sections));
  ...
  sections[assembly.section_count + 1u] = (loom_native_elf_section_t){
      .name = IREE_SV(".symtab"),
      .type = LOOM_NATIVE_ELF_SECTION_TYPE_SYMTAB,
      .alignment = 4,
      .entry_size = LOOM_AIE2P_ELF32_SYMBOL_SIZE,
      .link = (uint32_t)assembly.section_count + 1u,
      .info = (uint32_t)local_symbol_count + 1u,
      .contents = symbol_table,
  };

  iree_host_size_t load_segment_count = 0;
  for (iree_host_size_t i = 0; i < assembly.section_count; ++i) {
    if (iree_any_bit_set(sections[i].flags,
                         LOOM_NATIVE_ELF_SECTION_FLAG_ALLOC)) {
      ++load_segment_count;
    }
  }
  loom_native_elf_segment_t* load_segments = NULL;
  IREE_RETURN_IF_ERROR(iree_arena_allocate_array(
      scratch_arena, load_segment_count, sizeof(*load_segments),
      (void**)&load_segments));
  iree_host_size_t load_segment_index = 0;
  for (iree_host_size_t i = 0; i < assembly.section_count; ++i) {
    const loom_native_elf_section_t* section = &sections[i];
    if (!iree_any_bit_set(section->flags, LOOM_NATIVE_ELF_SECTION_FLAG_ALLOC)) {
      continue;
    }
    uint32_t segment_flags = LOOM_NATIVE_ELF_PROGRAM_FLAG_READ;
    if (iree_any_bit_set(section->flags, LOOM_NATIVE_ELF_SECTION_FLAG_WRITE)) {
      segment_flags |= LOOM_NATIVE_ELF_PROGRAM_FLAG_WRITE;
    }
    if (iree_any_bit_set(section->flags,
                         LOOM_NATIVE_ELF_SECTION_FLAG_EXECINSTR)) {
      segment_flags |= LOOM_NATIVE_ELF_PROGRAM_FLAG_EXECUTE;
    }
    load_segments[load_segment_index++] = (loom_native_elf_segment_t){
  ...
  IREE_ASSERT_EQ(load_segment_index, load_segment_count);
  const loom_native_elf32le_file_t file = {
      ...
      .entry = entry_address,
      .sections = sections,
      .section_count = output_section_count,

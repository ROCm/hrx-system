...
static iree_status_t loom_aie2p_table_lookup_build_selector(
  ...
  loom_op_t* op = NULL;
  IREE_RETURN_IF_ERROR(
      loom_vector_constant_build(tree->builder, loom_attr_i64(1u << level),
                                 tree->index_type, tree->location, &op));
...
iree_status_t loom_aie2p_table_lookup_rewrite(
  ...
  *out_rewritten = false;
  ...
  // Scalar lookup needs two extracts and three insertion/control operations
  // per result lane. Known index lanes omit their extraction; narrow-index
  // extension can only increase that cost.
  const uint32_t packed_cost = 2 + 4 * levels + 2 * (uint32_t)table_count - 1;
  const uint32_t scalar_cost =
      5 * (uint32_t)result_count - 2 - known_index_count;
  ...
  for (uint32_t level = 0; level < levels && iree_status_is_ok(status);
       ++level) {
    status = loom_aie2p_table_lookup_build_selector(

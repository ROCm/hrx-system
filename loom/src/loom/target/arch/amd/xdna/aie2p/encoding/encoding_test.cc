...
TEST(EncodingTest, RejectsIncompleteOrOutOfRangeInputs) {
  ...
  IREE_EXPECT_STATUS_IS(IREE_STATUS_INVALID_ARGUMENT,
                        loom_aie2p_encoding_pack_instruction(
                            ids.vector_add_32, missing_field,
                            IREE_ARRAYSIZE(missing_field), &encoded_value));
  ...
  IREE_EXPECT_STATUS_IS(IREE_STATUS_OUT_OF_RANGE,
                        loom_aie2p_encoding_pack_instruction(
                            ids.vector_add_32, out_of_range,
                            IREE_ARRAYSIZE(out_of_range), &encoded_value));
}
...
TEST(EncodingTest, VerifiedPackingConsumesDeclaredLowFieldBits) {
  ...
  IREE_EXPECT_STATUS_IS(IREE_STATUS_OUT_OF_RANGE,
                        loom_aie2p_encoding_pack_instruction(
                            vector_select, source_values,
                            IREE_ARRAYSIZE(source_values), &rejected_value));
}

...
TEST(XdnaDeviceProfileTest, ResolvesExactStrixHaloPciRevision) {
  ...
  IREE_EXPECT_STATUS_IS(
      ...
      loom_xdna_device_profile_resolve_pci(0x1022, 0x17F0, 0x12, &profile));
  EXPECT_EQ(profile, nullptr);
}

TEST(XdnaDeviceProfileTest, ResolvesExactStrixPciRevision) {
  const loom_xdna_device_profile_t* profile = nullptr;
  IREE_ASSERT_OK(
      loom_xdna_device_profile_resolve_pci(0x1022, 0x17F0, 0x10, &profile));
  ASSERT_NE(profile, nullptr);
  EXPECT_STREQ(profile->key, "amd.xdna.strix.17f0_10");
  EXPECT_STREQ(profile->display_name, "NPU Strix");
  EXPECT_EQ(profile->identity, UINT64_C(0x5354524958000001));
  EXPECT_EQ(profile->firmware_abi_identity, UINT64_C(0x4E5055320006000C));
  EXPECT_EQ(profile->firmware_protocol_major, 6u);
  EXPECT_EQ(profile->firmware_protocol_minor, 12u);
  EXPECT_EQ(loom_xdna_device_profile_lookup(IREE_SV("amd.xdna.strix.17f0_10")),
            profile);
}
...
TEST(XdnaDeviceProfileTest, ResolvesSerializedIdentityWithoutFallback) {
  ...
  IREE_ASSERT_OK(loom_xdna_device_profile_resolve_identity(
      UINT64_C(0x5354524958000001), 1, UINT64_C(0x4E5055320006000C), &profile));
...
TEST(XdnaDeviceProfileTest, AcceptsEveryContiguousPhysicalPartitionFit) {
  for (uint8_t revision : {0x10, 0x11}) {
    const loom_xdna_device_profile_t* profile = nullptr;
    IREE_ASSERT_OK(loom_xdna_device_profile_resolve_pci(0x1022, 0x17F0,
                                                        revision, &profile));
    for (uint16_t width = 1; width <= 8; ++width) {
      for (uint16_t origin = 0; origin + width <= 8; ++origin) {
        IREE_EXPECT_OK(loom_xdna_device_profile_validate_partition(
            profile, origin, width));
      }
    }
    IREE_EXPECT_STATUS_IS(

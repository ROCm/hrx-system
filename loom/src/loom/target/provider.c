...
static iree_status_t loom_target_provider_set_validate_contracts(
  ...
  for (iree_host_size_t i = 0; i < provider_set->provider_count; ++i) {
    ...
    const bool has_op_kind = materialization.op_kind != LOOM_OP_KIND_UNKNOWN;
    ...
    const bool has_any_materialization = has_op_kind || has_symbol_stem ||
                                         has_record_match || has_record_builder;
    const bool has_complete_materialization = has_op_kind && has_symbol_stem &&
                                              has_record_match &&
                                              has_record_builder;
    if (has_any_materialization != has_complete_materialization) {
      return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                              "target provider %" PRIhsz
                              " must define a target op kind and every "
                              "structured materialization hook",
                              i);
    }
    if (has_complete_materialization && provider->profile_type == NULL) {
      return iree_make_status(
          IREE_STATUS_INVALID_ARGUMENT,
          "target provider %" PRIhsz
          " structured materialization requires a profile type",
          i);
    }
    ...
    for (iree_host_size_t j = 0; j < i; ++j) {
      if (provider_set->providers[j]->materialization.op_kind ==
          materialization.op_kind) {
        return iree_make_status(
            IREE_STATUS_INVALID_ARGUMENT,
            "target op kind 0x%04X has materialization providers at indices "
            "%" PRIhsz " and %" PRIhsz,
            (unsigned)materialization.op_kind, j, i);
      }
    }
  }
...
static bool loom_target_provider_find_equal_materialized_record(
  ...
  *out_target_ref = loom_symbol_ref_null();
  for (iree_host_size_t i = 0; i < module->symbols.count; ++i) {
    ...
    if (defining_op == NULL ||
        defining_op->kind != provider->materialization.op_kind) {
      continue;
    }

...
static iree_status_t loom_spirv_emit_physical_storage_buffer_byte_offset_packet(
  ...
  // OpPtrAccessChain scales its element operand by the pointee size. Loom's
  // storage-buffer address coordinate is already measured in bytes, so add it
  // to the untyped physical address before selecting the access scalar type.
...
static iree_status_t loom_spirv_emit_cooperative_matrix_layout_operands(
  ...
  return loom_spirv_emit_u32_constant(
      state->type_context, row->cooperative_matrix_byte_stride, out_stride_id);
}

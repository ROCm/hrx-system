...
TEST_CASE_METHOD(HrxTestFixture, "Stream fill buffer",
                 "[stream_ops][fill]") {
  hrx_stream_t stream = nullptr;
  REQUIRE_OK(hrx().stream_create(device_, 0, &stream));

  hrx_buffer_t buf = nullptr;
  REQUIRE_OK(hrx().buffer_allocate(stream, 1024, HRX_MEMORY_TYPE_HOST_LOCAL | HRX_MEMORY_TYPE_DEVICE_VISIBLE, HRX_BUFFER_USAGE_DEFAULT | HRX_BUFFER_USAGE_MAPPING_SCOPED, &buf));

  uint32_t pattern = 0xCAFEBABE;
  REQUIRE_OK(hrx().stream_fill_buffer(
...
TEST_CASE_METHOD(HrxTestFixture, "Stream copy buffer",
                 "[stream_ops][copy]") {
  hrx_stream_t stream = nullptr;
  REQUIRE_OK(hrx().stream_create(device_, 0, &stream));
  ...
  REQUIRE_OK(hrx().buffer_allocate(stream, 512, HRX_MEMORY_TYPE_HOST_LOCAL | HRX_MEMORY_TYPE_DEVICE_VISIBLE, HRX_BUFFER_USAGE_DEFAULT | HRX_BUFFER_USAGE_MAPPING_SCOPED, &src));
  REQUIRE_OK(hrx().buffer_allocate(stream, 512, HRX_MEMORY_TYPE_HOST_LOCAL | HRX_MEMORY_TYPE_DEVICE_VISIBLE, HRX_BUFFER_USAGE_DEFAULT | HRX_BUFFER_USAGE_MAPPING_SCOPED, &dst));
...
TEST_CASE_METHOD(HrxTestFixture, "Stream update buffer",
                 "[stream_ops][copy]") {
  hrx_stream_t stream = nullptr;
  REQUIRE_OK(hrx().stream_create(device_, 0, &stream));

  hrx_buffer_t buf = nullptr;
  REQUIRE_OK(hrx().buffer_allocate(stream, 64, HRX_MEMORY_TYPE_HOST_LOCAL | HRX_MEMORY_TYPE_DEVICE_VISIBLE, HRX_BUFFER_USAGE_DEFAULT | HRX_BUFFER_USAGE_MAPPING_SCOPED, &buf));
